# OpenAI-Compatible API Server for Agent ZAZA
...
SSE streaming. By exposing this endpoint, agent-zaza becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  agent-zaza   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.agent-zaza/data/.env`
2. Run `zaza gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with agent-zaza through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns agent-zaza as a model) |
...
Authorization: Bearer zaza-api-key-here
...
But agent-zaza sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "agent-zaza"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured agent-zaza model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of agent-zaza's core prompt
...
Once implemented, agent-zaza works as a drop-in backend for:
