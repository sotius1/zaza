"""Persistent slash-command worker — one ZazaCLI per TUI session.
...
def _run(cli: ZazaCLI, command: str) -> str:
...
def main():
    ...
    os.environ["ZAZA_SESSION_KEY"] = args.session_key
    os.environ["ZAZA_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = ZazaCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
