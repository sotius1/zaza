...
def _state_file() -> Path:
    return get_zaza_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.agent-zaza/data/config.yaml. Tolerates missing file."""
    try:
...
CURATOR_REVIEW_PROMPT = (
    "You are running as ZAZA' background skill CURATOR. This is an "
    ...
    "into ~/.agent-zaza/data/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: zaza-config-*, zaza-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.agent-zaza/data/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.agent-zaza/data/logs/curator/``)
    ...
    authored skill data in ``~/.agent-zaza/data/skills/``.
    """
    return get_zaza_home() / "logs" / "curator"
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    if archived:
        ...
        lines.append("_Archived skills are at `~/.agent-zaza/data/skills/.archive/`. "
                     "Restore any via `zaza curator restore <name>`._\n")
    ...
    lines.append("- Restore an archived skill: `zaza curator restore <name>`")
    lines.append("- All archives live under `~/.agent-zaza/data/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    def _llm_pass():
        ...
        # recorded in state so `zaza curator status` can point at it.
        try:
