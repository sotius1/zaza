...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time ZAZA starts and finds ``~/.openclaw/``.

    Points users at ``zaza claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``zaza claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to ZAZA, run "
        "`zaza claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `zaza claw cleanup` (renames it to ~/.openclaw.pre-migration — "
