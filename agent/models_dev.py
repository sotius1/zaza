...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.agent-zaza/data/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: ZAZA ↔ models.dev
# ---------------------------------------------------------------------------

# ZAZA provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → ZAZA (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_zaza_home() / "models_dev_cache.json"
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a ZAZA provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal ZAZA agent traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a ZAZA provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve ZAZA ID → models.dev ID
...
def get_model_info(
    ...
    Accepts ZAZA or models.dev provider ID.  Tries exact match then
