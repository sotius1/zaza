...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - zaza_home (str): The active ZAZA_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.agent-zaza/data``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "zaza").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'zaza memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], zaza_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'zaza memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``zaza_home`` is the active ZAZA_HOME directory path.
