...
as ``kind: backend``) or ``~/.agent-zaza/data/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``zaza tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``zaza tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``zaza tools`` picker.
...
def _images_cache_dir() -> Path:
    """Return ``$ZAZA_HOME/cache/images/``, creating parents as needed."""
    from zaza_constants import get_zaza_home

    path = get_zaza_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$ZAZA_HOME/cache/images/``.
