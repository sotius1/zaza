...
# `export ZAZA_REDACT_SECRETS=true`) cannot enable/disable redaction
# mid-session.  OFF by default — user must opt in via
# `security.redact_secrets: true` in config.yaml (bridged to this env var
# in zaza_cli/main.py and gateway/run.py) or `ZAZA_REDACT_SECRETS=true`
# in ~/.agent-zaza/data/.env.
_REDACT_ENABLED = os.getenv("ZAZA_REDACT_SECRETS", "").lower() in ("1", "true", "yes", "on")
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across ZAZA — used by
    ``zaza config``, ``zaza status``, ``zaza dump``, and anywhere
