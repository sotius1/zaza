#!/usr/bin/env python3
"""
Agent ZAZA CLI - Interactive Terminal Interface

A beautiful command-line interface for Agent ZAZA.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    agent-zaza                             # Start interactive mode with all tools
    agent-zaza --toolsets web,terminal     # Start with specific toolsets
    agent-zaza --skills code-review,github-auth
    agent-zaza -q "your question"          # Single query mode
    agent-zaza --list-tools                # List available tools and exit
"""
...
os.environ["ZAZA_QUIET"] = "1"  # Our own modules
...
from zaza_cli.banner import _format_context_length, format_banner_version_label
...
from zaza_cli.browser_connect import (
...
_zaza_home = get_zaza_home()
_project_env = Path(__file__).parent / '.env'
load_zaza_dotenv(zaza_home=_zaza_home, project_env=_project_env)
...
def _load_prefill_messages(file_path: str) -> List[Dict[str, Any]]:
    ...
    Relative paths are resolved from ~/.agent-zaza/data/.
...
def _parse_reasoning_config(effort: str) -> dict | None:
    ...
    from zaza_constants import parse_reasoning_effort
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.agent-zaza/data/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If ZAZA_IGNORE_USER_CONFIG=1 is set (via ``zaza chat --ignore-user-config``),
    the user config at ``~/.agent-zaza/data/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({ZAZA_HOME}/config.yaml)
    user_config_path = _zaza_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("ZAZA_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain ZAZA, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me ZAZA - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        try:
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and ZazaCLI.__init__ checks "default" first.
            ...
            # location is model.provider (written by `zaza model`).
    ...
    from zaza_cli.config import _expand_env_vars
    defaults = _expand_env_vars(defaults)

    # Apply terminal config to environment variables (so terminal_tool picks them up)
    terminal_config = defaults.get("terminal", {})
    
    # Normalize config key: the new config system (zaza_cli/config.py) and all
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["ZAZA_REDACT_SECRETS"] = str(redact).lower()
...
# Initialize centralized logging early — agent.log + errors.log in ~/.agent-zaza/data/logs/.
# This ensures CLI sessions produce a log trail even before AIAgent is instantiated.
try:
    from zaza_logging import setup_logging
...
try:
    from zaza_cli.config import print_config_warnings
...
try:
    from zaza_cli.skin_engine import init_skin_from_config
...
from zaza_cli.banner import build_welcome_banner
from zaza_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from zaza_cli.callbacks import prompt_for_secret
...
def _run_cleanup():
    ...
    try:
        from zaza_cli.plugins import invoke_hook as _invoke_hook
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run zaza -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"zaza-{short_id}"
    branch_name = f"zaza/{wt_name}"
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`zaza_cli.config.load_config` (the authoritative loader that
    ...
    if session_db is None:
        return
    try:
        from zaza_cli.config import load_config as _load_full_config
        from zaza_constants import get_zaza_home as _get_zaza_home
        ...
        session_db.maybe_auto_prune_and_vacuum(
            ...
            sessions_dir=_get_zaza_home() / "sessions",
        )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`zaza_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``zaza/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("zaza-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``zaza/zaza-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``zaza -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("zaza/zaza-") or b.startswith("pr-"))
    ]
...
from zaza_constants import is_termux as _is_termux_environment
...
class ChatConsole:
    ...
    def status(self, *_args, **_kwargs):
        ...
        ``ZazaCLI._busy_command()``.
        """
        yield self

# ZAZA-native logo + hero are defined in zaza_cli/banner.py.
# These re-exports exist for legacy import paths inside cli.py only;
# new code should import them from zaza_cli.banner directly.
from zaza_cli.banner import ZAZA_AGENT_LOGO, ZAZA_CADUCEUS  # noqa: E402,F401
...
def _build_compact_banner() -> str:
    ...
    No legacy "NOUS ZAZA" wording, no kawaii decoration — just an agent
    name, version, and a thin separator. Skins can override the agent name
    via SkinConfig.branding["agent_name"].
    """
    try:
        from zaza_cli.skin_engine import get_active_skin
    ...
    title_color = _skin.get_color("banner_title", "#00ff88") if _skin else "#00ff88"
    dim_color = _skin.get_color("banner_dim", "#64748b") if _skin else "#64748b"
    accent_color = _skin.get_color("banner_accent", "#a855f7") if _skin else "#a855f7"
    agent_name = _skin.get_branding("agent_name", "ZAZA") if _skin else "ZAZA"

    version_line = format_banner_version_label()

    w = max(min(shutil.get_terminal_size().columns - 2, 78), 24)
    rule = "─" * (w - len(agent_name) - 4)

    return (
        f"\n  [bold {title_color}]▎[/] [bold {accent_color}]{agent_name}[/]  "
        f"[dim {dim_color}]{rule}[/]\n"
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from zaza_cli.plugins import get_plugin_manager
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.agent-zaza/data/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.agent-zaza/data/config.yaml on first use)
...
class ZazaCLI:
    """
    Interactive CLI for the Agent ZAZA.
    ...
    def __init__(
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from zaza_cli.runtime_provider import _auto_detect_local_model
        ...
        self.requested_provider = (
            ...
            or os.getenv("ZAZA_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("ZAZA_MAX_ITERATIONS"):
            self.max_turns = int(os.getenv("ZAZA_MAX_ITERATIONS"))
        else:
            self.max_turns = 90
        ...
        # by `zaza chat --ignore-rules` in zaza_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIAgent so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("ZAZA_IGNORE_RULES") == "1"
        
        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
            os.getenv("ZAZA_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        # it's shared across all ZAZA processes for this ZAZA_HOME.
        # Never blocks startup on failure.
        _run_state_db_auto_maintenance(self._session_db)

        # Opportunistic shadow-repo cleanup — deletes orphan/stale
        # checkpoint repos under ~/.agent-zaza/data/checkpoints/.  Opt-in via
        ...
        self._history_file = _zaza_home / ".zaza_history"
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        The leading glyph reflects the agent's current state (idle/thinking/
        working/waiting/done) — see ``zaza_cli.agent_state``.
        """
        from zaza_cli.agent_state import state_glyph
        glyph = state_glyph()
        try:
            ...
            if width < 52:
                text = f"{glyph} {snapshot['model_short']} · {duration_label}"
                return self._trim_status_bar_text(text, width)
            if width < 76:
                parts = [f"{glyph} {snapshot['model_short']}", percent_label]
            ...
            parts = [f"{glyph} {snapshot['model_short']}", context_label, percent_label]
            parts.append(duration_label)
            ...
            return f"{glyph} {self.model if getattr(self, 'model', None) else 'ZAZA'}"
    ...
    def _get_status_bar_fragments(self):
        ...
        try:
            ...
            if width < 52:
                frags = [
                    ("class:status-bar-strong", f" {state_glyph()} "),
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from zaza_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from zaza_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from zaza_cli.models import normalize_opencode_model_id, opencode_model_api_mode
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            # Visible state transition: agent is now THINKING (streaming response)
            try:
                from zaza_cli.agent_state import set_state, AgentState
            ...
            try:
                from zaza_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", " › ZAZA ")
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        """
        from zaza_cli.runtime_provider import (
        ...
        if not isinstance(api_key, str) or not api_key:
            ...
            if _has_custom_base:
                ...
                print("\n⚠️  Provider resolver returned an empty API key. "
                      "Set OPENROUTER_API_KEY or run: zaza setup")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: zaza setup")
            return False
        ...
        # `zaza chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `zaza auth add openai-codex`
        # without `zaza model`), fall back to the provider's first catalog
        # model so the API call doesn't fail with "model must be non-empty".
        if not self.model and resolved_provider:
            try:
                from zaza_cli.models import get_default_model_for_provider
    ...
    def _resolve_turn_agent_config(self, user_message: str) -> dict:
        ...
        """
        from zaza_cli.models import resolve_fast_mode_overrides
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                ...
                _cprint(f"{_DIM}Use a session ID from a previous CLI run (zaza sessions list).{_RST}")
                return False
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len <= 8192:
            ...
            self._console_print(
                "[dim]   ZAZA needs 16k–32k minimum. Tool schemas + system prompt alone use ~4k–8k.[/]"
            )
        ...
        # Warn if the configured model is a Nous ZAZA LLM (not agentic)
        from zaza_cli.model_switch import is_nous_zaza_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_nous_zaza_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  ZAZA ZAZA 3 & 4 models are NOT agentic and are not "
                "designed for use with Agent ZAZA.[/]"
            )
            self._console_print(
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "user":
                ...
                lines.append("  ◆ ZAZA: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ ZAZA: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.agent-zaza/data/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from zaza_cli.clipboard import save_clipboard_image

        img_dir = get_zaza_home() / "images"
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: zaza --checkpoints")
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for ZAZA config/state.
        ...
        if subcmd in ("list", "ls"):
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_zaza_home()}/state-snapshots/):\n")
    ...
    def _handle_paste_command(self):
        ...
        from zaza_cli.clipboard import has_clipboard_image
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run zaza chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'zaza setup' to configure[/]")
        except Exception:
    ...
    def _fast_command_available(self) -> bool:
        try:
            from zaza_cli.models import model_supports_fast_mode
        except Exception:
            return False
        agent = getattr(self, "agent", None)
    ...
    def show_help(self):
        ...
        from zaza_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from zaza_cli.skin_engine import get_active_help_header
            header = get_active_help_header("› Komendy")
        except Exception:
            header = "› Komendy"
        header = (header or "").strip() or "› Komendy"
        inner_width = 55
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with ZAZA!{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in zaza_cli/colors.py
        ...
        from zaza_cli.tools_config import _get_platform_tools
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from zaza_cli.main import _relative_time
    ...
    def show_history(self):
        ...
        print()
        print("+" + "-" * 50 + "+")
        print("|" + " " * 12 + "› Conversation History" + " " * 15 + "|")
        print("+" + "-" * 50 + "+")
            ...
            if self._session_db:
                try:
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("ZAZA_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
                            "max_iterations": self.max_turns,
                            "reasoning_config": self.reasoning_config,
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                return
            _cprint("  Tip:   Use /history or `zaza sessions list` to find sessions.")
            return
        ...
        # Resolve title or ID
        from zaza_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `zaza sessions list` to see available sessions.")
            return
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.agent-zaza/data/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``zaza --resume <id>``
        ...
        saved_dir = get_zaza_home() / "sessions" / "saved"
        try:
            saved_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            print(f"(x_x) Failed to create save directory {saved_dir}: {e}")
            return
        path = saved_dir / f"zaza_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: zaza --resume {self.session_id}")
        except Exception as e:
    ...
    def retry_last(self):
        ...
        print(f"↻ retry: \"{last_message[:60]}{'...' if len(last_message) > 60 else ''}\"")
        return last_message
    ...
    def undo_last(self):
        ...
        print(f"↶ undid {removed_count} message(s): \"{removed_msg[:60]}{'...' if len(removed_msg) > 60 else ''}\"")
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from zaza_cli.curses_ui import curses_single_select
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        try:
            from zaza_cli.model_switch import resolve_display_context_length
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `zaza model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from zaza_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from zaza_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from zaza_cli.model_switch import switch_model, parse_model_flags, list_authenticated_providers
        from zaza_cli.providers import get_label
        ...
        custom_provs = None
        try:
            from zaza_cli.config import get_compatible_custom_providers, load_config
            cfg = load_config()
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from zaza_cli.commands import resolve_command
        ...
        if len(parts) > 1:
            ...
            if personality_name in ("none", "default", "neutral"):
                ...
                if save_config_value("agent.system_prompt", self.system_prompt):
                    print(f"✓ personality → '{personality_name}' (saved to config)")
                else:
                    print(f"✓ personality → '{personality_name}' (session only)")
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to zaza_cli.curator so the CLI and the `zaza curator`
        ...
        try:
            from zaza_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to zaza_cli.skills_hub."""
        from zaza_cli.skills_hub import handle_skills_slash
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_zaza_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_zaza_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in zaza_cli/commands.py instead of touching every dispatch site.
        from zaza_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in ("quit", "exit", "q"):
            ...
            if self._app:
                ...
                try:
                    from zaza_cli.tips import get_random_tip
                    _tip = get_random_tip()
                    try:
                        from zaza_cli.skin_engine import get_active_skin
            ...
            from zaza_cli.config import reload_env
            count = reload_env()
            ...
            try:
                ...
                if not plugins:
                    ...
                    print(f"Drop plugin directories into {display_zaza_home()}/plugins/ to get started.")
                else:
            ...
            # Check for plugin-registered slash commands
                ...
                from zaza_cli.plugins import get_plugin_command_handler
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from zaza_cli.commands import COMMANDS
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from zaza_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_zaza_home()}/skins/\n")
            return
    ...
    def _handle_footer_command(self, cmd_original: str) -> None:
        ...
        """
        from zaza_cli.config import load_config
        from zaza_cli.colors import Colors as _Colors
    ...
    def _toggle_yolo(self):
        ...
        current = bool(os.environ.get("ZAZA_YOLO_MODE"))
        if current:
            os.environ.pop("ZAZA_YOLO_MODE", None)
            _cprint(
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while ZAZA is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while ZAZA is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while ZAZA is busy."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from zaza_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
            model = getattr(agent, "model", None) or getattr(self, "model", None)
    ...
    def _handle_debug_command(self):
        ...
        from zaza_cli.debug import run_debug_share
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            for quiet_logger in ('tools', 'run_agent', 'trajectory_compressor', 'cron', 'zaza_cli'):
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from zaza_cli.config import get_config_path as _get_config_path
                ...
                try:
                    if (
                        ...
                        if not is_seen(CLI_CONFIG, TOOL_PROGRESS_FLAG):
                            ...
                            mark_seen(_zaza_home / "config.yaml", TOOL_PROGRESS_FLAG)
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "zaza_voice"), exist_ok=True)
            mp3_path = os.path.join(
                tempfile.gettempdir(), "zaza_voice",
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _zaza_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            print(f"  zaza --resume {self.session_id}")
            if session_title:
                print(f"  zaza -c \"{session_title}\"")
            print()
            ...
            try:
                from zaza_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from zaza_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from zaza_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        # Only show when the user explicitly configured Honcho for ZAZA
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Agent ZAZA! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Agent ZAZA! Type your message or /help for commands."
            _welcome_color = "#FFF8DC"
        self._console_print(f"[{_welcome_color}]{_welcome_text}[/]")
        # First-time OpenClaw-residue banner — fires once if ~/.openclaw/ exists
        # after an OpenClaw→ZAZA migration (especially migrations done by
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from zaza_cli.config import get_config_path as _get_cfg_path_resid
                ...
                if self._agent_running and not (text and _looks_like_slash_command(text)):
                    ...
                    try:
                        ...
                        if not is_seen(CLI_CONFIG, BUSY_INPUT_FLAG):
                            ...
                            mark_seen(_zaza_home / "config.yaml", BUSY_INPUT_FLAG)
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from zaza_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Agent ZAZA")
        ...
        # Dynamic prompt: shows ZAZA symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("ZAZA needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'ZAZA needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("ZAZA needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = ZazaCLI._compute_model_picker_viewport(
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``ZAZA_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("ZAZA_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: zaza setup"
            )
            _run_cleanup()
...
def main(
    ...
    """
    Agent ZAZA CLI - Interactive AI Assistant
    ...
    Examples:
        ...
        python cli.py --skills agent-zaza-dev,github-auth
    ...
    os.environ["ZAZA_INTERACTIVE"] = "1"
    
    # Handle gateway mode (messaging + cron)
    if gateway:
        import asyncio
        from gateway.run import start_gateway
        print("Starting ZAZA Gateway (messaging platforms)...")
    ...
    # Default to agent-zaza-cli-toolset toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
    ...
    # mode registers its own inside ZazaCLI.run(), but `-q` runs
    ...
    # so main unwinds normally.  ZAZA_SIGTERM_GRACE overrides the 1.5 s
        ...
        if quiet:
            ...
            if cli._ensure_runtime_credentials():
                ...
                if cli._init_agent(
                    ...
                    # machine-readable (no styled "ZAZA" box, no tool-gen
...
if __name__ == "__main__":
    # ZAZA gating — handles: zaza-agent login / logout / me, and forces EPIC plan
    # before letting ZAZA initialize. Standalone subcommands exit early; the
    # main agent loop only runs when require_epic() returns successfully.
    import sys as _sys

    _zaza_subcommands = {"login", "logout", "me", "whoami"}
    if len(_sys.argv) >= 2 and _sys.argv[1] in _zaza_subcommands:
        try:
            import zaza_auth as _zauth

            _sys.exit(_zauth.main(_sys.argv[1:]))
        except ImportError:
            print(
                "[zaza] zaza_auth.py niedostępny — sprawdź instalację Agent ZAZA.",
                file=_sys.stderr,
            )
            _sys.exit(1)

    # Default flow: enforce EPIC plan, then hand off to the agent core.
    if os.environ.get("ZAZA_SKIP_AUTH") != "1":
        try:
            ...
            print(
                "[zaza] WARNING: zaza_auth.py niedostępny — uruchamiam bez gatingu.",
                file=_sys.stderr,
            )
        except SystemExit:
            raise
        except Exception as _e:
            print(f"[zaza] Błąd auth: {_e}", file=_sys.stderr)
            _sys.exit(1)

    # Bootstrap default capabilities (Serena, Chrome DevTools, Sequential
    # Thinking, persistent memory) on first run. Idempotent — guarded by a
    # stamp file in ZAZA_HOME, never overwrites user-customized servers.
    try:
        import zaza_defaults as _zdefaults
        _zdefaults.ensure_zaza_defaults()
    except Exception as _e:
        # Defaults are nice-to-have, not load-bearing — never block startup.
        print(f"[zaza] defaults skipped: {_e}", file=_sys.stderr)
