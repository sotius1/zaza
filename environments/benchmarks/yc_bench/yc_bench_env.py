...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs ZAZAAgentLoop with terminal tool only
...
Requires: pip install agent-zaza[yc-bench]
"""
...
class YCBenchEvalConfig(ZAZAAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends ZAZAAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(ZAZAAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The ZAZAAgentLoop then drives the
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs ZAZAAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our ZAZAAgentLoop.
