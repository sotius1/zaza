"""
ZAZAAgentLoop -- Reusable Multi-Turn Agent Engine

Runs the agent-zaza tool-calling loop using standard OpenAI-spec tool calling.
...
identical to agent-zaza's run_agent.py. Tool execution is dispatched via
...
# Resized at runtime by ZAZAAgentBaseEnv.__init__ via resize_tool_pool().
...
def resize_tool_pool(max_workers: int):
    ...
    Called by ZAZAAgentBaseEnv.__init__ based on config.tool_pool_size.
...
class ZAZAAgentLoop:
    """
    Runs agent-zaza's tool-calling loop using standard OpenAI-spec tool calling.
    ...
    async def run(self, messages: List[Dict[str, Any]]) -> AgentResult:
        ...
        for turn in range(self.max_turns):
            ...
            # agent-zaza's standalone parsers. This handles the case where
            ...
            if (
                ...
                try:
                    ...
                    fallback_parser = get_parser("zaza")
            ...
            if assistant_msg.tool_calls:
                ...
                # Execute each tool call via agent-zaza's dispatch
