"""
ZAZAAgentBaseEnv -- Abstract Base Environment for ZAZA-Agent + Atropos

Provides the Atropos integration plumbing that all agent-zaza environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via ZAZAAgentLoop
...
# Ensure the agent-zaza repo root is on sys.path so that imports like
...
# Load API keys from agent-zaza/.env so all environments can access them
...
# Import agent-zaza toolset infrastructure
...
class ZAZAAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for agent-zaza Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of zaza toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: zaza, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
    ...
    default_result_size_chars: int = Field(
        ...
        "to sandbox. Results exceeding this are written to /tmp/zaza-results/ "
...
class ZAZAAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for agent-zaza Atropos integration.
    ...
    name: Optional[str] = "agent-zaza"
    env_config_cls = ZAZAAgentEnvConfig

    def __init__(
        self,
        config: ZAZAAgentEnvConfig,
        ...
        # Set terminal environment variables so zaza tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY agent-zaza tool (terminal, file, web,
