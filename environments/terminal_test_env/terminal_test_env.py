...
Training tasks (3):
    1. Create ~/greeting.txt with "Hello from Agent ZAZA"
...
TRAIN_TASKS = [
    {
        "prompt": "Create a file at ~/greeting.txt containing exactly the text: Hello from Agent ZAZA",
        "verify_path": "~/greeting.txt",
        "expected_content": "Hello from Agent ZAZA",
    },
    {
...
class TerminalTestEnv(ZAZAAgentBaseEnv):
    ...
    def config_init(cls) -> Tuple[TerminalTestEnvConfig, List[APIServerConfig]]:
        ...
        Claude for inference. API keys loaded from ~/agent-zaza/.env.
        """
        env_config = TerminalTestEnvConfig(
            ...
            tokenizer_name="ZAZA/q-30b-t-h45-e1",
