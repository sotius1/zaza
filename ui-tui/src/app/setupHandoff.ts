import type { RunExternalProcess } from '@zaza/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`zaza ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching zaza: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`zaza ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
