...
React + Ink terminal UI for ZAZA. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `ZAZA_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.agent-zaza/data/.zaza_history` or under `ZAZA_HOME`.
...
```text
ui-tui/
  packages/zaza-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      zaza-ink.d.ts    type declarations for @zaza/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent ZAZACLI subprocess for slash commands
```
