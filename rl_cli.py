...
from zaza_constants import OPENROUTER_BASE_URL, get_zaza_home
...
if tinker_atropos_dir.exists():
    ...
    os.environ['ZAZA_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to agent-zaza directory if submodule not found
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.agent-zaza/data/config.yaml if not provided)
    ...
    """
    # Load config from ~/.agent-zaza/data/config.yaml
    config = load_zaza_config()
    ...
    if check_server:
        ...
        if ok:
            ...
            if missing:
                ...
                print("   Add them to ~/.agent-zaza/data/.env")
            else:
