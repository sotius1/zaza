...
```bash
zaza plugins enable google_meet
zaza meet install                 # pip deps + Chromium (transcribe only)
zaza meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
zaza meet auth                    # optional; skips guest-lobby wait
zaza meet setup                   # preflight checks
```

`zaza meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or ZAZA_MEET_REALTIME_KEY in ~/.agent-zaza/data/.env
```
...
pip install playwright websockets && python -m playwright install chromium
zaza plugins enable google_meet
zaza meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
zaza meet node approve my-mac ws://<mac-ip>:18789 <token>
zaza meet node ping my-mac                   # confirm reachable
```

Run `zaza meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A ZAZA agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `zaza meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `ZAZA_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `zaza_meet_src`). |
...
Local:
```
$ZAZA_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$ZAZA_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `zaza meet node approve`.
