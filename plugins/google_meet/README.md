...
Let the zaza agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where zaza runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `zaza meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `zaza meet` CLI |
...
| `cli.py` | `zaza meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$ZAZA_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `zaza meet node {run,list,approve,remove,status,ping}` |
...
zaza meet install                                      # pip + Chromium
zaza meet setup                                        # preflight
zaza meet auth                                         # optional
zaza meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
zaza meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.agent-zaza/data/.env
zaza meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
zaza meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
zaza meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.agent-zaza/data/.env
zaza meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, zaza will **not** switch your system audio input automatically — the
...
zaza plugins enable google_meet
zaza meet node run --display-name my-mac --host 0.0.0.0 --port 18789
