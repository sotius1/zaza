...
Config file: `$ZAZA_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `zaza` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `zaza-{identity}` → `zaza-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per ZAZA profile:

```json
{
  "container_tag": "zaza-{identity}"
}
```

For a profile named `coder`, this resolves to `zaza-coder`. The default profile resolves to `zaza-default`. Without `{identity}`, all profiles share the same container.
