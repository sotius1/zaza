...
def _load_supermemory_config(zaza_home: str) -> dict:
    config = _default_config()
    config_path = Path(zaza_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, zaza_home: str) -> None:
    config_path = Path(zaza_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `zaza memory setup`.
        # All other options are documented for $ZAZA_HOME/supermemory.json
    ...
    def save_config(self, values, zaza_home):
        ...
        _save_supermemory_config(sanitized, zaza_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from zaza_constants import get_zaza_home
        self._zaza_home = kwargs.get("zaza_home") or str(get_zaza_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._zaza_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "zaza", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "zaza_memory", "target": target, "type": "explicit_memory"},
