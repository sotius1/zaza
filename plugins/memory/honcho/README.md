...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/zaza>
...
```bash
zaza honcho setup    # full interactive wizard (cloud or local)
zaza memory setup    # generic picker, also works
```

Or manually:
```bash
zaza config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.agent-zaza/data/.env
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$ZAZA_HOME/honcho.json` | Profile-local (isolated ZAZA instances) |
| 2 | `~/.agent-zaza/data/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active ZAZA profile: `zaza` (default) or `zaza.<profile>`.
...
| 4 | `per-session` strategy | ZAZA session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`agent-zaza`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`zaza`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-agent-zaza`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening zaza in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — ZAZA session ID (timestamp + hex). Every `zaza` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple ZAZA profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`zaza`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active ZAZA profile: `zaza` (default) or `zaza.<profile>` (e.g. `zaza -p coder` → host key `zaza.coder`).
...
| `ZAZA_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `zaza honcho setup` | Full interactive setup wizard |
| `zaza honcho status` | Show resolved config for active profile |
| `zaza honcho enable` / `disable` | Toggle Honcho for active profile |
| `zaza honcho mode <mode>` | Change recall or observation mode |
| `zaza honcho peer --user <name>` | Update user peer name |
| `zaza honcho peer --ai <name>` | Update AI peer name |
| `zaza honcho tokens --context <N>` | Set context token budget |
| `zaza honcho tokens --dialectic <N>` | Set dialectic max chars |
| `zaza honcho map <name>` | Map current directory to a session name |
| `zaza honcho sync` | Create host blocks for all ZAZA profiles |
