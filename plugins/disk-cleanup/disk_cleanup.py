"""disk_cleanup — ephemeral file cleanup for Agent ZAZA.
...
Rules:
  ...
  - empty dirs    → always delete (under ZAZA_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ZAZA_HOME and /tmp/zaza-*
Never touches: ~/.agent-zaza/data/logs/ or any system directory.
"""
...
try:
    ...
    def get_zaza_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ZAZA_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".agent-zaza/data").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ZAZA_HOME/logs/``."""
    return get_zaza_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ZAZA_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ZAZA_HOME or ``/tmp/zaza-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    zaza_home = get_zaza_home()
    try:
        path.resolve().relative_to(zaza_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/zaza-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("zaza-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside ZAZA_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ZAZA_HOME (but leave ZAZA_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "agent-zaza", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(zaza_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == zaza_home:
                continue
            try:
                rel_parts = dirpath.relative_to(zaza_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    zaza_home = get_zaza_home()
    try:
        rel = path.resolve().relative_to(zaza_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "agent-zaza",
        }:
            return None
        ...
        # Path isn't under ZAZA_HOME (e.g. /tmp/zaza-*) — fall through.
        pass
