...
Auto-tracks and cleans up ephemeral files created during Agent ZAZA
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ZAZA_HOME` and `/tmp/zaza-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ZAZA_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `ZAZA_HOME` or `/tmp/zaza-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ZAZA_HOME/disk-cleanup/` is itself excluded
- `$ZAZA_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
