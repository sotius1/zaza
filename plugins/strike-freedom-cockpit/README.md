...
1. **Theme** — copy the theme YAML into your ZAZA home:

   ```
   cp theme/strike-freedom.yaml ~/.agent-zaza/data/dashboard-themes/
   ```
...
2. **Plugin** — the `dashboard/` directory gets auto-discovered because
   ...
   whole plugin directory into `~/.agent-zaza/data/plugins/`:

   ```
   cp -r . ~/.agent-zaza/data/plugins/strike-freedom-cockpit
   ```
