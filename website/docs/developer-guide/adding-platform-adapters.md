...
This guide covers adding a new messaging platform to the ZAZA gateway. A platform adapter connects ZAZA to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.
...
1. **`zaza_cli/config.py`** — Add all `NEWPLAT_*` vars to `_EXTRA_ENV_KEYS`
2. **`zaza_cli/gateway.py`** — Add entry to `_PLATFORMS` list with key, label, emoji, token_var, setup_instructions, and vars
3. **`zaza_cli/platforms.py`** — Add `PlatformInfo` entry with label and default_toolset (used by `skills_config` and `tools_config` TUIs)
4. **`zaza_cli/setup.py`** — Add `_setup_newplat()` function (can delegate to `gateway.py`) and add tuple to the messaging platforms list
5. **`zaza_cli/status.py`** — Add platform detection entry: `"NewPlat": ("NEWPLAT_TOKEN", "NEWPLAT_HOME_CHANNEL")`
6. **`zaza_cli/dump.py`** — Add `"newplat": "NEWPLAT_TOKEN"` to platform detection dict
...
1. **`toolsets.py`** — Add `"zaza-newplat"` toolset definition with `_ZAZA_CORE_TOOLS`
2. **`toolsets.py`** — Add `"zaza-newplat"` to the `"zaza-gateway"` includes list
...
| `website/docs/reference/toolsets-reference.md` | zaza-newplat toolset |
