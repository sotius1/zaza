...
description: "How to add a new inference provider to Agent ZAZA — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

ZAZA can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `zaza model` menu entries
...
1. `zaza_cli/auth.py` decides how credentials are found.
2. `zaza_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `zaza_cli/models.py` and `zaza_cli/main.py` make the provider show up in the CLI. (`zaza_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `zaza_cli/auth.py`
2. `zaza_cli/models.py`
3. `zaza_cli/runtime_provider.py`
4. `zaza_cli/main.py`
...
:::tip
`zaza_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `zaza setup`.
:::
...
- `PROVIDER_REGISTRY` in `zaza_cli/auth.py`
- `_PROVIDER_LABELS` in `zaza_cli/models.py`
- `_PROVIDER_ALIASES` in both `zaza_cli/auth.py` and `zaza_cli/models.py`
- CLI `--provider` choices in `zaza_cli/main.py`
...
## Step 2: Add auth metadata in `zaza_cli/auth.py`
...
- What env vars should ZAZA check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `zaza_cli/models.py`
...
## Step 4: Resolve runtime data in `zaza_cli/runtime_provider.py`
...
Be careful with API-key precedence. ZAZA already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `zaza_cli/main.py`

A provider is not discoverable until it shows up in the interactive `zaza model` flow.

Update these in `zaza_cli/main.py`:
...
:::tip
`zaza_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `zaza model` and `zaza setup` automatically.
:::
...
When you add a native provider, double-check that ZAZA is only sending fields that provider actually understands.
...
```bash
source venv/bin/activate
python -m zaza_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
- [ ] `ProviderConfig` added in `zaza_cli/auth.py`
- [ ] aliases added in `zaza_cli/auth.py` and `zaza_cli/models.py`
- [ ] model catalog added in `zaza_cli/models.py`
- [ ] runtime branch added in `zaza_cli/runtime_provider.py`
- [ ] CLI wiring added in `zaza_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `zaza model` but not `zaza setup`
