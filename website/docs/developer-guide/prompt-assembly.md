...
description: "How ZAZA builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

ZAZA deliberately separates:
...
1. agent identity — `SOUL.md` from `ZAZA_HOME` when available, otherwise falls back to `DEFAULT_AGENT_IDENTITY` in `prompt_builder.py`
...
```
# Layer 1: Agent Identity (from ~/.agent-zaza/data/SOUL.md)
You are ZAZA, an AI assistant created by ZAZA.
...
`SOUL.md` lives at `~/.agent-zaza/data/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_zaza_md(cwd_path)       # 1. .zaza.md / ZAZA.md (walks to git root)
    ...
    # SOUL.md from ZAZA_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.zaza.md`, `ZAZA.md` | CWD up to git root | ZAZA-native project config |
...
- **YAML frontmatter stripped** — `.zaza.md` frontmatter is removed (reserved for future config overrides)
...
1. `.zaza.md` / `ZAZA.md` (walks to git root)
