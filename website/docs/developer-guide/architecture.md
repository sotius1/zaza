...
sidebar_position: 1
title: "Architecture"
description: "Agent ZAZA internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Agent ZAZA internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ zaza_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # ZAZACLI — interactive terminal UI (~10,000 lines)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── zaza_state.py           # SQLite session/state database with FTS5
├── zaza_constants.py       # ZAZA_HOME, profile-aware paths
...
│
├── zaza_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `zaza` subcommands (~6,000 lines)
...
│   ├── skills_config.py      # zaza skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # zaza tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # zaza gateway start/stop
│
...
```text
User input → ZAZACLI.process_input()
...
- **`prompt_builder.py`** — Assembles the system prompt from: personality (SOUL.md), memory (MEMORY.md, USER.md), skills, context files (AGENTS.md, .zaza.md), tool-use guidance, and model-specific instructions
...
Three discovery sources: `~/.agent-zaza/data/plugins/` (user), `.zaza/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `zaza plugins` or `config.yaml`.

→ [Plugin Guide](/docs/guides/build-a-zaza-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes ZAZA as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`zaza -p <name>`) gets its own ZAZA_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
