...
description: "How ZAZA stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `zaza_cli/cron.py` | CLI `zaza cron` subcommands |
...
Jobs are stored in `~/.agent-zaza/data/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `zaza cron` commands are run or during active CLI sessions.
...
```python
# ~/.agent-zaza/data/scripts/check_competitors.py
...
2. **Environment variable** — `ZAZA_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.agent-zaza/data/cron/output/` |
...
The `zaza cron` CLI provides direct job management:

```bash
zaza cron list                    # Show all jobs
zaza cron create                  # Interactive job creation (alias: add)
zaza cron edit <job_id>           # Edit job configuration
zaza cron pause <job_id>          # Pause a running job
zaza cron resume <job_id>         # Resume a paused job
zaza cron run <job_id>            # Trigger immediate execution
zaza cron remove <job_id>         # Delete a job
```
