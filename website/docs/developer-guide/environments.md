...
description: "Building RL training environments, running evaluation benchmarks, and generating SFT data with the ZAZA-Agent Atropos integration"
---

# Environments, Benchmarks & Data Generation

Agent ZAZA includes a full environment framework that connects its tool-calling capabilities to the [Atropos](https://github.com/ZAZA/atropos) RL training framework. This enables three workflows:
...
The Python environment framework documented here lives under the repo's `environments/` directory and is the implementation-level API for ZAZA/Atropos integration. This is separate from the user-facing `rl_*` tools, which operate as an orchestration surface for remote RL training workflows.
:::
...
classDiagram
    ...
    BaseEnv <|-- ZAZAAgentBaseEnv
    ZAZAAgentBaseEnv <|-- TerminalTestEnv
    ZAZAAgentBaseEnv <|-- ZAZASweEnv
    ZAZAAgentBaseEnv <|-- TerminalBench2EvalEnv
...
The agent-zaza layer (`environments/zaza_base_env.py`). Adds:
- **Terminal backend configuration** — sets `TERMINAL_ENV` for sandboxed execution (local, Docker, Modal, Daytona, SSH, Singularity)
- **Tool resolution** — `_resolve_tools_for_group()` calls agent-zaza's `get_tool_definitions()` to get the right tool schemas based on enabled/disabled toolsets
- **Agent loop integration** — `collect_trajectory()` runs `ZAZAAgentLoop` and scores the result
...
Your environment inherits from `ZAZAAgentBaseEnv` and implements five methods:
...
`ZAZAAgentLoop` (`environments/agent_loop.py`) is the reusable multi-turn agent engine. It runs the same tool-calling pattern as agent-zaza's main loop:
...
| **Generic** | `call_tool(name, args)` — escape hatch for any agent-zaza tool |
...
parser = get_parser("zaza")  # or "mistral", "llama3_json", "qwen", "deepseek_v3", etc.
content, tool_calls = parser.parse(raw_model_output)
```

Available parsers: `zaza`, `mistral`, `llama3_json`, `qwen`, `qwen3_coder`, `deepseek_v3`, `deepseek_v3_1`, `kimi_k2`, `longcat`, `glm45`, `glm47`.
...
Dataset: [ZAZA/terminal-bench-2](https://huggingface.co/datasets/ZAZA/terminal-bench-2) on HuggingFace.
...
TBLite is a thin subclass of TerminalBench2 — only the dataset and timeouts differ. Created by the OpenThoughts Agent team (Snorkel AI + Bespoke Labs). Dataset: [ZAZA/openthoughts-tblite](https://huggingface.co/datasets/ZAZA/openthoughts-tblite).
...
pip install "agent-zaza[yc-bench]"
...
```bash
python environments/zaza_swe_env/zaza_swe_env.py serve \
...
- Set `tool_call_parser` in config to match your model's format (e.g., `"zaza"`, `"qwen"`, `"mistral"`)
...
class MyEnvConfig(ZAZAAgentEnvConfig):
    my_custom_field: str = "default_value"

class MyEnv(ZAZAAgentBaseEnv):
    name = "my-env"
...
### ZAZAAgentEnvConfig Fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled_toolsets` | `List[str]` | `None` (all) | Which zaza toolsets to enable |
...
| `tool_call_parser` | `str` | `"zaza"` | Parser for Phase 2 raw output |
...
env:
  ...
  dataset_name: "ZAZA/terminal-bench-2"
  tokenizer_name: "ZAZA/ZAZA-3-Llama-3.1-8B"
  use_wandb: true
...
- Python >= 3.11
- `atroposlib`: `pip install git+https://github.com/ZAZA/atropos.git`
- An LLM API key (OpenRouter, OpenAI, or self-hosted VLLM/SGLang)

### For Modal-sandboxed benchmarks (TB2, TBLite)

- [Modal](https://modal.com) account and CLI: `pip install "agent-zaza[modal]"`
- `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` environment variables

### For YC-Bench

- `pip install "agent-zaza[yc-bench]"` (installs the yc-bench CLI + SQLAlchemy)
...
```
environments/
├── zaza_base_env.py          # Abstract base class (ZAZAAgentBaseEnv)
├── agent_loop.py               # Multi-turn agent engine (ZAZAAgentLoop)
├── tool_context.py             # Per-rollout tool access for reward functions
├── patches.py                  # Async-safety patches for Modal backend
│
├── tool_call_parsers/          # Phase 2 client-side parsers
│   ├── zaza_parser.py        # ZAZA/ChatML <tool_call> format
...
├── zaza_swe_env/             # SWE-bench training environment
│
