...
The ACP adapter wraps ZAZA' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
zaza acp / zaza-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> load ~/.agent-zaza/data/.env
  -> configure stderr logging
  -> construct ZAZAACPAgent
  -> acp.run_agent(agent)
```
...
- `allow_once` -> ZAZA `once`
- `allow_always` -> ZAZA `always`
- reject options -> ZAZA `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps ZAZA tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["zaza-acp"])
...
Instead it reuses ZAZA' runtime resolver:

- `acp_adapter/auth.py`
- `zaza_cli/runtime_provider.py`

So ACP advertises and uses the currently configured ZAZA provider/credentials.
...
- `toolsets.py` — `zaza-acp` toolset definition
- `zaza_cli/main.py` — `zaza acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `zaza-acp` script
