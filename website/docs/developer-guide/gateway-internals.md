...
The messaging gateway is the long-running process that connects ZAZA to 14+ external messaging platforms through a unified architecture.
...
1. `resolve_command()` from `zaza_cli/commands.py` maps input to canonical name (handles aliases, prefix matching)
...
|--------|-----------------|
| `~/.agent-zaza/data/.env` | API keys, bot tokens, platform credentials |
| `~/.agent-zaza/data/config.yaml` | Model settings, tool configuration, display options |
...
Hooks are discovered from `gateway/builtin_hooks/` (always active) and `~/.agent-zaza/data/hooks/` (user-installed). Each hook is a directory with a `HOOK.yaml` manifest and `handler.py`.
...
- `zaza gateway start` / `zaza gateway stop` — manual control
- `systemctl` (Linux) or `launchctl` (macOS) — service management
- PID file at `~/.agent-zaza/data/gateway.pid` — profile-scoped process tracking

**Profile-scoped vs global**: `start_gateway()` uses profile-scoped PID files. `zaza gateway stop` stops only the current profile's gateway. `zaza gateway stop --all` uses global `ps aux` scanning to kill all gateway processes (used during updates).
