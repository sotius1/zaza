...
[PR #12550](https://github.com/ZAZA/agent-zaza/pull/12550) proposed a
...
(`zaza-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per ZAZA `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `zaza-acp`, `zaza-api-server`, core toolsets (gated on CDP reachability)
...
- `zaza_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`
