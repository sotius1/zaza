...
Agent ZAZA uses a SQLite database (`~/.agent-zaza/data/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `zaza_state.py`


## Architecture Overview

```
~/.agent-zaza/data/state.db (SQLite, WAL mode)
...
Multiple zaza processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.agent-zaza/data/state.db
...
Default path: `~/.agent-zaza/data/state.db`

This is derived from `zaza_constants.get_zaza_home()` which resolves to
`~/.agent-zaza/data/` by default, or the value of `ZAZA_HOME` environment variable.
