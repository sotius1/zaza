...
description: "Run Agent ZAZA directly on an Android phone with Termux"
---

# ZAZA on Android with Termux

This is the tested path for running Agent ZAZA directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop ZAZA from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

ZAZA now ships a Termux-aware installer path:
...
- links `zaza` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `zaza` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/zaza" "$PREFIX/bin/zaza"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `zaza` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.agent-zaza/data/.env`.
...
### `zaza doctor` says ripgrep or Node is missing
