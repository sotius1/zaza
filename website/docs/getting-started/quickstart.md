...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Agent ZAZA — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working ZAZA setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Who this is for

- Brand new and want the shortest path to a working setup
- Switching providers and don't want to lose time to config mistakes
- Setting up ZAZA for a team, bot, or always-on workflow
...
|---|---|---|
| I just want ZAZA working on my machine | `zaza setup` | Run a real chat and verify it responds |
| I already know my provider | `zaza model` | Save the config, then start chatting |
| I want a bot or always-on setup | `zaza gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `zaza model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `zaza model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if ZAZA cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Agent ZAZA
...
The single most important setup step. Use `zaza model` to walk through the choice interactively:
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `zaza model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `zaza model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `zaza model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax-M2.7 via browser OAuth — no API key needed | `zaza model` → MiniMax (OAuth) |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `zaza model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `zaza model` (requires `copilot` CLI + `copilot login`) |
...
Agent ZAZA requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `zaza model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

ZAZA separates secrets from normal config:

- **Secrets and tokens** → `~/.agent-zaza/data/.env`
- **Non-secret settings** → `~/.agent-zaza/data/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
zaza config set model anthropic/claude-opus-4.6
zaza config set terminal.backend docker
zaza config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
zaza            # classic CLI
zaza --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
ZAZA ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `zaza` vs `zaza --tui`.
:::
...
```bash
zaza --continue    # Resume the most recent session
zaza -c            # Short form
```
...
```bash
zaza gateway setup    # Interactive platform configuration
```
...
- `zaza tools` — tune tool access per platform
- `zaza skills` — browse and install reusable workflows
...
```bash
zaza config set terminal.backend docker    # Docker isolation
zaza config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
pip install "agent-zaza[voice]"
...
```yaml
# Add to ~/.agent-zaza/data/config.yaml
mcp_servers:
  github:
    command: npx
...
|---|---|---|
| ZAZA opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `zaza model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `zaza gateway setup` and check `zaza gateway status` |
| `zaza --continue` can't find old session | Switched profiles or session never saved | Check `zaza sessions list` and confirm you're in the right profile |
...
| `zaza doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `zaza model`
3. `zaza setup`
4. `zaza sessions list`
5. `zaza --continue`
6. `zaza gateway status`
...
| `zaza tools` | Configure which tools are enabled per platform |
| `zaza setup` | Full setup wizard (configures everything at once) |
| `zaza doctor` | Diagnose issues |
| `zaza update` | Update to latest version |
| `zaza gateway` | Start the messaging gateway |
| `zaza --continue` | Resume last session |
