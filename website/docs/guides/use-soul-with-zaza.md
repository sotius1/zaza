---
sidebar_position: 7
title: "Use SOUL.md with ZAZA"
description: "How to use SOUL.md to shape Agent ZAZA's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with ZAZA

`SOUL.md` is the **primary identity** for your ZAZA instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want ZAZA to feel like the same assistant every time you talk to it — or if you want to replace the ZAZA persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm ZAZA should be
- what ZAZA should avoid stylistically
- how ZAZA should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who ZAZA is and how ZAZA speaks
...
ZAZA now uses only the global SOUL file for the current instance:

```text
~/.agent-zaza/data/SOUL.md
```

If you run ZAZA with a custom home directory, it becomes:
...
ZAZA automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, ZAZA does not overwrite it
- if the file exists but is empty, ZAZA adds nothing from it to the prompt

## How ZAZA uses it

When ZAZA starts a session, it reads `SOUL.md` from `ZAZA_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how ZAZA feels.
...
ZAZA already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How ZAZA should sound.

# Avoid
What ZAZA should not do.

# Defaults
How ZAZA should behave when ambiguity appears.
```
...
```bash
nano ~/.agent-zaza/data/SOUL.md
```

or

```bash
vim ~/.agent-zaza/data/SOUL.md
```

Then restart ZAZA or start a new session.
...
### I edited SOUL.md but ZAZA still sounds the same

Check:
- you edited `~/.agent-zaza/data/SOUL.md` or `$ZAZA_HOME/SOUL.md`
...
### ZAZA is ignoring parts of my SOUL.md
