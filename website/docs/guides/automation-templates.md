...
Copy-paste recipes for common automation patterns. Each template uses ZAZA's built-in [cron scheduler](/docs/user-guide/features/cron) for time-based triggers and [webhook platform](/docs/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`zaza webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `zaza webhook subscribe`) |
...
```bash
zaza cron create "0 2 * * *" \
  "You are a project manager triaging the ZAZA/agent-zaza GitHub repo.

1. Run: gh issue list --repo ZAZA/agent-zaza --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
zaza webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
zaza cron create "0 9 * * 1" \
  "Scan the ZAZA/agent-zaza repo for documentation drift.

1. Run: gh pr list --repo ZAZA/agent-zaza --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (zaza_cli/commands.py, zaza_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (zaza_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
zaza cron create "0 6 * * *" \
  "Run a dependency security audit on the agent-zaza project.

1. cd ~/.agent-zaza/data/agent-zaza && source .venv/bin/activate
...
```bash
zaza webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
zaza webhook subscribe alert-triage \
...
```python title="~/.agent-zaza/data/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "ZAZA-Monitor/1.0"})
...
zaza cron create "every 30m" \
  ...
  --script ~/.agent-zaza/data/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
zaza cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Agent ZAZA development." \
...
```bash
zaza webhook subscribe github-issues \
  --events "issues" \
...
```bash
zaza webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
zaza webhook subscribe stripe-payments \
...
```bash
zaza cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the agent-zaza codebase.
