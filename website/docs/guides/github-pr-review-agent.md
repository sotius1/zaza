...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Agent ZAZA  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to ZAZA instantly when PRs are opened or updated.
:::
...
- **Agent ZAZA installed** — see the [Installation guide](/docs/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  zaza gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.agent-zaza/data/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure ZAZA can access GitHub. Start a chat:
...
```
Run: gh pr list --repo ZAZA/agent-zaza --state open --limit 3
```
...
Still in the chat, ask ZAZA to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo ZAZA/agent-zaza
```
...
A skill gives ZAZA consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.agent-zaza/data/skills/code-review
```

Create `~/.agent-zaza/data/skills/code-review/SKILL.md`:
...
Verify it loaded — start `zaza` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach ZAZA your team's standards:
...
2. Teach ZAZA your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
zaza gateway status    # Is the gateway running?
zaza cron list         # Is the job enabled?
```
...
- **[Build a Plugin](/docs/guides/build-a-zaza-plugin)** — wrap the review logic into a shareable plugin
