...
This tutorial walks you through setting up a Telegram bot powered by Agent ZAZA that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Agent ZAZA installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.agent-zaza/data/.env`

:::tip
A $5/month VPS is plenty for running the gateway. ZAZA itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team ZAZA Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_zaza_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Agent ZAZA. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.agent-zaza/data/.env`:
...
# View live logs
journalctl --user -u zaza-gateway -f
...
zaza gateway start
zaza gateway stop
tail -f ~/.agent-zaza/data/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `zaza gateway install` to update the plist.
:::
...
```bash
zaza gateway stop && zaza gateway start
```
...
zaza pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.agent-zaza/data/.env`:
...
Control how much detail the bot shows when using tools. In `~/.agent-zaza/data/config.yaml`:
...
Customize how the bot communicates by editing `~/.agent-zaza/data/SOUL.md`:

For a full guide, see [Use SOUL.md with ZAZA](/docs/guides/use-soul-with-zaza).
...
```bash
# From the CLI
zaza cron list          # View all scheduled jobs
zaza cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u zaza-gateway` (Linux) or `~/.agent-zaza/data/logs/gateway.log` (macOS) |
| Cron job output | `~/.agent-zaza/data/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.agent-zaza/data/cron/jobs.json` |
| Pairing data | `~/.agent-zaza/data/pairing/` |
| Session history | `~/.agent-zaza/data/sessions/` |
