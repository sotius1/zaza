...
title: "Build a ZAZA Plugin"
description: "Step-by-step guide to building a complete ZAZA plugin with tools, hooks, data files, and skills"
---

# Build a ZAZA Plugin

This guide walks through building a complete ZAZA plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.
...
```bash
mkdir -p ~/.agent-zaza/data/plugins/calculator
cd ~/.agent-zaza/data/plugins/calculator
```
...
This tells ZAZA: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
4. **Accept `**kwargs`:** ZAZA may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `zaza my-plugin <subcommand>`)
- If this function crashes, the plugin is disabled but ZAZA continues fine
...
```
~/.agent-zaza/data/plugins/calculator/
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.agent-zaza/data/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.agent-zaza/data/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `zaza plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), ZAZA injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.

#### Return format

```python
# Dict with context key
return {"context": "Recalled memories:\n- User prefers dark mode\n- Last project: agent-zaza"}
...
- **The system prompt is ZAZA's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
Plugins can add their own `zaza <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for zaza my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: zaza my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for zaza my-plugin."""
...
After registration, users can run `zaza my-plugin status`, `zaza my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `zaza name` in a terminal |
...
```toml
# pyproject.toml
[project.entry-points."zaza_agent.plugins"]
my-plugin = "my_plugin_package"
```

```bash
pip install zaza-plugin-calculator
# Plugin auto-discovered on next zaza startup
```
...
```python
# Wrong — will break if ZAZA passes extra context
def handler(args):
    ...
