...
description: "Diagnose and fix common ZAZA cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`zaza gateway` or `zaza serve`). For one-off debugging, you can manually trigger a tick with `zaza cron tick`.
...
```bash
date
zaza cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.agent-zaza/data/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.agent-zaza/data/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.agent-zaza/data/.env` |
...
| `local` | Write access to `~/.agent-zaza/data/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `zaza cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `zaza skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `zaza skills list`.
...
2. `~/.agent-zaza/data/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `zaza cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the ZAZA config directory). Verify:
```bash
ls ~/.agent-zaza/data/scripts/your-script.py   # Must exist
zaza cron edit <job_id> --script ~/.agent-zaza/data/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `zaza skills install <skill-name>`.
...
Jobs are stored in `~/.agent-zaza/data/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.agent-zaza/data/cron/jobs.json
chmod 600 ~/.agent-zaza/data/cron/jobs.json   # Your user should own it
```
...
```bash
zaza cron list                    # Show all jobs, states, next_run times
zaza cron run <job_id>            # Schedule for next tick (for testing)
zaza cron edit <job_id>           # Fix configuration issues
zaza logs                         # View recent ZAZA logs
zaza skills list                  # Verify installed skills
```
...
1. Run the job with `zaza cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.agent-zaza/data/logs/agent.log` for scheduler messages and `~/.agent-zaza/data/logs/errors.log` for warnings
3. Open an issue at [github.com/ZAZA/agent-zaza](https://github.com/ZAZA/agent-zaza) with:
