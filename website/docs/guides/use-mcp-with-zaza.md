---
sidebar_position: 6
title: "Use MCP with ZAZA"
description: "A practical guide to connecting MCP servers to Agent ZAZA, filtering their tools, and using them safely in real workflows"
---

# Use MCP with ZAZA

This guide shows how to actually use MCP with Agent ZAZA in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native ZAZA tool
- you want ZAZA to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect ZAZA to internal APIs, databases, or company systems without modifying ZAZA core

Do not use MCP when:
- a built-in ZAZA tool already solves the job well
...
- ZAZA remains the agent
- MCP servers contribute tools
- ZAZA discovers those tools at startup or reload time
...
If you installed ZAZA with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- ZAZA banner/status should show MCP integration when configured
- ask ZAZA what tools it has available
...
There are two categories of MCP-exposed functionality in ZAZA:
...
So ZAZA will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want ZAZA to reason over a bounded workspace.
...
Now ZAZA can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing ZAZA core.
...
Because ZAZA now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
