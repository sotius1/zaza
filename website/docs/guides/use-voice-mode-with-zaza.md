---
sidebar_position: 8
title: "Use Voice Mode with ZAZA"
description: "A practical guide to setting up and using ZAZA voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with ZAZA
...
- you want ZAZA sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in ZAZA.
...
## Step 1: make sure normal ZAZA works first
...
ZAZA supports both local and cloud speech stacks.
...
### If you use `zaza setup`

If you choose NeuTTS in the setup wizard, ZAZA checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. ZAZA transcribes and responds
...
- asking ZAZA to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full ZAZA loop.

## Tuning CLI behavior

### Silence threshold

If ZAZA starts/stops too aggressively, tune:
...
ZAZA stays a normal chat bot, but can speak replies.
...
- you want ZAZA to function like a portable research or ops assistant
...
ZAZA joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- ZAZA detects speech boundaries
- transcripts are posted in the associated text channel
- ZAZA responds in text and audio
...
1. get text ZAZA working
2. install `agent-zaza[voice]`
