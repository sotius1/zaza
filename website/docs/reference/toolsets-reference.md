...
description: "Reference for ZAZA core, composite, platform, and dynamic toolsets"
---
...
- **Platform** — A complete tool configuration for a specific deployment context (e.g., `zaza-cli` is the default for interactive CLI sessions)
...
```bash
zaza chat --toolsets web,file,terminal
zaza chat --toolsets debugging        # composite — expands to file + terminal + web
zaza chat --toolsets all              # everything
```
...
```yaml
toolsets:
  - zaza-cli          # default for CLI
  # - zaza-telegram   # override for Telegram gateway
```

### Interactive management

```bash
zaza tools                            # curses UI to enable/disable per platform
```
...
| `code_execution` | `execute_code` | Run Python scripts that call ZAZA tools programmatically. |
...
| `feishu_drive` | `feishu_drive_add_comment`, `feishu_drive_list_comments`, `feishu_drive_list_comment_replies`, `feishu_drive_reply_comment` | Feishu/Lark drive comment operations. Scoped to the comment agent; not exposed on `zaza-cli` or other messaging toolsets. |
...
Platform toolsets define the complete tool configuration for a deployment target. Most messaging platforms use the same set as `zaza-cli`:

| Toolset | Differences from `zaza-cli` |
|---------|-------------------------------|
| `zaza-cli` | Full toolset — all 36 core tools including `clarify`. The default for interactive CLI sessions. |
| `zaza-acp` | Drops `clarify`, `cronjob`, `image_generate`, `send_message`, `text_to_speech`, homeassistant tools. Focused on coding tasks in IDE context. |
| `zaza-api-server` | Drops `clarify`, `send_message`, and `text_to_speech`. Adds everything else — suitable for programmatic access where user interaction isn't possible. |
| `zaza-telegram` | Same as `zaza-cli`. |
| `zaza-discord` | Same as `zaza-cli`. |
| `zaza-slack` | Same as `zaza-cli`. |
| `zaza-whatsapp` | Same as `zaza-cli`. |
| `zaza-signal` | Same as `zaza-cli`. |
| `zaza-matrix` | Same as `zaza-cli`. |
| `zaza-mattermost` | Same as `zaza-cli`. |
| `zaza-email` | Same as `zaza-cli`. |
| `zaza-sms` | Same as `zaza-cli`. |
| `zaza-bluebubbles` | Same as `zaza-cli`. |
| `zaza-dingtalk` | Same as `zaza-cli`. |
| `zaza-feishu` | Same as `zaza-cli`. Note: the `feishu_doc` / `feishu_drive` toolsets are used only by the document-comment handler, not by the regular Feishu chat adapter. |
| `zaza-qqbot` | Same as `zaza-cli`. |
| `zaza-wecom` | Same as `zaza-cli`. |
| `zaza-wecom-callback` | Same as `zaza-cli`. |
| `zaza-weixin` | Same as `zaza-cli`. |
| `zaza-homeassistant` | Same as `zaza-cli` plus the `homeassistant` toolset always on. |
| `zaza-webhook` | Same as `zaza-cli`. |
| `zaza-gateway` | Internal gateway orchestrator toolset — union of the broadest possible tool set when the gateway needs to accept any message source. |
...
## Relationship to `zaza tools`

The `zaza tools` command provides a curses-based UI for toggling individual tools on or off per platform. This operates at the tool level (finer than toolsets) and persists to `config.yaml`. Disabled tools are filtered out even if their toolset is enabled.
