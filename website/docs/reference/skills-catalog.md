...
| [`agent-zaza`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-agent-zaza) | Complete guide to using and extending Agent ZAZA — CLI usage, setup, configuration, spawning additional agents, gateway platforms, skills, voice, tools, profiles, and a concise contributor reference. Load this skill when helping users... | `autonomous-ai-agents/agent-zaza` |
...
| Skill | Description | Path |
|-------|-------------|------|
| [`native-mcp`](/docs/user-guide/skills/bundled/mcp/mcp-native-mcp) | Built-in MCP (Model Context Protocol) client that connects to external MCP servers, discovers their tools, and registers them as native Agent ZAZA tools. Supports stdio and HTTP transports with automatic reconnection, security filterin... | `mcp/native-mcp` |
...
| [`google-workspace`](/docs/user-guide/skills/bundled/productivity/productivity-google-workspace) | Gmail, Calendar, Drive, Contacts, Sheets, and Docs integration for ZAZA. Uses ZAZA-managed OAuth2 setup, prefers the Google Workspace CLI (`gws`) when available for broader API coverage, and falls back to the Python client libraries... | `productivity/google-workspace` |
...
| Skill | Description | Path |
|-------|-------------|------|
| [`plan`](/docs/user-guide/skills/bundled/software-development/software-development-plan) | Plan mode for ZAZA — inspect context, write a markdown plan into the active workspace's `.zaza/plans/` directory, and do not execute the work. | `software-development/plan` |
