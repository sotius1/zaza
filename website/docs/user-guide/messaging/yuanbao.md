...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Agent ZAZA to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect ZAZA to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.agent-zaza/data/.env`:
...
All standard ZAZA commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.agent-zaza/data/logs/gateway.log`
...
4. Consider enabling verbose logging: `ZAZA_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. ZAZA automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
ZAZA_LOG_LEVEL=debug zaza gateway
```
...
```bash
zaza chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
