...
description: Control your smart home with Agent ZAZA via Home Assistant integration.
...
Agent ZAZA integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Agent ZAZA"
5. Copy the token
...
Agent ZAZA registers four tools for smart home control:
...
Configure which events the agent sees in `~/.agent-zaza/data/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Agent ZAZA".
