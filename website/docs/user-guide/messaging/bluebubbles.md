...
Connect ZAZA to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between ZAZA and the BlueBubbles server
...
Or set environment variables directly in `~/.agent-zaza/data/.env`:
...
When someone messages your iMessage, ZAZA automatically sends them a pairing code. Approve it with:
```bash
zaza pairing approve bluebubbles <CODE>
```
Use `zaza pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.agent-zaza/data/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.agent-zaza/data/.env`):
```bash
...
ZAZA will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → ZAZA
ZAZA → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** ZAZA sends messages via the BlueBubbles REST API.
...
### Chat Addressing
You can address chats by email or phone number — ZAZA resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `zaza logs gateway` for webhook errors (or `zaza logs -f` to follow in real-time)
