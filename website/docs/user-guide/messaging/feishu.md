...
sidebar_position: 11
title: "Feishu / Lark"
description: "Set up Agent ZAZA as a Feishu or Lark bot"
---

# Feishu / Lark Setup

Agent ZAZA integrates with Feishu and Lark as a full-featured bot. Once connected, you can chat with the agent in direct messages or group chats, receive cron job results in a home chat, and send text, images, audio, and file attachments through the normal gateway flow.

The integration supports both connection modes:

- `websocket` — recommended; ZAZA opens the outbound connection and you do not need a public webhook endpoint
...
| Context | Behavior |
|---------|----------|
| Direct messages | ZAZA responds to every message. |
| Group chats | ZAZA responds only when the bot is @mentioned in the chat. |
...
Select **Feishu / Lark** and scan the QR code with your Feishu or Lark mobile app. ZAZA will automatically create a bot application with the correct permissions and save the credentials.
...
5. Run `zaza gateway setup`, select **Feishu / Lark**, and enter the credentials when prompted.
...
Use WebSocket mode when ZAZA runs on your laptop, workstation, or a private server. No public URL is required. The official Lark SDK opens and maintains a persistent outbound WebSocket connection with automatic reconnection.
...
Use webhook mode only when you already run ZAZA behind a reachable HTTP endpoint.

```bash
FEISHU_CONNECTION_MODE=webhook
```

In webhook mode, ZAZA starts an HTTP server (via `aiohttp`) and serves a Feishu endpoint at:
...
The `FEISHU_GROUP_POLICY` environment variable controls whether and how ZAZA responds in group chats:
...
| Value | Behavior |
|-------|----------|
| `open` | ZAZA responds to @mentions from any user in any group. |
| `allowlist` | ZAZA only responds to @mentions from users listed in `FEISHU_ALLOWED_USERS`. |
| `disabled` | ZAZA ignores all group messages entirely. |
...
Beyond chat, the adapter can also answer `@`-mentions left on **Feishu/Lark documents**. When a user comments on a document (local text selection or whole-doc comment) and @-mentions the bot, ZAZA reads the document plus the surrounding comment thread and posts an LLM reply inline on the thread.
...
Rules live in `~/.agent-zaza/data/feishu_comment_rules.json` (pairing grants in `~/.agent-zaza/data/feishu_comment_pairing.json`) with mtime-cached hot-reload — edits take effect on the next comment event without restarting the gateway.
...
|---------|---------|---------|
| Quiet period | `ZAZA_FEISHU_TEXT_BATCH_DELAY_SECONDS` | 0.6s |
| Max messages per batch | `ZAZA_FEISHU_TEXT_BATCH_MAX_MESSAGES` | 8 |
| Max characters per batch | `ZAZA_FEISHU_TEXT_BATCH_MAX_CHARS` | 4000 |
...
| Setting | Env Var | Default |
|---------|---------|---------|
| Quiet period | `ZAZA_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | 0.8s |
...
Inbound messages are deduplicated using message IDs with a 24-hour TTL. The dedup state is persisted across restarts to `~/.agent-zaza/data/feishu_seen_message_ids.json`.

| Setting | Env Var | Default |
|---------|---------|---------|
| Cache size | `ZAZA_FEISHU_DEDUP_CACHE_SIZE` | 2048 entries |
...
| `ZAZA_FEISHU_DEDUP_CACHE_SIZE` | — | `2048` | Max deduplicated message IDs to track |
| `ZAZA_FEISHU_TEXT_BATCH_DELAY_SECONDS` | — | `0.6` | Text burst debounce quiet period |
| `ZAZA_FEISHU_TEXT_BATCH_MAX_MESSAGES` | — | `8` | Max messages merged per text batch |
| `ZAZA_FEISHU_TEXT_BATCH_MAX_CHARS` | — | `4000` | Max characters merged per text batch |
| `ZAZA_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | — | `0.8` | Media burst debounce quiet period |
...
| `FEISHU_APP_ID or FEISHU_APP_SECRET not set` | Set both env vars or configure via `zaza gateway setup` |
| `Another local ZAZA gateway is already using this Feishu app_id` | Only one ZAZA instance can use the same app_id at a time. Stop the other gateway first. |
...
Feishu / Lark uses the `zaza-feishu` platform preset, which includes the same core tools as Telegram and other gateway-based messaging platforms.
