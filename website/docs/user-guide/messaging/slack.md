...
sidebar_position: 4
title: "Slack"
description: "Set up Agent ZAZA as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Agent ZAZA to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your ZAZA instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. ZAZA uses the modern
...
The fastest path is to paste a manifest ZAZA generates for you. It
...
### Option A: From a ZAZA-generated manifest (recommended)

1. Generate the manifest:
   ```bash
   zaza slack manifest --write
   ```
   This writes `~/.agent-zaza/data/slack-manifest.json` and prints paste-in
   instructions.
...
4. Enter an app name (e.g., "Agent ZAZA") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, ZAZA can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `zaza-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a ZAZA configuration issue.
:::
...
ZAZA uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
zaza gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
zaza gateway              # Foreground
zaza gateway install      # Install as a user service
sudo zaza gateway install --system   # Linux only: boot-time system service
```
...
Every ZAZA command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
ZAZA command with its description.

Under the hood: ZAZA ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/ZAZA/agent-zaza/blob/main/zaza_cli/commands.py)
...
When ZAZA adds new commands (e.g. after `zaza update`), regenerate
...
3. Paste the new contents of `~/.agent-zaza/data/slack-manifest.json`
4. **Save**. Slack will prompt to reinstall the app if scopes or slash
   commands changed.

### Legacy `/zaza <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/zaza btw run the tests` — ZAZA routes it the same way as `/btw
run the tests`. Free-form questions also work: `/zaza what's the
...
```bash
zaza slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how ZAZA behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Agent ZAZA what time is it?`). In channels, ZAZA replies in a thread attached to that message. |
| **Threads** | If you @mention ZAZA inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.agent-zaza/data/config.yaml`.
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to ZAZA in `#general` will have separate histories and contexts.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where ZAZA will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Agent ZAZA`).

---

## Multi-Workspace Support

ZAZA can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
Or in `~/.agent-zaza/data/config.yaml`:
...
In addition to tokens in the environment or config, ZAZA also loads tokens from an **OAuth token file** at:

```
~/.agent-zaza/data/slack_tokens.json
```
...
- When a message arrives, ZAZA uses the correct workspace-specific client to respond.
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Agent ZAZA` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Agent ZAZA` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. ZAZA now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @Agent ZAZA`)
...
- Tokens should be stored in `~/.agent-zaza/data/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your ZAZA config directory
