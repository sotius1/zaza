...
Agent ZAZA automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.agent-zaza/data/state.db`) — structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.agent-zaza/data/sessions/`) — raw conversation transcripts including tool calls (gateway)
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`zaza` or `zaza chat`) |
...
zaza -c "my project"   # → resumes "my project #3"
```
...
zaza --resume 20250305_091523_a1b2c3d4
zaza -r 20250305_091523_a1b2c3d4

# Resume by title
zaza --resume "refactoring auth"

# Or with the chat subcommand
zaza chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `zaza sessions list`.

### Conversation Recap on Resume

When you resume a session, ZAZA displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a ZAZA session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.agent-zaza/data/config.yaml`:
...
ZAZA automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `zaza sessions list` or `zaza sessions browse`.
...
```bash
zaza sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), ZAZA creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`zaza -c "my project"`), it automatically picks the most recent session in the lineage.
...
ZAZA provides a full set of session management commands via `zaza sessions`:
...
# Filter by platform
zaza sessions list --source telegram

# Show more sessions
zaza sessions list --limit 50
```
...
zaza sessions export backup.jsonl

# Export sessions from a specific platform
zaza sessions export telegram-history.jsonl --source telegram

# Export a single session
zaza sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
zaza sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
zaza sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
zaza sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
zaza sessions prune --older-than 30

# Only prune sessions from a specific platform
zaza sessions prune --source telegram --older-than 60

# Skip confirmation
zaza sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`zaza insights`](/docs/reference/cli-commands#zaza-insights).
...
When ZAZA cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, ZAZA uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to ZAZA in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.agent-zaza/data/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.agent-zaza/data/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.agent-zaza/data/sessions/sessions.json` | Maps session keys to active session IDs |
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every ZAZA process in the same `ZAZA_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.agent-zaza/data/config.yaml`:
...
zaza sessions delete <session_id>
