...
All skills live in **`~/.agent-zaza/data/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point ZAZA at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling ZAZA to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.zaza/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
zaza chat --toolsets skills -q "What skills do you have?"
zaza chat --toolsets skills -q "Show me the axolotl skill"
```
...
When a missing value is encountered, ZAZA asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `zaza setup` or `~/.agent-zaza/data/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `zaza config migrate` prompts for unconfigured settings, and `zaza config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.agent-zaza/data/skills/                  # Single source of truth
...
If you maintain skills outside of ZAZA — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell ZAZA to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.agent-zaza/data/config.yaml`:
...
- **Read-only**: External dirs are only scanned for skill discovery. When the agent creates or edits a skill, it always writes to `~/.agent-zaza/data/skills/`.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, ZAZA ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.agent-zaza/data/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
```bash
zaza skills browse                              # Browse all hub skills (official first)
zaza skills browse --source official            # Browse only official optional skills
zaza skills search kubernetes                   # Search all sources
zaza skills search react --source skills-sh     # Search the skills.sh directory
zaza skills search https://mintlify.com/docs --source well-known
zaza skills inspect openai/skills/k8s           # Preview before installing
zaza skills install openai/skills/k8s           # Install with security scan
zaza skills install official/security/1password
zaza skills install skills-sh/vercel-labs/json-render/json-render-react --force
zaza skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
zaza skills list --source hub                   # List hub-installed skills
zaza skills check                               # Check installed hub skills for upstream updates
zaza skills update                              # Reinstall hub skills with upstream changes when needed
zaza skills audit                               # Re-scan all hub skills for security
zaza skills uninstall k8s                       # Remove a hub skill
zaza skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
zaza skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
zaza skills publish skills/my-skill --to github --repo owner/repo
zaza skills snapshot export setup.json          # Export skill config
zaza skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with ZAZA. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `zaza skills search <query> --source skills-sh`. ZAZA resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
ZAZA currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the ZAZA repository itself and install with builtin trust.
...
This is Vercel's public skills directory. ZAZA can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
zaza skills search https://mintlify.com/docs --source well-known
zaza skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
zaza skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

ZAZA can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
zaza skills install openai/skills/k8s
zaza skills tap add myorg/skills-repo
```
...
- ZAZA source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

ZAZA supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

ZAZA source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

ZAZA can search and convert agent entries from LobeHub's public catalog into installable ZAZA skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- ZAZA source id: `lobehub`

#### 8. Direct URL (`url`)

Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). ZAZA fetches the URL, parses the YAML frontmatter, security-scans it, and installs.
...
```bash
zaza skills install https://sharethis.chat/SKILL.md
zaza skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
zaza skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `zaza skills update` re-fetches from the same URL automatically when you want to refresh.
...
`zaza skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with ZAZA | Always trusted |
...
```bash
zaza skills check          # Report which installed hub skills changed upstream
zaza skills update         # Reinstall only the skills with updates available
zaza skills update react   # Update one specific installed hub skill
```
...
## Bundled skill updates (`zaza skills reset`)

ZAZA ships with a set of bundled skills in `skills/` inside the repo. On install and on every `zaza update`, a sync pass copies those into `~/.agent-zaza/data/skills/` and records a manifest at `~/.agent-zaza/data/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, ZAZA recomputes the hash of your local copy and compares it to the origin hash:
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `ZAZA_HOME`, so `zaza -p coder skills reset <name>` only affects that profile.
:::
