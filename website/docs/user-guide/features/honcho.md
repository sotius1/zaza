...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of ZAZA's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple ZAZA instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
zaza memory setup    # select "honcho" from the provider list
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$ZAZA_HOME/honcho.json` (profile-local). The setup wizard handles this for you.
...
- `per-session` — each `zaza` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — ZAZA syncs them back at session init.
...
```bash
zaza honcho status          # Connection status, config, and key settings
zaza honcho setup           # Interactive setup wizard
zaza honcho strategy        # Show or set session strategy
zaza honcho peer            # Update peer names for multi-agent setups
zaza honcho mode            # Show or set recall mode
zaza honcho tokens          # Show or set context token budget
zaza honcho identity        # Show Honcho peer identity
zaza honcho sync            # Sync host blocks for all profiles
zaza honcho enable          # Enable Honcho
zaza honcho disable         # Disable Honcho
```

## Migrating from `zaza honcho`

If you previously used the standalone `zaza honcho setup`:
...
No re-login or re-setup needed. Run `zaza memory setup` and select "honcho" — the wizard detects your existing config.
