...
Agent ZAZA has three layers of resilience that keep your sessions running when providers hit issues:
...
When your main LLM provider encounters errors — rate limits, server overload, auth failures, connection drops — ZAZA can automatically switch to a backup provider:model pair mid-session without losing your conversation.

### Configuration

Add a `fallback_model` section to `~/.agent-zaza/data/config.yaml`:
...
| Nous Portal | `nous` | `zaza auth` (OAuth) |
| OpenAI Codex | `openai-codex` | `zaza model` (ChatGPT OAuth) |
...
| Google Gemini (OAuth) | `google-gemini-cli` | `zaza model` (Google OAuth; optional: `ZAZA_GEMINI_PROJECT_ID`) |
...
| Qwen Portal (OAuth) | `qwen-oauth` | `zaza model` (Qwen Portal OAuth; optional: `ZAZA_QWEN_BASE_URL`) |
| MiniMax (OAuth) | `minimax-oauth` | `zaza model` (MiniMax portal OAuth) |
...
ZAZA uses separate lightweight models for side tasks. Each task has its own provider resolution chain that acts as a built-in fallback system.
...
When a task's provider is set to `"auto"` (the default), ZAZA tries providers in order until one works:
...
If the resolved provider fails at call time, ZAZA also has an internal retry: if the provider is not OpenRouter and no explicit `base_url` is set, it tries OpenRouter as a last-resort fallback.
...
For `auxiliary.session_search`, ZAZA also supports:
...
| `"codex"` | Force Codex OAuth | `zaza model` → Codex |
...
`base_url` takes precedence over `provider`. ZAZA uses the configured `api_key` for authentication, falling back to `OPENAI_API_KEY` if not set. It does **not** reuse `OPENROUTER_API_KEY` for custom endpoints.
...
If no provider is available for compression, ZAZA drops middle conversation turns without generating a summary rather than failing the session.
