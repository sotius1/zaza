...
Schedule tasks to run automatically with natural language or cron expressions. ZAZA exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
:::warning
Cron-run sessions cannot recursively create more cron jobs. ZAZA disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
zaza cron create "every 2h" "Check server status"
zaza cron create "every 1h" "Summarize new feed items" --skill blogwatcher
zaza cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
ZAZA will use the unified `cronjob` tool internally.
...
```bash
# Standalone CLI
zaza cron create --schedule "every 1d at 09:00" \
...
```bash
zaza cron edit <job_id> --schedule "every 4h"
zaza cron edit <job_id> --prompt "Use the revised task"
zaza cron edit <job_id> --skill blogwatcher --skill maps
zaza cron edit <job_id> --add-skill maps
zaza cron edit <job_id> --remove-skill blogwatcher
zaza cron edit <job_id> --clear-skills
```
...
```bash
zaza cron list
zaza cron pause <job_id>
zaza cron resume <job_id>
zaza cron run <job_id>
zaza cron remove <job_id>
zaza cron status
zaza cron tick
```
...
sudo zaza gateway install --system   # Linux: boot-time system service for servers
zaza gateway             # Or run in foreground
...
1. loads jobs from `~/.agent-zaza/data/cron/jobs.json`
...
A file lock at `~/.agent-zaza/data/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.agent-zaza/data/cron/output/`) | Default on CLI |
...
If the agent's final response starts with `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.agent-zaza/data/cron/output/`), but no message is sent to the delivery target.
...
Or set the `ZAZA_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 120s default.
...
The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, ZAZA skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
Jobs are stored in `~/.agent-zaza/data/cron/jobs.json`. Output from job runs is saved to `~/.agent-zaza/data/cron/output/{job_id}/{timestamp}.md`.

Jobs may store `model` and `provider` as `null`. When those fields are omitted, ZAZA resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
