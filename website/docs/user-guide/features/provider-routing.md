...
When using [OpenRouter](https://openrouter.ai) as your LLM provider, Agent ZAZA supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
Add a `provider_routing` section to your `~/.agent-zaza/data/config.yaml`:
...
- **CLI mode** — configured in `~/.agent-zaza/data/config.yaml`, loaded at startup
