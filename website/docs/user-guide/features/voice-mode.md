...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Agent ZAZA — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Agent ZAZA supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with ZAZA](/docs/guides/use-voice-mode-with-zaza).

## Prerequisites

Before using voice features, make sure you have:

1. **Agent ZAZA installed** — `pip install agent-zaza` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `zaza model` or set your preferred provider credentials in `~/.agent-zaza/data/.env`
3. **A working base setup** — run `zaza` to verify the agent responds to text before enabling voice

:::tip
The `~/.agent-zaza/data/` directory and default `config.yaml` are created automatically the first time you run `zaza`. You only need to create `~/.agent-zaza/data/.env` manually for API keys.
:::
...
pip install "agent-zaza[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "agent-zaza[all]"
```
...
```bash
zaza                # Start the interactive CLI
```
...
1. Start the CLI with `zaza` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.agent-zaza/data/config.yaml` (default: `ctrl+b`).
:::
...
```bash
zaza gateway        # Start the gateway (connects to configured platforms)
zaza gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@zazabyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.agent-zaza/data/.env`:
```bash
...
```bash
zaza gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.agent-zaza/data/.env`
...
- Review gateway logs: `tail -f ~/.agent-zaza/data/logs/gateway.log`
