...
:::tip Get Started
The Tool Gateway is included with paid Nous Portal subscriptions. **[Manage your subscription →](https://portal.zaza.com/manage-subscription)**
:::

The **Tool Gateway** lets paid [Nous Portal](https://portal.zaza.com) subscribers use web search, image generation, text-to-speech, and browser automation through their existing subscription — no need to sign up for separate API keys from Firecrawl, FAL, OpenAI, or Browser Use.
...
The Tool Gateway is available to **paid** [Nous Portal](https://portal.zaza.com/manage-subscription) subscribers. Free-tier accounts do not have access — [upgrade your subscription](https://portal.zaza.com/manage-subscription) to unlock it.
...
When you run `zaza model` and select Nous Portal as your provider, ZAZA automatically offers to enable the Tool Gateway:
...
Set the `use_gateway` flag directly in `~/.agent-zaza/data/config.yaml`:
...
The gateway authenticates using your Nous Portal credentials (stored in `~/.agent-zaza/data/auth.json` after `zaza model`).
...
```bash
zaza tools    # Select the tool → choose a direct provider
```
...
When you select a non-gateway provider in `zaza tools`, the `use_gateway` flag is automatically set to `false` to prevent contradictory config.
...
For self-hosted or custom gateway deployments, you can override the gateway endpoints via environment variables in `~/.agent-zaza/data/.env`:

```bash
TOOL_GATEWAY_DOMAIN=zaza.com     # Base domain for gateway routing
...
Tools that were routed through the gateway will stop working until you [renew your subscription](https://portal.zaza.com/manage-subscription) or switch to direct API keys via `zaza tools`.
...
Modal (serverless terminal backend) is available as an optional add-on through the Nous subscription. It's not enabled by the Tool Gateway prompt — configure it separately via `zaza setup terminal` or in `config.yaml`.
