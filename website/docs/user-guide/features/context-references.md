...
Type `@` followed by a reference to inject content directly into your message. ZAZA expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- ZAZA env: `$ZAZA_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$ZAZA_HOME/skills/.hub/`
