...
description: "Customize Agent ZAZA's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Agent ZAZA's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `ZAZA_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who ZAZA is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

ZAZA now seeds a default `SOUL.md` automatically in:

```text
~/.agent-zaza/data/SOUL.md
```

More precisely, it uses the current instance's `ZAZA_HOME`, so if you run ZAZA with a custom home directory, it will use:
...
- ZAZA creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- ZAZA loads `SOUL.md` only from `ZAZA_HOME`
- ZAZA does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, ZAZA falls back to a built-in default identity
...
If ZAZA loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `ZAZA_HOME`, the personality belongs to the ZAZA instance itself.

That also makes it easier to teach users:
- "Edit `~/.agent-zaza/data/SOUL.md` to change ZAZA' default personality."
...
- how ZAZA should handle uncertainty, disagreement, or ambiguity
...
## What ZAZA injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, ZAZA falls back to a built-in default identity ("You are Agent ZAZA, an intelligent AI assistant created by ZAZA..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
ZAZA ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain ZAZA, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives ZAZA its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.agent-zaza/data/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.agent-zaza/data/SOUL.md`
...
- [SOUL.md Guide](/docs/guides/use-soul-with-zaza)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how ZAZA speaks
- `display.skin` and `/skin` affect how ZAZA looks in the terminal
