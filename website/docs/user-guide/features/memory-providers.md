...
Agent ZAZA ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
zaza memory setup      # interactive picker + configuration
zaza memory status     # check what's active
zaza memory off        # disable external provider
```

You can also select the active memory provider via `zaza plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.agent-zaza/data/config.yaml`:
...
When a memory provider is active, ZAZA automatically:
...
**Setup Wizard:**
```bash
zaza honcho setup        # (legacy command) 
# or
zaza memory setup        # select "honcho"
```

**Config:** `$ZAZA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$ZAZA_HOME/honcho.json` > `~/.agent-zaza/data/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/zaza-ai/agent-zaza/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/zaza).
...
:::tip Migrating from `zaza honcho`
If you previously used `zaza honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per ZAZA profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All ZAZA profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per ZAZA profile. Host key `zaza` → default; `zaza.<profile>` for others. |
...
```bash
zaza profile create coder --clone
```

`--clone` creates a `zaza.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every ZAZA profile, creates host blocks for any profile without one, inherits settings from the default `zaza` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
See the [config reference](https://github.com/zaza-ai/agent-zaza/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/zaza).
...
# Then configure ZAZA
zaza memory setup    # select "openviking"
# Or manually:
zaza config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.agent-zaza/data/.env
```
...
**Setup:**
```bash
zaza memory setup    # select "mem0"
# Or manually:
zaza config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.agent-zaza/data/.env
```

**Config:** `$ZAZA_HOME/mem0.json`

| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `zaza-user` | User identifier |
| `agent_id` | `zaza` | Agent identifier |
...
**Setup:**
```bash
zaza memory setup    # select "hindsight"
# Or manually:
zaza config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.agent-zaza/data/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p zaza ui start`

**Config:** `$ZAZA_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `zaza` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Agent ZAZA and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/ZAZA/agent-zaza/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
zaza memory setup    # select "holographic"
# Or manually:
zaza config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.zaza-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$ZAZA_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
zaza memory setup    # select "retaindb"
# Or manually:
zaza config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.agent-zaza/data/.env
```
...
# Then configure ZAZA
zaza memory setup    # select "byterover"
# Or manually:
zaza config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$ZAZA_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
zaza memory setup    # select "supermemory"
# Or manually:
zaza config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.agent-zaza/data/.env
```

**Config:** `$ZAZA_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `zaza` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `zaza-{identity}` → `zaza-coder`) to isolate memories per ZAZA profile
...
- **Local storage providers** (Holographic, ByteRover) use `$ZAZA_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$ZAZA_HOME/` so each profile has its own credentials
