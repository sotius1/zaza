...
title: "Git Worktrees"
description: "Run multiple ZAZA agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Agent ZAZA is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with ZAZA so each session has a clean, isolated working directory.

## Why Use Worktrees with ZAZA?

ZAZA treats the **current working directory** as the project root:

- CLI: the directory where you run `zaza` or `zaza chat`
...
git worktree add ../repo-feature feature/zaza-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/zaza-experiment` checked out in that directory

Now you can `cd` into the new worktree and run ZAZA there:

```bash
cd ../repo-feature

# Start ZAZA in the worktree
zaza
```
...
git worktree add ../repo-experiment-a feature/zaza-a
git worktree add ../repo-experiment-b feature/zaza-b
```
...
- Works on its own branch (`feature/zaza-a` vs `feature/zaza-b`).
...
- ZAZA checkpoint data under `~/.agent-zaza/data/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per ZAZA experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/zaza-checkpoints-docs`, `feature/zaza-refactor-tests`.
- **Commit frequently**
...
- **Avoid running ZAZA from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `zaza -w` (Automatic Worktree Mode)

ZAZA has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `zaza/zaza-<hash>`).
...
```bash
zaza -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `zaza -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each ZAZA session its own clean checkout.
