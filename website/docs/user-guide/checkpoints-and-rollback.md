...
Agent ZAZA automatically snapshots your project before **destructive operations** and lets you restore it with a single command. Checkpoints are **enabled by default** — there's zero cost when no file-mutating tools fire.

This safety net is powered by an internal **Checkpoint Manager** that keeps a separate shadow git repository under `~/.agent-zaza/data/checkpoints/` — your real project `.git` is never touched.
...
- ZAZA detects when tools are about to **modify files** in your working tree.
...
flowchart LR
  ...
  shadowRepo["Shadow git repo\n~/.agent-zaza/data/checkpoints/<hash>"]
...
Checkpoints are enabled by default. Configure in `~/.agent-zaza/data/config.yaml`:
...
checkpoints:
  ...
  # Auto-maintenance (opt-in): sweep ~/.agent-zaza/data/checkpoints/ at startup
  ...
  # .last_prune marker inside ~/.agent-zaza/data/checkpoints/.
...
ZAZA responds with a formatted list showing change statistics:
...
To keep checkpointing safe and fast, ZAZA applies several guardrails:

- **Git availability** — if `git` is not found on `PATH`, checkpoints are transparently disabled.
- **Directory scope** — ZAZA skips overly broad directories (root `/`, home `$HOME`).
...
- A `ZAZA_WORKDIR` file pointing back to the original project root
...
- **Combine with Git worktrees** for maximum safety — keep each ZAZA session in its own worktree/branch, with checkpoints as an extra layer.
