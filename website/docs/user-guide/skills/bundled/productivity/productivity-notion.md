...
3. Store it in `~/.agent-zaza/data/.env`:
   ```
...
curl -s -X PATCH "https://api.notion.com/v1/blocks/{page_id}/children" \
  ...
  -d '{
    "children": [
      {"object": "block", "type": "paragraph", "paragraph": {"rich_text": [{"text": {"content": "Hello from ZAZA!"}}]}}
    ]
  }'
```
...
- Add `-s` flag to curl to suppress progress bars (cleaner output for ZAZA)
