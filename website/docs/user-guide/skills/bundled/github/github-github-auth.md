...
- Give it a name like "agent-zaza"
- Select scopes:
...
- Give it a title like "agent-zaza-&lt;machine-name>"
...
elif [ -f ~/.agent-zaza/data/.env ] && grep -q "^GITHUB_TOKEN=" ~/.agent-zaza/data/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.agent-zaza/data/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
