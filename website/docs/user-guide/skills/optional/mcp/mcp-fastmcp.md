...
| | |
|---|---|
| Source | Optional — install with `zaza skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into ZAZA or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to ZAZA. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.agent-zaza/data/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.agent-zaza/data/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is ZAZA integration, either:

- configure the server in `~/.agent-zaza/data/config.yaml` using the `native-mcp` skill, or
...
### ZAZA cannot see the deployed server

The server-building part may be correct while the ZAZA config is not. Load the `native-mcp` skill and configure the server in `~/.agent-zaza/data/config.yaml`, then restart ZAZA.
