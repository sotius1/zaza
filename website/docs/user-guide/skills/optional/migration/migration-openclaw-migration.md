---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Agent ZAZA"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Agent ZAZA"
---
...
Migrate a user's OpenClaw customization footprint into Agent ZAZA. Imports ZAZA-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `zaza skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Agent ZAZA (ZAZA) |
| License | MIT |
| Tags | `Migration`, `OpenClaw`, `ZAZA`, `Memory`, `Persona`, `Import` |
| Related skills | [`agent-zaza`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-agent-zaza) |
...
# OpenClaw -> ZAZA Migration

Use this skill when a user wants to move their OpenClaw setup into Agent ZAZA with minimal manual cleanup.
...
```bash
zaza claw migrate              # Full interactive migration
zaza claw migrate --dry-run    # Preview what would be migrated
zaza claw migrate --preset user-data   # Migrate without secrets
zaza claw migrate --overwrite  # Overwrite existing conflicts
zaza claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `zaza setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_zaza.py` to:

- import `SOUL.md` into the ZAZA home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into ZAZA memory entries
- merge OpenClaw command approval patterns into ZAZA `command_allowlist`
- migrate ZAZA-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.agent-zaza/data/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen ZAZA workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.agent-zaza/data/tts/`
- archive non-secret docs that do not have a direct ZAZA destination
...
- `scripts/openclaw_to_zaza.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.agent-zaza/data/skills/migration/openclaw-migration/scripts/openclaw_to_zaza.py`

Do not guess a shorter path like `~/.agent-zaza/data/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.agent-zaza/data/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of ZAZA-compatible secrets, currently:
...
ZAZA CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in ZAZA. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a ZAZA workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing ZAZA skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.agent-zaza/data/skills/migration/openclaw-migration/scripts/openclaw_to_zaza.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.agent-zaza/data/skills/migration/openclaw-migration/scripts/openclaw_to_zaza.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.agent-zaza/data/skills/migration/openclaw-migration/scripts/openclaw_to_zaza.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.agent-zaza/data/skills/migration/openclaw-migration/scripts/openclaw_to_zaza.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.agent-zaza/data/skills/migration/openclaw-migration/scripts/openclaw_to_zaza.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.agent-zaza/data/skills/migration/openclaw-migration/scripts/openclaw_to_zaza.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of ZAZA unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty ZAZA targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean ZAZA destination. Unsupported auth blobs must still be reported as skipped.
...
- ZAZA persona state imported
- ZAZA memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.agent-zaza/data/skills/openclaw-imports/`
