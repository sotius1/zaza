...
description: "Configure and use Honcho memory with ZAZA -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with ZAZA -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `zaza skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for ZAZA

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every ZAZA profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/zaza#running-honcho-locally-with-zaza

### Verify

```bash
zaza honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this ZAZA profile's AI peer
...
Honcho models conversations as interactions between **peers**. ZAZA creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this ZAZA instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each ZAZA run |
| `global` | Single session across all directories |

Manual override: `zaza honcho map my-project-name`
...
Each ZAZA profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block zaza.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `zaza.coder` host block in `honcho.json`
...
```bash
zaza honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for ZAZA when Honcho memory is active.
...
Config file: `$ZAZA_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `zaza honcho setup`. Ensure `memory.provider: honcho` is in `~/.agent-zaza/data/config.yaml`.

### Memory not persisting across sessions
Check `zaza honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `zaza profile create <name> --clone`. For existing profiles: `zaza honcho sync`.
...
| Command | Description |
|---------|-------------|
| `zaza honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `zaza honcho status` | Show resolved config, connection test, peer info for active profile |
| `zaza honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `zaza honcho disable` | Disable Honcho for the active profile |
| `zaza honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `zaza honcho peers` | Show peer identities across all profiles |
| `zaza honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `zaza honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `zaza honcho sessions` | List known directory-to-session-name mappings |
| `zaza honcho map <name>` | Map current working directory to a Honcho session name |
| `zaza honcho identity` | Seed AI peer identity or show both peer representations |
| `zaza honcho sync` | Create host blocks for all ZAZA profiles that don't have one yet |
| `zaza honcho migrate` | Step-by-step migration guide from OpenClaw native memory to ZAZA + Honcho |
| `zaza memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `zaza memory status` | Show active memory provider and config |
| `zaza memory off` | Disable external memory provider |
