...
| | |
|---|---|
| Source | Optional — install with `zaza skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Agent ZAZA |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during ZAZA terminal calls (desktop app flow only)
...
### Service Account (recommended for ZAZA)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.agent-zaza/data/.env` (the skill will prompt for this on first load).
...
## ZAZA Execution Pattern (desktop app flow)

ZAZA terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/zaza-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/zaza-op.sock"
