---
title: "Telephony — Give ZAZA phone capabilities without core tool changes"
sidebar_label: "Telephony"
description: "Give ZAZA phone capabilities without core tool changes"
---
...
Give ZAZA phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `zaza skills install official/productivity/telephony` |
...
This optional skill gives ZAZA practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `~/.agent-zaza/data/.env`
...
It does **not** turn ZAZA into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to ZAZA memory
...
### 1) "I want ZAZA to own a real phone number"
Use **Twilio**.
...
- pairs well with ZAZA `text_to_speech` plus a public file host or tunnel
...
### `~/.agent-zaza/data/telephony_state.json`
...
```bash
SCRIPT="$(find ~/.agent-zaza/data/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
zaza skills search telephony
zaza skills install official/productivity/telephony
```
...
Then save credentials into ZAZA:
...
3. Buy it and save it into `~/.agent-zaza/data/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is ZAZA calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing ZAZA's existing `text_to_speech` support.

Use this when:
- you want the call to use ZAZA's configured TTS voice rather than Twilio `<Say>`
...
Recommended ZAZA TTS -> Twilio Play workflow:

1. Generate the audio with ZAZA `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask ZAZA to create the message audio with `text_to_speech`
...
Important note:
- ZAZA TTS is great for prerecorded outbound messages
...
- ZAZA STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to ZAZA memory.
...
- Do not store arbitrary third-party phone numbers in ZAZA memory.
...
3. persist that number to `~/.agent-zaza/data/.env`
