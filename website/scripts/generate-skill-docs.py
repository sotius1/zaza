...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/ZAZA/agent-zaza/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    zaza_meta = (fm.get("metadata") or {}).get("zaza") or {}
    tags = zaza_meta.get("tags") or []
    related = zaza_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `zaza skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Agent ZAZA"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "ZAZA ships with a large built-in skill library copied into `~/.agent-zaza/data/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
