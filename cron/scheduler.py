...
Uses a file-based lock (~/.agent-zaza/data/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `zaza update` reloads
# the module) fail with ModuleNotFoundError for zaza_time et al.
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``zaza tools`` config for the ``cron`` platform.
    ...
    if per_job:
        return per_job
    try:
        from zaza_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("ZAZA_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid ZAZA_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within ZAZA_HOME/scripts/.  Both relative and
    ...
    Args:
        script_path: Path to a Python script.  Relative paths are resolved
            against ZAZA_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    scripts_dir = get_zaza_home() / "scripts"
    ...
    # escape — scripts MUST reside within ZAZA_HOME/scripts/.
    try:
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_zaza_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    origin = _resolve_origin(job)
    _cron_session_id = f"cron_{job_id}_{_zaza_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    os.environ["ZAZA_CRON_SESSION"] = "1"
    ...
    try:
        ...
        try:
            load_dotenv(str(_zaza_home / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_zaza_home / ".env"), override=True, encoding="latin-1")

        delivery_target = _resolve_delivery_target(job)
        if delivery_target:
            _VAR_MAP["ZAZA_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["ZAZA_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            if delivery_target.get("thread_id") is not None:
                _VAR_MAP["ZAZA_CRON_AUTO_DELIVER_THREAD_ID"].set(str(delivery_target["thread_id"]))

        model = job.get("model") or os.getenv("ZAZA_MODEL") or ""

        # Load config.yaml for model, reasoning, prefill, toolsets, provider routing
        _cfg = {}
        try:
            import yaml
            _cfg_path = str(_zaza_home / "config.yaml")
        ...
        prefill_file = os.getenv("ZAZA_PREFILL_MESSAGES_FILE", "") or _cfg.get("prefill_messages_file", "")
        if prefill_file:
            pfpath = Path(prefill_file).expanduser()
            if not pfpath.is_absolute():
                pfpath = _zaza_home / pfpath
            if pfpath.exists():
                try:
        ...
        try:
            runtime_kwargs = {
                "requested": job.get("provider") or os.getenv("ZAZA_INFERENCE_PROVIDER"),
            }
            if job.get("base_url"):
        ...
        agent = AIAgent(
            ...
            # ZAZA_HOME. When a workdir is configured, also inject project
        ...
        # override via ZAZA_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("ZAZA_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid ZAZA_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
**Job ID:** {job_id}
**Run Time:** {_zaza_now().strftime('%Y-%m-%d %H:%M:%S')}
...
def tick(verbose: bool = True, adapters=None, loop=None) -> int:
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _zaza_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _zaza_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set ZAZA_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("ZAZA_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid ZAZA_CRON_MAX_PARALLEL value; defaulting to unbounded")
