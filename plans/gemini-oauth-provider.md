...
## Client ID
- Need to register a "Desktop app" OAuth client on a ZAZA GCP project
...
## Token Lifecycle
- Store at `~/.agent-zaza/data/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `zaza_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `zaza_cli/models.py` — Add Gemini model catalog
4. `zaza_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `zaza_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `zaza_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
## Prerequisites
- ZAZA GCP project with Desktop OAuth client registered
- OR: accept user-provided client_id via ZAZA_GEMINI_CLIENT_ID env var
...
- agent-zaza Copilot OAuth: `zaza_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
