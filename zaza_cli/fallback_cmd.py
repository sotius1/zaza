"""
zaza fallback — manage the fallback provider chain.
...
Subcommands:
  zaza fallback [list]   Show the current fallback chain (default when no subcommand)
  zaza fallback add      Pick provider + model via the same picker as `zaza model`,
                           then append the selection to the chain
  zaza fallback remove   Pick an entry to delete from the chain
  zaza fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.agent-zaza/data/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from zaza_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `zaza model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  zaza fallback add")
        print()
        return
    ...
    print("  Docs: https://agent-zaza.zaza.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `zaza model`, then append the selection to the chain."""
    from zaza_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `zaza model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `zaza fallback list` to view, or `zaza fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``zaza fallback [subcommand]``."""
