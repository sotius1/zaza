"""zaza hooks — inspect and manage shell-script hooks.

Usage::

    zaza hooks list
    zaza hooks test <event> [--for-tool X] [--payload-file F]
    zaza hooks revoke <command>
    zaza hooks doctor

Consent records live under ``~/.agent-zaza/data/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.agent-zaza/data/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``zaza hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: zaza hooks {list|test|revoke|doctor}")
        print("Run 'zaza hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.agent-zaza/data/config.yaml.")
        print("See `zaza hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `zaza hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `zaza hooks test` and `zaza hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (ZAZA wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `zaza hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `zaza hooks doctor` would execute
