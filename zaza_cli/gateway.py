"""
Gateway subcommand for zaza CLI.

Handles: agent-zaza gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_zaza_home is imported lazily at call sites to avoid ImportError
# when zaza_constants is cached from a pre-update version during `zaza update`.
from zaza_cli.setup import (
    print_header, print_info, print_success, print_warning, print_error,
    prompt, prompt_choice, prompt_yes_no,
)
from zaza_cli.colors import Colors, color
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "agent-zaza-gateway*",
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    pids: list[int] = []
    patterns = [
        "zaza_cli.main gateway",
        "zaza_cli.main --profile",
        "zaza_cli.main -p",
        "zaza_cli/main.py gateway",
        "zaza_cli/main.py --profile",
        "zaza_cli/main.py -p",
        "agent-zaza gateway",
        "gateway/run.py",
    ]
    current_home = str(get_zaza_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"ZAZA_HOME={current_home}" in command
            )

        if "--profile " in command or " -p " in command:
            return False
        if "ZAZA_HOME=" in command and f"ZAZA_HOME={current_home}" not in command:
            return False
        return True
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``zaza update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            ZAZA profile are returned.
    """
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}agent-zaza gateway status\n"
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from zaza_constants import is_container
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `agent-zaza gateway`")
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (ZAZA_HOME-scoped).
...
from zaza_constants import is_container, is_termux, is_wsl
...
_SERVICE_BASE = "agent-zaza-gateway"
SERVICE_DESCRIPTION = "Agent ZAZA Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current ZAZA_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (ZAZA_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from zaza_constants import get_default_zaza_root
    home = get_zaza_home().resolve()
    default = get_default_zaza_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary ZAZA_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(zaza_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when ZAZA_HOME is a named profile.

    For ``~/.agent-zaza/data/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        zaza_home: Optional explicit ZAZA_HOME path. Defaults to the current
            ``get_zaza_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from zaza_constants import get_default_zaza_root
    home = Path(zaza_home or str(get_zaza_home())).resolve()
    default = get_default_zaza_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this ZAZA_HOME.

    Default ``~/.agent-zaza/data`` returns ``agent-zaza-gateway`` (backward compatible).
    Profile ``~/.agent-zaza/data/profiles/coder`` returns ``agent-zaza-gateway-coder``.
    Any other ZAZA_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older ZAZA installs that predate the
# agent-zaza-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (agent-zaza-gateway-*.service) and unrelated third-party
# "zaza" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("zaza.service",)
...
def _find_legacy_zaza_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy ZAZA gateway units.

    Detects unit files installed by older ZAZA versions that used a
    different service name (e.g. ``zaza.service`` before the rename to
    ``agent-zaza-gateway.service``). When both a legacy unit and the current
    ``agent-zaza-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``agent-zaza-gateway-coder.service`` and unrelated third-party
      ``zaza-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``zaza.service`` running an unrelated
...
def has_legacy_zaza_units() -> bool:
    """Return True when any legacy ZAZA gateway unit files exist."""
    return bool(_find_legacy_zaza_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy ZAZA gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_zaza_units()
    if not legacy:
        return
    print_warning("Legacy ZAZA gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current agent-zaza-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    agent-zaza gateway migrate-legacy")


def remove_legacy_zaza_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy ZAZA gateway unit files.

    Iterates over whatever ``_find_legacy_zaza_units()`` returns — which is
    ...
    legacy = _find_legacy_zaza_units()
    if not legacy:
        print("No legacy ZAZA gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy ZAZA gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: agent-zaza gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:
            ...
            print_info("  Re-run with: sudo agent-zaza gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    agent-zaza gateway uninstall")
    print_info("    sudo agent-zaza gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so ZAZA can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo agent-zaza gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo agent-zaza gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo agent-zaza gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode ZAZA often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.agent-zaza/data`` → ``ai.zaza.gateway.plist`` (backward compatible).
    Profile ``~/.agent-zaza/data/profiles/coder`` → ``ai.zaza.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.zaza.gateway-{suffix}" if suffix else "ai.zaza.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.agent-zaza/data/agent-zaza  -> /home/alice/.agent-zaza/data/agent-zaza
      /opt/zaza                 -> /opt/zaza  (kept as-is)
...
def _zaza_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current ZAZA_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_zaza_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.zaza                    → /home/alice/.zaza
      /root/.agent-zaza/data/profiles/coder     → /home/alice/.agent-zaza/data/profiles/coder
      /opt/custom-zaza               → /opt/custom-zaza  (kept as-is)
    """
    current_zaza = get_zaza_home().resolve()
    current_default = (Path.home() / ".agent-zaza/data").resolve()
    target_default = Path(target_home_dir) / ".agent-zaza/data"

    # Default ~/.agent-zaza/data → remap to target user's default
    if current_zaza == current_default:
        return str(target_default)

    # Profile or subdir of ~/.agent-zaza/data → preserve the relative structure
    try:
        relative = current_zaza.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.agent-zaza/data) — keep as-is
        return str(current_zaza)
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        zaza_home = _zaza_home_for_target_user(home_dir)
        profile_arg = _profile_arg(zaza_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m zaza_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="ZAZA_HOME={zaza_home}"
Restart=on-failure
RestartSec=30
...
"""
    ...
    zaza_home = str(get_zaza_home().resolve())
    profile_arg = _profile_arg(zaza_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current ZAZA install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    # Offer to remove legacy units (zaza.service from pre-rename installs)
    # before installing the new agent-zaza-gateway.service. If both remain, they
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}agent-zaza gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}agent-zaza gateway status{scope_flag}             # Check status")
...
def systemd_restart(system: bool = False):
    ...
    if pid is not None and _request_gateway_self_restart(pid):
        ...
        # unit immediately so `agent-zaza gateway restart` behaves idempotently.
        _run_systemctl(
            ["reset-failed", svc],
            system=system,
            check=False,
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}agent-zaza gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}agent-zaza gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}agent-zaza gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}agent-zaza gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.zaza.gateway-{suffix}" if suffix else "ai.zaza.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current ZAZA install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  agent-zaza gateway status             # Check status")
    from zaza_constants import display_zaza_home as _dhh
...
def launchd_stop():
    ...
    # `agent-zaza gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current ZAZA install")
    else:
        print("⚠ Service definition is stale relative to the current ZAZA install")
        print("  Run: agent-zaza gateway start")
    ...
    if deep:
        log_file = get_zaza_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and ZAZA will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @zaza:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. zaza) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where ZAZA delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your ZAZA agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address ZAZA will use (e.g., zaza@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. ZAZA connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: zaza pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and ZAZA will connect automatically over WebSocket",
        ],
        "vars": [
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_zaza_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'zaza pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: zaza pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'agent-zaza gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from zaza_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from zaza_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_weixin():
    ...
    print()
    print_info("  1. ZAZA will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. ZAZA will store the returned account_id/token in ~/.agent-zaza/data/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `agent-zaza gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_zaza_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `zaza pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `zaza pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"ZAZAAgent\"")
            ...
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: agent-zaza gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    try:
                        ...
                        print_info("  You can try manually: agent-zaza gateway install")
                else:
                    print_info("  You can install later: agent-zaza gateway install")
                    if supports_systemd_services():
                        print_info("  Or as a boot-time service: sudo agent-zaza gateway install --system")
                    print_info("  Or run in foreground:  agent-zaza gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: agent-zaza gateway run")
                print_info("  For persistence:   tmux new -s zaza 'agent-zaza gateway run'")
                ...
                if is_termux():
                    ...
                    print_info("  Run in foreground: agent-zaza gateway run")
                    print_info(f"  Or start it manually in the background (best effort): nohup agent-zaza gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
                else:
                    print_info("  Service install not supported on this platform.")
                    print_info("  Run in foreground: agent-zaza gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'agent-zaza gateway setup' when ready.")
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: agent-zaza gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: agent-zaza gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s zaza 'agent-zaza gateway run'")
                print()
            ...
            print()
            print("  agent-zaza gateway run                              # direct foreground")
            print("  tmux new -s zaza 'agent-zaza gateway run'         # persistent via tmux")
            print("  nohup agent-zaza gateway run > ~/.agent-zaza/data/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: agent-zaza gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: agent-zaza gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: agent-zaza gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: agent-zaza gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if supports_systemd_services():
                ...
                if linger_ok is not True:
                    ...
                    print()
                    print("  Then restart the gateway:")
                    print("    agent-zaza gateway restart")
                    return
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: agent-zaza gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  agent-zaza gateway install")
                    print("  sudo agent-zaza gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  agent-zaza gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup agent-zaza gateway run > ~/.agent-zaza/data/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s zaza 'agent-zaza gateway run'         # persistent via tmux")
                    print("  nohup agent-zaza gateway run > ~/.agent-zaza/data/logs/gateway.log 2>&1 &  # background")
                else:
                    print("  agent-zaza gateway install  # Install as user service")
                    print("  sudo agent-zaza gateway install --system  # Install as boot-time system service")

    elif subcmd == "migrate-legacy":
        # Stop, disable, and remove legacy ZAZA gateway unit files from
        # pre-rename installs (e.g. zaza.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_zaza_units(interactive=not yes, dry_run=dry_run)
