"""
Backup and import commands for zaza CLI.

`zaza backup` creates a zip archive of the entire ~/.agent-zaza/data/ directory
(excluding the agent-zaza repo and transient files).

`zaza import` restores from a backup zip, overlaying onto the current
ZAZA_HOME root.
"""
...
from zaza_constants import get_default_zaza_root, get_zaza_home, display_zaza_home
...
_EXCLUDED_DIRS = {
    "agent-zaza",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to zaza root) should be skipped."""
...
def run_backup(args) -> None:
    """Create a zip backup of the ZAZA home directory."""
    zaza_root = get_default_zaza_root()

    if not zaza_root.is_dir():
        print(f"Error: ZAZA home directory not found at {zaza_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"zaza-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"zaza-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_zaza_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(zaza_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(zaza_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(zaza_root)

            if _should_exclude(rel):
                continue

            # Skip the output zip itself if it happens to be inside zaza root
            try:
    ...
    print(f"\nRestore with: zaza import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a ZAZA backup.
    ...
    # Look for telltale files that a zaza home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a ZAZA backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.zaza/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a zaza dir name
        if prefix in (".agent-zaza/data", "zaza"):
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_zaza_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (zaza_root / "config.yaml").exists()
        has_env = (zaza_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has ZAZA configuration.")
        ...
        zaza_root.mkdir(parents=True, exist_ok=True)
        ...
        for member in members:
            ...
            target = zaza_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(zaza_root.resolve())
            except ValueError:
        ...
        profiles_dir = zaza_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # zaza_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: zaza profile list  (after installing zaza)")

        # Guidance
        print()
        if not (zaza_root / "agent-zaza").is_dir():
            print("Note: The agent-zaza codebase was not included in the backup.")
            print("  If this is a fresh install, run: zaza update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  zaza -p {pname} gateway install")

        print("Done. Your ZAZA configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and zaza backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to ZAZA_HOME).
...
# — `zaza update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(zaza_home: Optional[Path] = None) -> Path:
    home = zaza_home or get_zaza_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    zaza_home: Optional[Path] = None,
) -> Optional[str]:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(zaza_home)
    if not root.exists():
        return []
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(zaza_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for zaza backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_zaza_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, zaza_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``zaza_root`` to ``out_path``.
...
def _pre_update_backup_dir(zaza_home: Optional[Path] = None) -> Path:
    home = zaza_home or get_zaza_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of ZAZA_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<ZAZA_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``zaza update``) should continue even if the backup fails.
    """
    zaza_root = zaza_home or get_default_zaza_root()
    if not zaza_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(zaza_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, zaza_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `zaza claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of ZAZA_HOME under ``backups/`` before a
    ``zaza claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``zaza import <archive>``.  Writes to
    ``<ZAZA_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `zaza import` and the
