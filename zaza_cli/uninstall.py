"""
Agent ZAZA Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.agent-zaza/data/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove ZAZA PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing agent-zaza or zaza PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Agent ZAZA" comment and following line
                if '# Agent ZAZA' in line or '# agent-zaza' in line:
                    skip_next = True
                    continue
                if skip_next and ('zaza' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing zaza
                if 'zaza' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the zaza wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "zaza",
        Path("/usr/local/bin/zaza"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains zaza_cli reference)
                content = wrapper.read_text()
                if 'zaza_cli' in content or 'agent-zaza' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``zaza gateway run`` processes
    ...
    try:
        from zaza_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from zaza_cli.gateway import get_launchd_plist_path
...
def _is_default_zaza_home(zaza_home: Path) -> bool:
    """Return True when ``zaza_home`` points at the default (non-profile) root."""
    try:
        from zaza_constants import get_default_zaza_root
        return zaza_home.resolve() == get_default_zaza_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its ZAZA_HOME directory.

    We shell out to ``zaza -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current ZAZA_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m zaza_cli.main` so we don't depend on a `zaza`
    #    wrapper that may be half-removed mid-uninstall.
    zaza_invocation = [_sys.executable, "-m", "zaza_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                zaza_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's ZAZA_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.agent-zaza/data/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.agent-zaza/data/ for future reinstall
    """
    project_root = get_project_root()
    zaza_home = get_zaza_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie ZAZA_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_zaza_home(zaza_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Agent ZAZA Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {zaza_home / 'config.yaml'}")
    print(f"  Secrets: {zaza_home / '.env'}")
    print(f"  Data:    {zaza_home / 'cron/'}, {zaza_home / 'sessions/'}, {zaza_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their ZAZA_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL ZAZA data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the ZAZA code but keep your configuration and data.")
    ...
    log_info("Removing zaza command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.agent-zaza/data/, just remove the agent-zaza subdir
            if zaza_home in project_root.parents or project_root.parent == zaza_home:
    ...
    # 5. Optionally remove ~/.agent-zaza/data/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile ZAZA_HOME dirs live under
        ...
        try:
            if zaza_home.exists():
                shutil.rmtree(zaza_home)
                log_success(f"Removed {zaza_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {zaza_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {zaza_home}")
    ...
    if not full_uninstall:
        ...
        print(color("  curl -fsSL https://raw.githubusercontent.com/ZAZA/agent-zaza/main/scripts/install.sh | bash", Colors.DIM))
        print()
    
    print(color("Reload your shell to complete the process:", Colors.YELLOW))
    print("  source ~/.bashrc  # or ~/.zshrc")
    print()
    print("Thank you for using Agent ZAZA! ⚕")
    print()
