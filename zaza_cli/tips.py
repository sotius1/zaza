...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or zaza -c.",
    ...
    "Ctrl+Z suspends ZAZA to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "zaza -c resumes your most recent CLI session. zaza -c \"project name\" resumes by title.",
    "zaza -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "zaza -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "zaza chat -t web,terminal enables only specific toolsets for a focused session.",
    "zaza chat -s github-pr-workflow preloads a skill at launch.",
    "zaza chat -q \"query\" runs a single non-interactive query and exits.",
    "zaza chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "zaza chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "zaza --yolo bypasses all dangerous command approval prompts for the entire session.",
    "zaza chat --source telegram tags the session for filtering in zaza sessions list.",
    "zaza -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "zaza doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "zaza dump outputs a compact setup summary — great for bug reports.",
    "zaza config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "zaza config edit opens config.yaml in your default editor.",
    "zaza config check scans for missing or stale configuration options.",
    "zaza sessions browse opens an interactive session picker with search.",
    "zaza sessions stats shows session counts by platform and database size.",
    "zaza sessions prune --older-than 30 cleans up old sessions.",
    "zaza skills search react --source skills-sh searches the skills.sh public directory.",
    "zaza skills check scans installed hub skills for upstream updates.",
    "zaza skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "zaza skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "zaza mcp add github --command npx adds MCP servers from the command line.",
    "zaza mcp serve runs ZAZA itself as an MCP server for other agents.",
    "zaza auth add lets you add multiple API keys for credential pool rotation.",
    "zaza completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "zaza logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "zaza backup creates a zip backup of your entire ZAZA home directory.",
    "zaza profile create coder creates an isolated profile that becomes its own command.",
    "zaza profile create work --clone copies your current config and keys to a new profile.",
    "zaza update syncs new bundled skills to ALL profiles automatically.",
    "zaza gateway install sets up ZAZA as a system service (systemd/launchd).",
    "zaza memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "zaza webhook subscribe creates event-driven webhook routes with HMAC validation.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as zaza -w).",
    ...
    "ZAZA_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call ZAZA tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'zaza profile create coder' creates the 'coder' command.",
    "zaza profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, ZAZA prints a resume command with session ID and stats.",
    "zaza sessions export backup.jsonl exports all sessions for backup or analysis.",
    "zaza -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "zaza skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .zaza/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: zaza cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "ZAZA runs on 18 platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, email, and more.",
    "zaza gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to ZAZA via a local macOS server.",
    ...
    "Context files (.zaza.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.agent-zaza/data/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "ZAZA loads project context from .zaza.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — zaza tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.agent-zaza/data/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "zaza chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "zaza plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "zaza acp runs ZAZA as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "ZAZA_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "zaza login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "zaza status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.agent-zaza/data/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make ZAZA your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.agent-zaza/data/SOUL.md to customize.",
    ...
    "Before a gateway session reset, ZAZA auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.agent-zaza/data/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at ZAZA_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "ZAZA_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in ZAZA_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.agent-zaza/data/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'zaza profile create ls' would be rejected.",
    "zaza profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "zaza claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "ZAZA_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via zaza tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.agent-zaza/data/.env.",
    "Skills from trusted repos (ZAZA) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.agent-zaza/data/skills/.hub/quarantine/ holds skills pending security review.",
]
