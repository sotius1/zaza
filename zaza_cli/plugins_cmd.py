"""``zaza plugins`` CLI subcommand — install, update, remove, and list plugins.

Plugins are installed from Git repositories into ``~/.agent-zaza/data/plugins/``.
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from zaza_cli.config import get_env_value, save_env_value  # noqa: F811
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_zaza_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]✓[/green] Saved to {display_zaza_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_zaza_home()}/.env later)[/dim]")
...
def cmd_install(
        ...
        if target.exists():
            if not force:
                console.print(
                    ...
                    f"[bold]zaza plugins update {plugin_name}[/bold] to pull latest."
                )
                sys.exit(1)
    ...
    if not (target / "plugin.yaml").exists() and not (target / "__init__.py").exists():
        console.print(
            ...
            f"or __init__.py. It may not be a valid ZAZA plugin."
        )
    ...
    if should_enable:
        ...
        console.print(
            ...
            f"Run `zaza plugins enable {installed_name}` to activate.[/dim]"
        )

    console.print("[dim]Restart the gateway for the plugin to take effect:[/dim]")
    console.print("[dim]  zaza gateway restart[/dim]")
    console.print()
...
def _plugin_exists(name: str) -> bool:
    ...
    import zaza_cli
    repo_plugins = _P(zaza_cli.__file__).resolve().parent.parent / "plugins"
...
def _discover_all_plugins() -> list:
    ...
    import zaza_cli
    repo_plugins = Path(zaza_cli.__file__).resolve().parent.parent / "plugins"
...
def cmd_list() -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] zaza plugins install owner/repo")
        return
    ...
    console.print(table)
    console.print()
    console.print("[dim]Interactive toggle:[/dim] zaza plugins")
    console.print("[dim]Enable/disable:[/dim] zaza plugins enable/disable <name>")
...
def plugins_command(args) -> None:
    """Dispatch zaza plugins subcommands."""
