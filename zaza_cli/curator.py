"""CLI subcommand: `zaza curator <subcommand>`.
...
def _cmd_run(args) -> int:
    ...
    if not args.synchronous:
        print("llm pass running in background — check `zaza curator status` later")
    return 0
...
# ---------------------------------------------------------------------------
# argparse wiring (called from zaza_cli.main)
# ---------------------------------------------------------------------------
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by zaza_cli.main fallthrough)."""
    parser = argparse.ArgumentParser(prog="zaza curator")
    register_cli(parser)
