"""
Agent ZAZA — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m zaza_cli.main web          # Start on http://127.0.0.1:9119
    python -m zaza_cli.main web --port 8080
"""
...
WEB_DIST = Path(os.environ["ZAZA_WEB_DIST"]) if "ZAZA_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="Agent ZAZA", version=__version__)
...
_SESSION_HEADER_NAME = "X-ZAZA-Session-Token"

# In-browser Chat tab (/chat, /api/pty, …).  Off unless ``zaza dashboard --tui``
# or ZAZA_DASHBOARD_TUI=1.  Set from :func:`start_server`.
...
# streamed to a per-action log file under ``~/.agent-zaza/data/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_zaza_home() / "logs"

# Short ``name`` (from the URL) → absolute log file path.
_ACTION_LOG_FILES: Dict[str, str] = {
    "gateway-restart": "gateway-restart.log",
    "zaza-update": "zaza-update.log",
}
...
def _spawn_zaza_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``zaza <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``zaza_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "zaza_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "ZAZA_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``zaza gateway restart`` in the background."""
    try:
        proc = _spawn_zaza_action(["gateway", "restart"], "gateway-restart")
...
@app.post("/api/zaza/update")
async def update_zaza():
    """Kick off ``zaza update`` in the background."""
    try:
        proc = _spawn_zaza_action(["update"], "zaza-update")
    except Exception as exc:
        _log.exception("Failed to spawn zaza update")
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ZAZA supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# the canonical ``agent-zaza auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    ZAZA resolves Anthropic creds in this order at runtime:
    1. ``~/.agent-zaza/data/.anthropic_oauth.json`` — ZAZA-managed PKCE flow
    ...
    try:
        from agent.anthropic_adapter import (
            read_zaza_oauth_credentials,
            read_claude_code_credentials,
            _ZAZA_OAUTH_FILE,
        )
    except ImportError:
        read_claude_code_credentials = None  # type: ignore
        read_zaza_oauth_credentials = None  # type: ignore
        _ZAZA_OAUTH_FILE = None  # type: ignore

    zaza_creds = None
    if read_zaza_oauth_credentials:
        try:
            zaza_creds = read_zaza_oauth_credentials()
        except Exception:
            zaza_creds = None
    if zaza_creds and zaza_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "zaza_pkce",
            "source_label": f"ZAZA PKCE ({_ZAZA_OAUTH_FILE})",
            "token_preview": _truncate_token(zaza_creds.get("accessToken")),
            "expires_at": zaza_creds.get("expiresAt"),
            "has_refresh_token": bool(zaza_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into ZAZA even
    when they also have a separate ZAZA-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "agent-zaza auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "agent-zaza auth add nous",
        "docs_url": "https://portal.zaza.com",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "agent-zaza auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "agent-zaza auth add qwen-oauth",
    ...
    },
    {
        ...
        "name": "MiniMax (OAuth)",
        "flow": "pkce",
        "cli_command": "agent-zaza auth add minimax-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from zaza_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("zaza_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same ZAZA-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in ("anthropic", "claude-code"):
        try:
            from agent.anthropic_adapter import _ZAZA_OAUTH_FILE
            if _ZAZA_OAUTH_FILE.exists():
                _ZAZA_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from zaza_cli.auth import clear_provider_auth
...
#          → persists to ~/.agent-zaza/data/.anthropic_oauth.json AND credential pool
...
# Guarded so zaza web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both ZAZA file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``agent-zaza auth add anthropic``.
    """
    from agent.anthropic_adapter import _ZAZA_OAUTH_FILE
    payload = {
    ...
    _ZAZA_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    _ZAZA_OAUTH_FILE.write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    """
    if provider_id == "nous":
        from zaza_cli.auth import _request_device_code, PROVIDER_REGISTRY
        import httpx
        pconfig = PROVIDER_REGISTRY["nous"]
        portal_base_url = (
            os.getenv("ZAZA_PORTAL_BASE_URL")
...
def _nous_poller(session_id: str) -> None:
    ...
    from zaza_cli.auth import _poll_for_token, refresh_nous_oauth_from_state
    ...
    try:
        ...
        from zaza_cli.auth import persist_nous_credentials
...
async def get_logs(
    ...
    from zaza_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_zaza_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from zaza_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
async def get_skills():
    ...
    from zaza_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from zaza_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
#
# The endpoint spawns the same ``zaza --tui`` binary the CLI uses, behind
...
from zaza_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _resolve_chat_argv(
    ...
    Default: whatever ``zaza --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``ZAZA_TUI_RESUME`` env var —
    matching what ``zaza_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    `sidecar_url` (when set) is forwarded as ``ZAZA_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from zaza_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    if resume:
        env["ZAZA_TUI_RESUME"] = resume

    if sidecar_url:
        env["ZAZA_TUI_SIDECAR_URL"] = sidecar_url
...
# ZAZA_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index():
        ...
        token_script = (
            f'<script>window.__ZAZA_SESSION_TOKEN__="{_SESSION_TOKEN}";'
            f"window.__ZAZA_DASHBOARD_EMBEDDED_CHAT__={chat_js};</script>"
        )
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",   "label": "ZAZA Lime",    "description": "Lime accent on graphite — the canonical Agent ZAZA look"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.agent-zaza/data/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.agent-zaza/data/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_zaza_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.agent-zaza/data/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as zaza_cli.plugins):
    1. User plugins:    ~/.agent-zaza/data/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.agent-zaza/data/plugins/  (only if ZAZA_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    search_dirs = [
        (get_zaza_home() / "plugins", "user"),
        (PROJECT_ROOT / "plugins" / "memory", "bundled"),
        (PROJECT_ROOT / "plugins", "bundled"),
    ]
    if os.environ.get("ZAZA_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".agent-zaza/data" / "plugins", "project"))
    ...
    for plugin in _get_dashboard_plugins():
        ...
        try:
            spec = importlib.util.spec_from_file_location(
                f"zaza_dashboard_plugin_{plugin['name']}", api_path,
            )
...
def start_server(
    ...
    *,
    ...
    print(f"  Agent ZAZA Web UI → http://{host}:{port}")
