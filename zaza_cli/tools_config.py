"""
Unified tool configuration for Agent ZAZA.

`zaza tools` and `zaza setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.agent-zaza/data/config.yaml under
...
from zaza_cli.nous_subscription import (
...
from zaza_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _ZAZA_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
_DEFAULT_OFF_TOOLSETS = {"moa", "homeassistant", "rl", "spotify", "discord", "discord_admin"}

# Platform-scoped toolsets: only appear in the `zaza tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``zaza tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from zaza_cli.plugins import discover_plugins, get_plugin_toolsets
...
from zaza_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "ZAZA_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "ZAZA_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "ZAZA_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "ZAZA_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "ZAZA_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_zaza_home()}/agent-zaza && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in agent-zaza directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/zaza/agent-zaza:latest"
            )
            return
        ...
        # Run the full `zaza auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.agent-zaza/data/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from zaza_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: zaza auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `zaza auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: zaza auth spotify")
        except Exception as exc:
        ...
        try:
            from zaza_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: zaza plugins enable observability/langfuse")
        _print_info("    Restart ZAZA for tracing to take effect.")
        _print_info("    Verify: zaza plugins list")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "agent-zaza-cli-toolset" (which include all _ZAZA_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "agent-zaza-cli-toolset") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    # otherwise saving via `zaza tools` (which flips has_explicit_config
    ...
    skip |= {k for k in TOOLSETS if k.startswith("zaza-")}
    ...
    # shipped as a bundled plugin but user must opt in via `zaza tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `zaza tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by zaza tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (agent-zaza-cli-toolset, zaza-telegram, etc.)
    ...
    # Opening `zaza tools` is the user's opt-in to reconfigure tools, so treat
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `zaza tools` and `zaza setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ ZAZA Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://agent-zaza.zaza.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from zaza_constants import display_zaza_home
    print(color(f"  Tool configuration saved to {display_zaza_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'zaza' or gateway restart.", Colors.DIM))
    print()
