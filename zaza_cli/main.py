#!/usr/bin/env python3
"""
ZAZA CLI - Main entry point.

Usage:
    zaza                     # Interactive chat (default)
    zaza chat                # Interactive chat
    zaza gateway             # Run gateway in foreground
    zaza gateway start       # Start gateway as service
    zaza gateway stop        # Stop gateway service
    zaza gateway status      # Show gateway status
    zaza gateway install     # Install gateway service
    zaza gateway uninstall   # Uninstall gateway service
    zaza setup               # Interactive setup wizard
    zaza logout              # Clear stored authentication
    zaza status              # Show status of all components
    zaza cron                # Manage cron jobs
    zaza cron list           # List cron jobs
    zaza cron status         # Check if cron scheduler is running
    zaza doctor              # Check configuration and dependencies
    zaza honcho setup                    # Configure Honcho AI memory integration
    zaza honcho status                   # Show Honcho config and connection status
    zaza honcho sessions                 # List directory → session name mappings
    zaza honcho map <name>               # Map current directory to a session name
    zaza honcho peer                     # Show peer names and dialectic settings
    zaza honcho peer --user NAME         # Set user peer name
    zaza honcho peer --ai NAME           # Set AI peer name
    zaza honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    zaza honcho mode                     # Show current memory mode
    zaza honcho mode [hybrid|honcho|local]  # Set memory mode
    zaza honcho tokens                   # Show token budget settings
    zaza honcho tokens --context N       # Set session.context() token cap
    zaza honcho tokens --dialectic N     # Set dialectic result char cap
    zaza honcho identity                 # Show AI peer identity representation
    zaza honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    zaza honcho migrate                  # Step-by-step migration guide: OpenClaw native → ZAZA + Honcho
    zaza version             Show version
    zaza update              Update to latest version
    zaza uninstall           Uninstall Agent ZAZA
    zaza acp                 Run as an ACP server for editor integration
    zaza sessions browse     Interactive session picker with search

    zaza claw migrate --dry-run  # Preview migration without changes
"""
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to ZAZA_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (zaza tools, zaza setup, zaza model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'zaza {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any zaza module import.
#
# Many modules cache ZAZA_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("ZAZA_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.agent-zaza/data/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set ZAZA_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # 2. If no flag, check active_profile in the zaza root
    if profile_name is None:
        try:
            from zaza_constants import get_default_zaza_root

            active_path = get_default_zaza_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set ZAZA_HOME
    if profile_name is not None:
        try:
            from zaza_cli.profiles import resolve_profile_env

            zaza_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent zaza from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["ZAZA_HOME"] = zaza_home
...
# Load .env from ~/.agent-zaza/data/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from zaza_cli.config import get_zaza_home
from zaza_cli.env_loader import load_zaza_dotenv

load_zaza_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → ZAZA_REDACT_SECRETS env
# var BEFORE zaza_logging imports agent.redact (which snapshots the flag at
# module-import time). Without this, config.yaml's toggle is ignored because
# the setup_logging() call below imports agent.redact, which reads the env var
# exactly once. Env var in .env still wins — this is config.yaml fallback only.
try:
    if "ZAZA_REDACT_SECRETS" not in os.environ:
        import yaml as _yaml_early
        _cfg_path = get_zaza_home() / "config.yaml"
        if _cfg_path.exists():
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    os.environ["ZAZA_REDACT_SECRETS"] = str(_early_redact).lower()
            del _early_sec_cfg
        del _cfg_path
except Exception:
    pass  # best-effort — redaction stays at default (enabled) on config errors

# Initialize centralized file logging early — all `zaza` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from zaza_logging import setup_logging as _setup_logging
...
try:
    from zaza_cli.config import load_config as _load_config_early
    from zaza_constants import apply_ipv4_preference as _apply_ipv4
...
from zaza_cli import __version__, __release_date__
from zaza_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from zaza_cli.config import get_env_path, get_zaza_home, load_config
    from zaza_cli.auth import get_auth_status

    # Determine whether ZAZA itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from zaza_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_zaza_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from zaza_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_zaza_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if ZAZA has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants ZAZA to use their tokens.
    if _has_zaza_config:
        try:
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    db = None
    try:
        from zaza_state import SessionDB
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, zaza_bin
        cli_args: the original CLI arguments (everything after 'zaza')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    zaza_bin = container_info["zaza_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo zaza {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo zaza {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(session_id: Optional[str], active_session_file: Optional[str] = None) -> None:
    ...
    print(f"  zaza --tui --resume {target}")
    if title:
        print(f'  zaza --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @zaza/ink is missing or node_modules is behind package-lock.json.
    ...
    """
    ink = root / "node_modules" / "@zaza" / "ink" / "package.json"
    if not ink.is_file():
        return True
...
def _find_bundled_tui(tui_dir: Path) -> Optional[Path]:
    """Directory whose dist/entry.js we should run: ZAZA_TUI_DIR first, else repo ui-tui."""
    env = os.environ.get("ZAZA_TUI_DIR")
    if env:
        p = Path(env)
...
def _tui_build_needed(tui_dir: Path) -> bool:
    if _zaza_ink_bundle_stale(tui_dir):
        return True
...
def _zaza_ink_bundle_stale(tui_dir: Path) -> bool:
    ink_root = tui_dir / "packages" / "zaza-ink"
...
def _ensure_tui_node() -> None:
    ...
    ``ZAZA_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("ZAZA_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    zaza_home = os.environ.get("ZAZA_HOME") or str(Path.home() / ".agent-zaza/data")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "ZAZA_HOME": zaza_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(zaza_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (ZAZA_TUI_DIR or ui-tui, build when stale)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("ZAZA_NODE")
    ...
    # pre-built dist + node_modules (nix / full ZAZA_TUI_DIR) skips npm.
    if not tui_dev:
        ext_dir = os.environ.get("ZAZA_TUI_DIR")
        if ext_dir:
            p = Path(ext_dir)
            if (p / "dist" / "entry.js").exists() and not _tui_need_npm_install(p):
                node = _node_bin("node")
    ...
    if _tui_need_npm_install(tui_dir):
        if not os.environ.get("ZAZA_QUIET"):
    ...
    if tui_dev:
        if _zaza_ink_bundle_stale(tui_dir):
            result = subprocess.run(
                [npm, "run", "build", "--prefix", "packages/zaza-ink"],
                cwd=str(tui_dir),
                capture_output=True,
                text=True,
            )
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from zaza_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="agent-zaza-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["ZAZA_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["ZAZA_PYTHON_SRC_ROOT"] = os.environ.get(
        "ZAZA_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("ZAZA_PYTHON", sys.executable)
    env.setdefault("ZAZA_CWD", os.getcwd())
    env.setdefault("NODE_ENV", "development" if tui_dev else "production")
    if model:
        env["ZAZA_MODEL"] = model
        env["ZAZA_INFERENCE_MODEL"] = model
    if provider:
        env["ZAZA_TUI_PROVIDER"] = provider
        env["ZAZA_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["ZAZA_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    ...
    if resume_session_id:
        env["ZAZA_TUI_RESUME"] = resume_session_id
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("ZAZA_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'zaza sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like ZAZA isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  zaza setup")
        print()

        from zaza_cli.setup import (
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'zaza setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from zaza_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["ZAZA_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.agent-zaza/data/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["ZAZA_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["ZAZA_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["ZAZA_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from zaza_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    from zaza_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with ZAZA?")
        print()
    ...
    session_dir = get_zaza_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: zaza gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  zaza gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Agent ZAZA'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  zaza gateway")
        ...
        print()
        print("  Or install as a service: zaza gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'zaza whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from zaza_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``zaza model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from zaza_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    ...
    from zaza_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("ZAZA_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'zaza model' for "
                "available providers, or run 'zaza doctor' to diagnose config "
                "issues."
            )
    ...
    from zaza_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from zaza_cli.config import read_raw_config
    ...
    # a leftover OPENAI_BASE_URL in ~/.agent-zaza/data/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.agent-zaza/data/.env if the active provider is not 'custom'.
    ...
    """
    from zaza_cli.config import get_env_value, save_env_value, load_config
...
#
# ZAZA uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `zaza model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `zaza model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _aux_config_menu() -> None:
    ...
    """
    from zaza_cli.config import load_config
    ...
    while True:
        ...
        print("  ZAZA only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``zaza model`` flow, then route aux tasks to them here.
    """
    from zaza_cli.config import load_config
    from zaza_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from zaza_cli.auth import _prompt_model_selection
    from zaza_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from zaza_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from zaza_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from zaza_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from zaza_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from zaza_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from zaza_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `zaza auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from zaza_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from zaza_cli.models import fetch_api_models
...
def _model_flow_minimax_oauth(config, current_model="", args=None):
    ...
    from zaza_cli.models import _PROVIDER_MODELS
...
def _model_flow_google_gemini_cli(_config, current_model=""):
    ...
    Flow:
      ...
      5. Save to ~/.agent-zaza/data/config.yaml.
    """
    from zaza_cli.auth import (
...
def _model_flow_custom(config):
    ...
    """
    from zaza_cli.auth import _save_model_choice, deactivate_provider
    from zaza_cli.config import get_env_value, load_config, save_config
    ...
    from zaza_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `zaza model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from zaza_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from zaza_cli.config import get_env_value, save_env_value, load_config, save_config
    from zaza_cli import azure_detect
    import getpass
    ...
    print("Anthropic-style API endpoints.  ZAZA will probe your")
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from zaza_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from zaza_cli.config import save_env_value, load_config, save_config
    from zaza_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from zaza_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from zaza_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates ZAZA turns to `copilot --acp`.")
    print("  ZAZA currently starts its own ACP subprocess for each request.")
    print("  ZAZA uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set ZAZA_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so zaza knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   ZAZA typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with ZAZA, enable billing on your "
    ...
    if provider_id == "lmstudio":
        from zaza_cli.auth import AuthError
        from zaza_cli.models import fetch_lmstudio_models
        ...
        from zaza_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from zaza_constants import display_zaza_home as _dhh_fn

            print(
                f"    ZAZA will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from zaza_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'zaza model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate ZAZA CLI with a provider."""
    from zaza_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from zaza_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from zaza_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from zaza_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from zaza_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from zaza_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``zaza slack <subcommand>``. Currently supports:
    ...
    if sub in (None, ""):
        ...
        print(
            "usage: zaza slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `zaza slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from zaza_cli.slack_cli import slack_manifest_command
...
def cmd_hooks(args):
    ...
    from zaza_cli.hooks import hooks_command
    hooks_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from zaza_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from zaza_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from zaza_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from zaza_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up ZAZA home directory to a zip file."""
    if getattr(args, "quick", False):
        from zaza_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from zaza_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a ZAZA backup from a zip file."""
    from zaza_cli.backup import run_import
...
def cmd_version(args):
    """Show version."""
    print(f"Agent ZAZA v{__version__} ({__release_date__})")
    ...
    try:
        from zaza_cli.banner import check_for_updates
        from zaza_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Agent ZAZA."""
    _require_tty("uninstall")
    from zaza_cli.uninstall import run_uninstall
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``zaza update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``zaza_cli/web_dist/`` (per vite.config.ts
    outDir: "../zaza_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "zaza_cli" / "web_dist"
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``zaza update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``zaza web``).
    ...
    if r1.returncode != 0:
        print(
            ...
            + ("" if fatal else " (zaza web will not be available)")
        )
        if fatal:
...
def _warn_stale_dashboard_processes() -> None:
    ...
    ``zaza dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``zaza update`` replaces files on disk, the running
    ...
    patterns = [
        "zaza dashboard",
        "zaza_cli.main dashboard",
        "zaza_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "zaza.*dashboard"`) keeps us consistent
            # with `zaza_cli.gateway._scan_gateway_pids` and avoids the
    ...
    print("    kill <pid> && zaza dashboard --port <port> ...")


def _update_via_zip(args):
    """Update Agent ZAZA by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/ZAZA/agent-zaza/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="zaza-update-")
        zip_path = os.path.join(tmp_dir, f"agent-zaza-{branch}.zip")
        ...
        # GitHub ZIPs extract to agent-zaza-<branch>/
        extracted = os.path.join(tmp_dir, f"agent-zaza-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "zaza-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if ZAZA behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes zaza completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but ZAZA couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but ZAZA couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if ZAZA behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `zaza update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/ZAZA/agent-zaza.git",
    "git@github.com:ZAZA/agent-zaza.git",
    "https://github.com/ZAZA/agent-zaza",
    "git@github.com:ZAZA/agent-zaza",
}
OFFICIAL_REPO_URL = "https://github.com/ZAZA/agent-zaza.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_zaza_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_zaza_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official ZAZA repository.")
        print("  This means you may miss updates from ZAZA/agent-zaza.")
        print()
        try:
            response = (
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/ZAZA/agent-zaza.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/ZAZA/agent-zaza.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``zaza update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from zaza_constants import get_default_zaza_root

    default_home = get_default_zaza_root()
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "agent-zaza[matrix]" or "package-name[extra]".
...
class _UpdateOutputStream:
    """Stream wrapper used during ``zaza update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.agent-zaza/data/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``zaza update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``zaza update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.agent-zaza/data/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``zaza update --gateway``) the update is already
    ...
    try:
        ...
        # zaza_cli.config.get_zaza_home to simulate setup failure.
        from zaza_cli.config import get_zaza_home as _get_zaza_home

        logs_dir = _get_zaza_home() / "logs"
...
def _cmd_update_check():
    """Implement ``zaza update --check``: fetch and report without installing."""
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``zaza update`` without requiring a reinstall.  The
    ...
    ``zaza`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/zaza.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v zaza'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/zaza, code at /usr/local/lib/agent-zaza).
    fhs_link = Path("/usr/local/bin/zaza")
    ...
    path_comment = (
        "# Agent ZAZA — ensure /usr/local/bin is on PATH "
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of ZAZA_HOME before running the update.
    ...
    ZAZA_HOME directories.  The ``--backup`` flag on ``zaza update``
    ...
    try:
        from zaza_cli.backup import create_pre_update_backup
    except Exception as exc:
    ...
    # Render path using display_zaza_home so the user sees ~/.agent-zaza/data/...
    try:
        from zaza_constants import get_zaza_home, display_zaza_home
        home = get_zaza_home()
        try:
            display_path = f"{display_zaza_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  zaza import {out_path}")
...
def cmd_update(args):
    """Update Agent ZAZA to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from zaza_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Agent ZAZA")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # the old bytecode (e.g. get_zaza_home added to zaza_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload zaza_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_zaza_home() added since the last release.
        try:
            import importlib
            import zaza_constants as _hc
        ...
        if needs_migration:
            ...
            if gateway_mode:
                ...
                print(
                    "    Run 'zaza config migrate' later to apply any new config/env options."
                )
                response = "n"
            else:
                try:
            ...
            if response in ("", "y", "yes"):
                ...
                if gateway_mode and missing_env:
                    print("  ℹ API keys require manual entry: zaza config migrate")
            else:
                print()
                print("Skipped. Run 'zaza config migrate' later to configure.")
        else:
        ...
        # When running as ``zaza update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_zaza_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            # Discover all agent-zaza-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. agent-zaza-gateway.service or agent-zaza-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    ...
                    print("    Restart manually: zaza gateway run")
                    # Also restart for each profile if needed
                    if len(killed_pids) > 1:
                        print(
                            "    (or: zaza -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy ZAZA gateway unit files are still installed.
        # When both zaza.service (from a pre-rename install) and the
        # current agent-zaza-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `zaza update` surfaces the issue until the user migrates.
        try:
            from zaza_cli.gateway import (
                has_legacy_zaza_units,
                _find_legacy_zaza_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_zaza_units():
                print()
                print("⚠ Legacy ZAZA gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_zaza_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (zaza.service) fight the current")
                print("  agent-zaza-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    zaza gateway migrate-legacy")
                print()
        ...
        print("  zaza model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``zaza -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from zaza_constants import display_zaza_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `zaza profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_zaza_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → zaza -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.agent-zaza/data)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  zaza profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     zaza -p {name} chat")
                else:
        ...
        from zaza_cli.profiles import profile_exists
        ...
        from zaza_cli.profiles import rename_profile
        ...
        from zaza_cli.profiles import export_profile
        ...
        from zaza_cli.profiles import import_profile
...
def cmd_dashboard(args):
    ...
    if "ZAZA_WEB_DIST" not in os.environ:
        if not _build_web_ui(PROJECT_ROOT / "web", fatal=True):
            sys.exit(1)

    from zaza_cli.web_server import start_server

    embedded_chat = args.tui or os.environ.get("ZAZA_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from zaza_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter ZAZA log files."""
    from zaza_cli.logs import tail_log, list_logs
...
def main():
    """Main entry point for zaza CLI."""
    parser = argparse.ArgumentParser(
        prog="zaza",
        description="Agent ZAZA - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    zaza                        Start interactive chat
    zaza chat -q "Hello"        Single query mode
    zaza -c                     Resume the most recent session
    zaza -c "my project"        Resume a session by name (latest in lineage)
    zaza --resume <session_id>  Resume a specific session by ID
    zaza setup                  Run setup wizard
    zaza logout                 Clear stored authentication
    zaza auth add <provider>    Add a pooled credential
    zaza auth list              List pooled credentials
    zaza auth remove <p> <t>    Remove pooled credential by index, id, or label
    zaza auth reset <provider>  Clear exhaustion status for a provider
    zaza model                  Select default model
    zaza fallback [list]        Show fallback provider chain
    zaza fallback add           Add a fallback provider (same picker as `zaza model`)
    zaza fallback remove        Remove a fallback provider from the chain
    zaza config                 View configuration
    zaza config edit            Edit config in $EDITOR
    zaza config set model gpt-4 Set a config value
    zaza gateway                Run messaging gateway
    zaza -s agent-zaza-dev,github-auth
    zaza -w                     Start in isolated git worktree
    zaza gateway install        Install gateway background service
    zaza sessions list          List past sessions
    zaza sessions browse        Interactive session picker
    zaza sessions rename ID T   Rename/title a session
    zaza logs                   View agent.log (last 50 lines)
    zaza logs -f                Follow agent.log in real time
    zaza logs errors            View errors.log
    zaza logs --since 1h        Lines from the last hour
    zaza debug share             Upload debug report for support
    zaza update                 Update to latest version
...
""",
    ...
    # Mirrors `zaza chat --model ... --provider ...` semantics.
    parser.add_argument(
        "-m",
        "--model",
        default=None,
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via ZAZA_INFERENCE_MODEL env var."
        ),
    )
    parser.add_argument(
        "--provider",
        ...
        help=(
            ...
            "Applies to -z/--oneshot and --tui. Also settable via ZAZA_INFERENCE_PROVIDER env var."
        ),
    )
    parser.add_argument(
        "-t",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "without a TTY prompt.  Equivalent to ZAZA_ACCEPT_HOOKS=1 or "
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.agent-zaza/data/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    parser.add_argument(
        "--ignore-rules",
        action="store_true",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Agent ZAZA",
    )
    ...
    chat_parser.add_argument(
        ...
        help=(
            "Auto-approve any unseen shell hooks declared in config.yaml "
            "without a TTY prompt (see also ZAZA_ACCEPT_HOOKS env var and "
    ...
    chat_parser.add_argument(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.agent-zaza/data/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    chat_parser.add_argument(
        "--ignore-rules",
        action="store_true",
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: agent-zaza-cli-toolset)",
    )
    model_parser.add_argument(
    ...
    # =========================================================================
    from zaza_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://agent-zaza.zaza.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `zaza model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy zaza.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy ZAZA gateway unit files "
            "(e.g. zaza.service) left over from older installs. Profile "
            "units (agent-zaza-gateway-<profile>.service) and unrelated "
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Agent ZAZA with an interactive wizard. "
        "Run a specific section: zaza setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        help="(Default on existing installs.) Re-run the full wizard, "
             ...
             "compatibility — a bare 'zaza setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for ZAZA.",
    )
    ...
    slack_manifest.add_argument(
        ...
        help="Write manifest to a file instead of stdout. With no PATH "
             "writes to $ZAZA_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "ZAZA")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for ZAZA CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: agent-zaza-cli-toolset)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser("spotify", help="Authenticate ZAZA with Spotify via PKCE")
    auth_spotify.add_argument("spotify_action", nargs="?", choices=["login", "status", "logout"], default="login")
    auth_spotify.add_argument("--client-id", help="Spotify app client_id (or set ZAZA_SPOTIFY_CLIENT_ID)")
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Agent ZAZA components",
    )
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.agent-zaza/data/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.agent-zaza/data/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Agent ZAZA setup",
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your ZAZA setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Agent ZAZA. Use 'zaza debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    zaza debug share              Upload debug report and print URL
    zaza debug share --lines 500  Include more log lines
    zaza debug share --expire 30  Keep paste for 30 days
    zaza debug share --local      Print report locally (no upload)
    zaza debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'zaza debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up ZAZA home directory to a zip file",
        description="Create a zip archive of your entire ZAZA configuration, "
        "skills, sessions, and data (excludes the agent-zaza codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/zaza-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a ZAZA backup from a zip file",
        description="Extract a previously created ZAZA backup into your "
        "ZAZA home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Agent ZAZA configuration",
    )
    ...
    def cmd_pairing(args):
        from zaza_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.agent-zaza/data/skills/.bundled_manifest) "
            "so future 'zaza update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from zaza_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from zaza_cli.skills_hub import skills_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/zaza-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `zaza plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from zaza_cli.plugins_cmd import plugins_command
    ...
    try:
        from zaza_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_zaza_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_zaza_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_zaza_home()}/memories/\n")
        else:
            from zaza_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'zaza tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # zaza tools disable <name...> [--platform cli]
    ...
    # zaza tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from zaza_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from zaza_cli.tools_config import tools_command
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run ZAZA as an MCP server",
        description=(
            "Manage MCP server connections and run ZAZA as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'zaza mcp add' to connect to a new server, or\n"
            "'zaza mcp serve' to expose ZAZA conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run ZAZA as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_mcp(args):
        from zaza_cli.mcp_config import mcp_command
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_zaza_home() / "sessions"
            ...
            # Launch zaza --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            zaza_bin = shutil.which("zaza")
            if zaza_bin:
                os.execvp(zaza_bin, ["zaza", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "zaza_cli.main", "--resume", selected_id],
                )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to ZAZA",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to ZAZA",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.agent-zaza/data/ (by default a "
        "single restore-point archive is written to ~/.agent-zaza/data/backups/ "
        "before apply; restorable with 'zaza import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from zaza_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Agent ZAZA to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Agent ZAZA",
        description="Remove Agent ZAZA from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Agent ZAZA as an ACP (Agent Client Protocol) server",
        description="Start Agent ZAZA in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    _add_accept_hooks_flag(acp_parser)

    def cmd_acp(args):
        """Launch Agent ZAZA as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated ZAZA instances",
    )
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Agent ZAZA web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `zaza --tui` via PTY/WebSocket). "
            "Alternatively set ZAZA_DASHBOARD_TUI=1."
        ),
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter ZAZA log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    zaza logs                    Show last 50 lines of agent.log
    zaza logs -f                 Follow agent.log in real time
    zaza logs errors             Show last 50 lines of errors.log
    zaza logs gateway -n 100     Show last 100 lines of gateway.log
    zaza logs --level WARNING    Only show WARNING and above
    zaza logs --session abc123   Filter by session ID
    zaza logs --component tools  Only show tool-related lines
    zaza logs --since 1h         Lines from the last hour
    zaza logs --since 30m -f     Follow, starting from 30 min ago
    zaza logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``zaza -c Pokemon Agent Dev`` → ``zaza -c 'Pokemon Agent Dev'``
    ...
    from zaza_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'zaza -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (zaza hooks list, cron
    ...
    if (
        ...
        try:
            from zaza_cli.plugins import discover_plugins
            discover_plugins()
        except Exception:
            logger.debug(
        ...
        # ZAZA v0.5.0 — bootstrap memory + autonomy + offload store.
        # The bootstrap is idempotent and degrades gracefully if any
        # individual subsystem fails to initialise; we never want it to
        # break a session.
        try:
            from agent.bootstrap import bootstrap as _zaza_bootstrap
            _zaza_bootstrap()
        except Exception:
            logger.debug("ZAZA bootstrap failed", exc_info=True)

        # Eagerly import the new tool modules so their registry.register()
        # calls run before the agent enumerates tools.  Both are safe to
        # import — they have no heavy side-effects.
        try:
            import tools.memory_recall_tool  # noqa: F401
            import tools.fetch_tool_output  # noqa: F401
        except Exception:
            logger.debug("ZAZA new-tool registration failed", exc_info=True)

    # Handle top-level --oneshot / -z: single-shot mode, stdout = final
    # response only, nothing else. Bypasses cli.py entirely.
    if getattr(args, "oneshot", None):
        from zaza_cli.oneshot import run_oneshot
