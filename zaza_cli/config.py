"""
Configuration management for Agent ZAZA.

Config files are stored in ~/.agent-zaza/data/ for easy access:
- ~/.agent-zaza/data/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.agent-zaza/data/.env         - API keys and secrets

This module provides:
- zaza config          - Show current configuration
- zaza config edit     - Open config in editor
- zaza config set      - Set a specific value
- zaza config wizard   - Re-run setup wizard
"""
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `zaza plugins enable
    # observability/langfuse` or `zaza tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "ZAZA_LANGFUSE_ENV",
    "ZAZA_LANGFUSE_RELEASE",
    "ZAZA_LANGFUSE_SAMPLE_RATE",
    "ZAZA_LANGFUSE_MAX_CHARS",
    "ZAZA_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
...
from zaza_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("ZAZA_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_zaza_home() / ".managed"
...
def is_managed() -> bool:
    """Check if ZAZA is running in package-manager-managed mode.

    Two signals: the ZAZA_MANAGED env var (set by the systemd service),
    or a .managed marker file in ZAZA_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade agent-zaza"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "zaza update"


def format_managed_message(action: str = "modify this ZAZA installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("ZAZA_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this ZAZA installation is managed by NixOS "
            f"(ZAZA_MANAGED={env_hint}).\n"
            "Edit services.agent-zaza.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this ZAZA installation is managed by Homebrew "
            f"(ZAZA_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade agent-zaza"
        )

    return (
        f"Cannot {action}: this ZAZA installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall ZAZA."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ZAZA_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, zaza_bin
    or None if container mode is not active, we're already inside the
    container, or ZAZA_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("ZAZA_DEV") == "1":
        return None
    ...
    container_mode_file = get_zaza_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "agent-zaza")
    exec_user = info.get("exec_user", "zaza")
    zaza_bin = info.get("zaza_bin", "/data/current-package/bin/zaza")
...
# Re-export from zaza_constants — canonical definition lives there.
from zaza_constants import get_zaza_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_zaza_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_zaza_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the zaza group can
    share state with the gateway service.

    The mode can be overridden via the ZAZA_HOME_MODE environment variable
    (e.g. ZAZA_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse ZAZA_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("ZAZA_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When ZAZA runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("ZAZA_CONTAINER") or os.environ.get("ZAZA_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set ZAZA_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into ZAZA_HOME if the user doesn't have one yet."""
...
def ensure_zaza_home():
    """Ensure ~/.agent-zaza/data directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_zaza_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_zaza_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"ZAZA_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["agent-zaza-cli-toolset"],
    "agent": {
        "max_turns": 90,
        ...
        # the ZAZA-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, ZAZA auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), ZAZA sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when ZAZA
        ...
        # bundled ZAZA image, which drops to the `zaza` user via gosu).
    ...
    "browser": {
        ...
        "camofox": {
            # When true, ZAZA sends a stable profile-scoped userId to Camofox
    ...
    "checkpoints": {
        ...
        # ~/.agent-zaza/data/checkpoints/ (one per cd'd working directory). Field
        # reports put the typical offender at 1000+ repos / ~12 GB. When
        # auto_prune is on, zaza sweeps at startup (at most once per
        # min_interval_hours) and deletes:
        #   * orphan repos: ZAZA_WORKDIR no longer exists on disk
    ...
    # single read_file page exceeds these limits, ZAZA truncates the
    ...
    "display": {
        ...
        # When true, `zaza --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `zaza -c` muscle memory.  Default off so existing
        # users aren't surprised.  ZAZA_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/zaza`. Per-platform overrides go under
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.agent-zaza/data/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # always goes to ~/.agent-zaza/data/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${ZAZA_SKILL_DIR} and ${ZAZA_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `zaza curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # to use ZAZA' main auxiliary client resolution.
        "auxiliary": {
            "provider": None,
            "model": None,
        },
    ...
    # This section is only needed for zaza-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Agent ZAZA*" header.
    ...
    # stored approval from ~/.agent-zaza/data/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or ZAZA_ACCEPT_HOOKS=1.
    ...
    "cron": {
        ...
        # Also overridable via ZAZA_CRON_MAX_PARALLEL env var.
    ...
    "code_execution": {
        ...
        #     agent-zaza's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.agent-zaza/data/logs/.
    ...
    # update model picker lists without shipping a agent-zaza release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://agent-zaza.zaza.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `zaza model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.agent-zaza/data/state.db.
    ...
    "sessions": {
        ...
        # default of ``zaza sessions prune``.
        "retention_days": 90,
    ...
    # ``zaza update`` behaviour.
    "updates": {
        # Run a full ``zaza backup``-style zip of ZAZA_HOME before every
        # ``zaza update``.  Backups land in ``<ZAZA_HOME>/backups/`` and
        # can be restored with ``zaza import <path>``.  Off by default —
        # on large ZAZA_HOME directories the zip can add minutes to every
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'zaza model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Nous Subscribers only, used to derive vendor hosts, e.g. zaza.com -> firecrawl-gateway.zaza.com",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the ZAZA auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @zaza:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. ZAZA_BOT)",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'agent-zaza' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote ZAZA API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote ZAZA API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote ZAZA API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # ZAZA_TOOL_PROGRESS and ZAZA_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.zaza.config`` entries, then checks
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older ZAZA versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``zaza_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'zaza setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — ZAZA won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'zaza doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("ZAZA_HOME", "~/.agent-zaza/data")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("ZAZA_TOOL_PROGRESS")
            old_mode = get_env_value("ZAZA_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from ZAZA_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from ZAZA_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("ZAZA_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from ZAZA_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$ZAZA_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_zaza_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `zaza plugins enable <name>` to activate."
                    )
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: zaza config set <key> <value>")
    ...
    # metadata.zaza.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _zaza_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.agent-zaza/data/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.agent-zaza/data/config.yaml.
    ...
    (which change ``ZAZA_HOME`` and therefore ``get_config_path()``)
    don't collide.
    """
    ensure_zaza_home()
    config_path = get_config_path()
...
#   openai-codex (OAuth — zaza auth) — OpenAI Codex
#   nous         (OAuth — zaza auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.agent-zaza/data/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.agent-zaza/data/.env.
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real ZAZA env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.agent-zaza/data/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.agent-zaza/data/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.agent-zaza/data/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.agent-zaza/data/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to ZAZA — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known ZAZA vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.agent-zaza/data/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ ZAZA Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  zaza config edit     # Edit config file", Colors.DIM))
    print(color("  zaza config set <key> <value>", Colors.DIM))
    print(color("  zaza setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: zaza config set <key> <value>")
            print()
            print("Examples:")
            print("  zaza config set model anthropic/claude-sonnet-4")
            print("  zaza config set terminal.backend docker")
            print("  zaza config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'zaza config migrate' to add them")
        ...
        print("  zaza config           Show current configuration")
        print("  zaza config edit      Open config in editor")
        print("  zaza config set <key> <value>   Set a config value")
        print("  zaza config check     Check for missing/outdated config")
        print("  zaza config migrate   Update config with new options")
        print("  zaza config path      Show config file path")
        print("  zaza config env-path  Show .env file path")
        sys.exit(1)
