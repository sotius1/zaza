...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with agent-zaza;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.agent-zaza/data/plugins/<name>/``
3. **Project plugins** – ``./.agent-zaza/data/plugins/<name>/`` (opt-in via
   ``ZAZA_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``zaza_agent.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "zaza_agent.plugins"

_NS_PARENT = "zaza_plugins"
...
class PluginManifest:
    ...
    # lookups and by ``zaza plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``zaza honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``zaza <subcommand>``
    ...
    def register_skill(
        ...
        ``~/.agent-zaza/data/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        #
        # Repo-shipped plugins live next to zaza_cli/. Two layouts are
        ...
        # 2. User plugins (~/.agent-zaza/data/plugins/)
        user_dir = get_zaza_home() / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 3. Project plugins (./.agent-zaza/data/plugins/)
        if _env_enabled("ZAZA_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".agent-zaza/data" / "plugins"
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with zaza and must
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `zaza plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``zaza_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``zaza_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``zaza tools`` TUI so plugin-provided toolsets appear
