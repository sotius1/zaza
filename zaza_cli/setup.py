"""
Interactive setup wizard for Agent ZAZA.
...
Config files are stored in ~/.agent-zaza/data/ for easy access.
"""
...
from zaza_cli.nous_subscription import get_nous_subscription_features
...
_DOCS_BASE = "https://agent-zaza.zaza.com/docs"
...
# display_zaza_home imported lazily at call sites (stale-module safety during zaza update)
...
from zaza_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ ZAZA Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure ZAZA using environment variables or config commands:")
    print_info("  zaza config set model.provider custom")
    print_info("  zaza config set model.base_url http://localhost:8080/v1")
    print_info("  zaza config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'zaza setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from zaza_cli.curses_ui import curses_radiolist
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from zaza_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'zaza setup')")


def _print_setup_summary(config: dict, zaza_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'zaza setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from zaza_cli.plugins import _ensure_plugins_discovered
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'zaza setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'zaza setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'zaza setup terminal'"))
    ...
    # Spotify (OAuth via zaza auth spotify — check auth.json, not env vars)
    try:
        from zaza_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'zaza setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {zaza_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('zaza setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('zaza setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('zaza setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('zaza setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('zaza setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('zaza config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('zaza config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('zaza config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('zaza', Colors.GREEN)}              Start chatting")
    print(f"   {color('zaza gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('zaza doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `zaza tools` and `zaza setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``zaza model``)
    ...
    provider added to ``zaza model`` is automatically available here.
    ...
    # Delegate to the shared zaza model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from zaza_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: zaza model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from zaza_cli.auth_commands import auth_add_command
            ...
            print_info(
                "ZAZA can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one ZAZA can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'zaza setup' or configure AUXILIARY_VISION_* settings")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.agent-zaza/data/.env."
            )
        ...
        if not existing:
            ...
            if api_key:
                ...
                print_warning(
                    ...
                    f"zaza setup model or {_dhh()}/.env to use xAI TTS. "
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'zaza setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where ZAZA runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using ZAZA via Telegram/Discord, this is where")
        print_info(
        ...
        print_info("Requires the optional SDK: pip install 'agent-zaza[vercel]'")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'agent-zaza[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    save_env_value("ZAZA_MAX_ITERATIONS", "90")
    ...
    print_info("  Run `zaza setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    # ── Max Iterations ──
    current_max = get_env_value("ZAZA_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("ZAZA_MAX_ITERATIONS", str(max_iter))
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where ZAZA delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `zaza update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://agent-zaza.zaza.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under ZAZA_HOME, and print
    ...
    """
    try:
        from zaza_cli.slack_cli import _build_full_manifest
        from zaza_constants import get_zaza_home

        manifest = _build_full_manifest(
            bot_name="ZAZA",
            bot_description="Your ZAZA agent on Slack",
        )
        ...
        print_info(
            "   Re-run `zaza slack manifest --write` anytime to refresh after "
            "ZAZA adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where ZAZA delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where ZAZA delivers cron job results and notifications.")
...
def _setup_whatsapp():
    ...
    print_info("Requires Node.js. Run 'zaza whatsapp' for guided setup.")
    print()
    if prompt_yes_no("Enable WhatsApp now?", True):
        save_env_value("WHATSAPP_ENABLED", "true")
        print_success("WhatsApp enabled")
        print_info("Run 'zaza whatsapp' to choose your mode (separate bot number")
        print_info("or personal self-chat) and pair via QR code.")


def _setup_weixin():
    """Configure Weixin (personal WeChat) via iLink Bot API QR login."""
    from zaza_cli.gateway import _setup_weixin as _gateway_setup_weixin
    _gateway_setup_weixin()


def _setup_signal():
    """Configure Signal via gateway setup."""
    from zaza_cli.gateway import _setup_signal as _gateway_setup_signal
    _gateway_setup_signal()


def _setup_email():
    """Configure Email via gateway setup."""
    from zaza_cli.gateway import _setup_email as _gateway_setup_email
    _gateway_setup_email()


def _setup_sms():
    """Configure SMS (Twilio) via gateway setup."""
    from zaza_cli.gateway import _setup_sms as _gateway_setup_sms
    _gateway_setup_sms()


def _setup_dingtalk():
    """Configure DingTalk via gateway setup."""
    from zaza_cli.gateway import _setup_dingtalk as _gateway_setup_dingtalk
    _gateway_setup_dingtalk()


def _setup_feishu():
    """Configure Feishu / Lark via gateway setup."""
    from zaza_cli.gateway import _setup_feishu as _gateway_setup_feishu
    _gateway_setup_feishu()


def _setup_yuanbao():
    """Configure Yuanbao via gateway setup."""
    from zaza_cli.gateway import _setup_yuanbao as _gateway_setup_yuanbao
    _gateway_setup_yuanbao()


def _setup_wecom():
    """Configure WeCom (Enterprise WeChat) via gateway setup."""
    from zaza_cli.gateway import _setup_wecom as _gateway_setup_wecom
    _gateway_setup_wecom()


def _setup_wecom_callback():
    """Configure WeCom Callback (self-built app) via gateway setup."""
    from zaza_cli.gateway import _setup_wecom_callback as _gw_setup
    _gw_setup()
...
def _setup_bluebubbles():
    ...
    print_info("Connects ZAZA to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from zaza_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://agent-zaza.zaza.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://agent-zaza.zaza.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  zaza config edit")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with ZAZA from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'zaza setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     zaza config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_zaza_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: zaza gateway install")
            else:
                print_info("  You can install later: zaza gateway install")
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo zaza gateway install --system")
                print_info("  Or run in foreground:  zaza gateway")
        else:
            from zaza_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   zaza gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   zaza gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `zaza setup tools` and `zaza tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``zaza_cli.auth`` — lists every supported
    ...
    """
    try:
        from zaza_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from zaza_cli.config
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_zaza migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_zaza", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and ZAZA.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure ZAZA to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point ZAZA at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point ZAZA at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point ZAZA at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point ZAZA at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to ZAZA equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing ZAZA config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in ZAZA.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ ZAZA's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(zaza_home: Path) -> bool:
    ...
    print_info("ZAZA can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: zaza claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: zaza claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing ZAZA configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing ZAZA config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in ZAZA (use zaza claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      zaza setup           — full or quick (auto-detected)
      zaza setup model     — just model/provider
      zaza setup tts       — just text-to-speech
      zaza setup terminal  — just terminal backend
      zaza setup gateway   — just messaging platforms
      zaza setup tools     — just tool configuration
      zaza setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ ZAZA Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Agent ZAZA Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Agent ZAZA installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, zaza_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have ZAZA configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'zaza setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(zaza_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up ZAZA?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, zaza_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {zaza_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'zaza config edit'")
    ...
    save_config(config)
    _print_setup_summary(config, zaza_home)

    _offer_launch_chat()


def _resolve_zaza_chat_argv() -> Optional[list[str]]:
    """Resolve argv for launching ``zaza chat`` in a fresh process."""
    zaza_bin = shutil.which("zaza")
    if zaza_bin:
        return [zaza_bin, "chat"]

    try:
        if importlib.util.find_spec("zaza_cli") is not None:
            return [sys.executable, "-m", "zaza_cli.main", "chat"]
    except Exception:
        pass
...
def _offer_launch_chat():
    ...
    print()
    if not prompt_yes_no("Launch zaza chat now?", True):
        return

    chat_argv = _resolve_zaza_chat_argv()
    if not chat_argv:
        print_info("Could not relaunch ZAZA automatically. Run 'zaza chat' manually.")
        return

    os.execvp(chat_argv[0], chat_argv)


def _run_first_time_quick_setup(config: dict, zaza_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'zaza setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    zaza setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  zaza setup gateway")
    print()
...
def _run_quick_setup(config: dict, zaza_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'zaza setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect ZAZA to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'zaza setup gateway'.")
