"""agent-zaza webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    agent-zaza webhook subscribe <name> [options]
    agent-zaza webhook list
    agent-zaza webhook remove <name>
    agent-zaza webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.agent-zaza/data/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _zaza_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
    _dhh = display_zaza_home()
    return f"""
  Webhook platform is not enabled. To set it up:

  1. Run the gateway setup wizard:
     agent-zaza gateway setup
  ...
  Then start the gateway: agent-zaza gateway run
"""
...
def webhook_command(args):
    """Entry point for 'agent-zaza webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: agent-zaza webhook {subscribe|list|remove|test}")
        print("Run 'agent-zaza webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (agent-zaza gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: agent-zaza webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from agent-zaza webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (agent-zaza gateway run)")
