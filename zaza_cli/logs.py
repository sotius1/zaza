"""``zaza logs`` — view and filter ZAZA log files.

Supports tailing, following, session filtering, level filtering,
component filtering, and relative time ranges.  All log files live
under ``~/.agent-zaza/data/logs/``.

Usage examples::

    zaza logs                    # last 50 lines of agent.log
    zaza logs -f                 # follow agent.log in real time
    zaza logs errors             # last 50 lines of errors.log
    zaza logs gateway -n 100    # last 100 lines of gateway.log
    zaza logs --level WARNING    # only WARNING+ lines
    zaza logs --session abc123   # filter by session ID substring
    zaza logs --component tools  # only tool-related lines
    zaza logs --since 1h         # lines from the last hour
    zaza logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_zaza_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when ZAZA runs — try 'zaza chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_zaza_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_zaza_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_zaza_home()}/logs/")
        return

    print(f"Log files in {display_zaza_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'zaza chat' to generate logs)")
