...
- ``zaza_cli.providers``        -- canonical provider identity + overlays
- ``zaza_cli.model_normalize``  -- per-provider name formatting
...
_ZAZA_MODEL_WARNING = (
    "ZAZA ZAZA 3 & 4 models are NOT agentic and are not designed "
    "for use with Agent ZAZA. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real ZAZA ZAZA 3 / ZAZA 4 chat families.
# The previous substring check (`"zaza" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``zaza-brain:qwen3-14b-ctx16k`` that just
# happen to carry "zaza" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   ZAZA/ZAZA-3-Llama-3.1-70B, zaza-4-405b, openrouter/zaza3:70b
# Negative examples it must NOT match:
#   zaza-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_ZAZA_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])zaza[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_zaza_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous ZAZA 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_NOUS_ZAZA_NON_AGENTIC_RE.search(model_name))


def _check_zaza_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous ZAZA 3/4 chat model."""
    if is_nous_zaza_non_agentic(model_name):
        return _ZAZA_MODEL_WARNING
    return ""
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from zaza_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'zaza model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'zaza doctor' — config issues detected:"
    ...
    # Mirrors the same logic in zaza_cli.runtime_provider.resolve_runtime_provider;
    ...
    zaza_warn = _check_zaza_model_warning(new_model)
    if zaza_warn:
        warnings.append(zaza_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from zaza_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from zaza_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by zaza provider ID
    ...
    # --- 1. Check ZAZA-mapped providers ---
    for zaza_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(zaza_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (ZAZA_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from zaza_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and zaza_id in store.get("credential_pool", {}):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # show up in the picker without requiring a ZAZA release.
        model_ids = curated.get(zaza_id, [])
        if zaza_id in _MODELS_DEV_PREFERRED:
            model_ids = _merge_with_models_dev(zaza_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check ZAZA-only providers (nous, openai-codex, copilot, opencode-go) ---
    from zaza_cli.providers import ZAZA_OVERLAYS
    from zaza_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → ZAZA provider ID.
    # ZAZA_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use ZAZA IDs ("copilot").
    _mdev_to_zaza = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in ZAZA_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve ZAZA slug — e.g. "github-copilot" → "copilot"
        zaza_slug = _mdev_to_zaza.get(pid, pid)
        if zaza_slug.lower() in seen_slugs:
            continue
        ...
        if not has_creds:
            try:
                ...
                if store and (
                    pid in providers_store or zaza_slug in providers_store
                    or pid in pool_store or zaza_slug in pool_store
                ):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(zaza_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", zaza_slug, exc)
        ...
        # configured.
        if not has_creds and zaza_slug == "anthropic":
            try:
                ...
                if (zaza_creds and zaza_creds.get("accessToken")) or \
        ...
        if zaza_slug in {"copilot", "copilot-acp"}:
            model_ids = provider_model_ids(zaza_slug)
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                model_ids = _ids if _ids is not None else (curated.get(zaza_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(zaza_slug, []) or curated.get(pid, [])
        else:
            # Use curated list — look up by ZAZA slug, fall back to overlay key
            model_ids = curated.get(zaza_slug, []) or curated.get(pid, [])
            # Merge with models.dev for preferred providers (same rationale as above).
            if zaza_slug in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(zaza_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": zaza_slug,
            "name": get_label(zaza_slug),
            "is_current": zaza_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "zaza",
        ...
        seen_slugs.add(zaza_slug.lower())
        _record_builtin_endpoint(zaza_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or ZAZA_OVERLAYS (keeps /model in sync
    # with `zaza model`).
    try:
        from zaza_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is ZAZA's canonical write key (matches
            ...
            # ZAZA writes ``models:`` as a dict keyed by model id
            # (see zaza_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # ZAZA's own writer uses; a hyphen variant is accepted
            ...
            # active model. ZAZA's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, zaza_cli/config.py) already consume that dict.
