"""
Dump command for zaza CLI.

Outputs a compact, plain-text summary of the user's ZAZA setup
...
from zaza_cli.config import get_zaza_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``zaza dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from zaza_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(zaza_home: Path) -> int:
...
def _cron_summary(zaza_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = zaza_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"zaza_home:      {display_zaza_home()}")
    ...
    toolsets = config.get("toolsets", ["agent-zaza-cli-toolset"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(zaza_home)}")
    lines.append(f"  skills:             {_count_skills(zaza_home)}")
