"""Slash command definitions and autocomplete for the ZAZA CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of ZAZA config/state", "Session",
    ...
    CommandDef("busy", "Control what Enter does while ZAZA is working", "Configuration",
    ...
    CommandDef("update", "Update Agent ZAZA to the latest version", "Info",
               gateway_only=True),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`zaza_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/zaza`` subcommand mapping, and
    ...
    """
    try:
        from zaza_cli.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``zaza skills
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/zaza <verb>`` subcommand.
    ...
    avoidance. ``/zaza`` is always reserved as the first entry so the
    legacy ``/zaza <subcommand>`` form keeps working for anything that
    ...
    # Reserve /zaza as the catch-all top-level command.
    entries.append(("zaza", "Talk to ZAZA or run a subcommand", "[subcommand] [args]"))
    seen.add("zaza")
...
def slack_app_manifest(request_url: str = "https://agent-zaza.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /zaza handler.

    Maps both canonical names and aliases so /zaza bg do stuff works
    the same as /zaza background do stuff.

    Plugin-registered slash commands are included so ``/zaza <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from zaza_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
