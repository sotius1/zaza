"""
Single source of truth for provider identity in Agent ZAZA.
...
2. **ZAZA overlays** — transport type, auth patterns, aggregator flags,
...
# -- ZAZA overlay ----------------------------------------------------------
# ZAZA-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class ZAZAOverlay:
    """ZAZA-specific provider metadata layered on top of models.dev."""
...
ZAZA_OVERLAYS: Dict[str, ZAZAOverlay] = {
    "openrouter": ZAZAOverlay(
    ...
    "nous": ZAZAOverlay(
        ...
        base_url_override="https://inference-api.zaza.com/v1",
    ),
    "openai-codex": ZAZAOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "qwen-oauth": ZAZAOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="ZAZA_QWEN_BASE_URL",
    ),
    "google-gemini-cli": ZAZAOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": ZAZAOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": ZAZAOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": ZAZAOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": ZAZAOverlay(
    ...
    "minimax-oauth": ZAZAOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": ZAZAOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": ZAZAOverlay(
        transport="openai_chat",
    ...
    "alibaba-coding-plan": ZAZAOverlay(
        transport="openai_chat",
    ...
    "opencode": ZAZAOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": ZAZAOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "zaza", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. ZAZA overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + ZAZA overlay
    ...
    overlay = ZAZA_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + zaza extra
    ...
    if overlay is not None:
        # ZAZA-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in ZAZA_OVERLAYS
    if provider == "bedrock":
