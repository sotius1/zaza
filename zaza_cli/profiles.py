"""
Profile management for multiple isolated ZAZA instances.

Each profile is a fully independent ZAZA_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.agent-zaza/data/profiles/<name>/`` by default.

The "default" profile is ``~/.agent-zaza/data`` itself — backward compatible,
zero migration needed.

Usage::

    zaza profile create coder          # fresh profile + bundled skills
    zaza profile create coder --clone  # also copy config, .env, SOUL.md
    zaza profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    agent-zaza -p coder chat                 # or via flag
    zaza profile use coder             # set as sticky default
    zaza profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See zaza_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    ``~/.agent-zaza/data`` contains ``profiles/<name>/`` for sibling named profiles.
...
# Directories/files to exclude when exporting the default (~/.agent-zaza/data) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "agent-zaza",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "zaza", "default", "test", "tmp", "root", "sudo",
})

# ZAZA subcommands that cannot be used as profile names/aliases
_ZAZA_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the zaza root, NOT to the current ZAZA_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where ZAZA_HOME points outside
    ``~/.agent-zaza/data``, profiles live under ``ZAZA_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_zaza_home() / "profiles"


def _get_default_zaza_home() -> Path:
    """Return the default (pre-profile) ZAZA_HOME path.

    In standard deployments this is ``~/.agent-zaza/data``.
    In Docker/custom deployments where ZAZA_HOME is outside ``~/.agent-zaza/data``
    (e.g. ``/opt/data``), returns ZAZA_HOME directly.
    """
    from zaza_constants import get_default_zaza_root
    return get_default_zaza_root()


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_zaza_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name == "default":
        return  # special alias for ~/.agent-zaza/data
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its ZAZA_HOME directory."""
    if name == "default":
        return _get_default_zaza_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, zaza subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _ZAZA_SUBCOMMANDS:
        return f"'{name}' conflicts with a zaza subcommand"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_zaza_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.agent-zaza/data)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.agent-zaza/data/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches ZAZA_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "ZAZA_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.agent-zaza/data).\n"
            "To remove everything, use: zaza uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set ZAZA_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("ZAZA_HOME")
    try:
        os.environ["ZAZA_HOME"] = str(profile_dir)
        from zaza_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["ZAZA_HOME"] = old_home
        elif "ZAZA_HOME" in os.environ:
            del os.environ["ZAZA_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.agent-zaza/data/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: zaza profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from ZAZA_HOME.

    Returns ``"default"`` if ZAZA_HOME is not set or points to ``~/.agent-zaza/data``.
    Returns the profile name if ZAZA_HOME points into ``~/.agent-zaza/data/profiles/<name>``.
    Returns ``"custom"`` if ZAZA_HOME is set to an unrecognized path.
    """
    from zaza_constants import get_zaza_home
    zaza_home = get_zaza_home()
    resolved = zaza_home.resolve()

    default_resolved = _get_default_zaza_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.agent-zaza/data itself — its parent is ~/ and its
        # directory name is ".agent-zaza/data", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: zaza profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.agent-zaza/data itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.agent-zaza/data). "
            "Specify a different name: zaza profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="zaza_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"zaza.{old_name}"
    new_host = f"zaza.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_zaza_home() / "honcho.json",
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for zaza profile names."""
    return '''# Agent ZAZA profile completion
# Add to ~/.bashrc: eval "$(zaza completion bash)"

_zaza_profiles() {
    local profiles_dir="$HOME/.agent-zaza/data/profiles"
...
_zaza_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_zaza_profiles)" -- "$cur"))
        return
    fi
...
complete -F _zaza_completion zaza
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for zaza profile names."""
    return '''#compdef zaza
# Agent ZAZA profile completion
# Add to ~/.zshrc: eval "$(zaza completion zsh)"

_zaza() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.agent-zaza/data/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.agent-zaza/data/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a ZAZA_HOME path string.

    Called early in the CLI entry point, before any zaza modules
    are imported, to set the ZAZA_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: zaza profile create {profile_name}"
        )
