"""
ZAZA MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (ZAZA-specific extra)

Usage:
    zaza mcp serve
    zaza mcp serve --verbose
...
logger = logging.getLogger("zaza.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using ZAZA_HOME."""
    try:
        from zaza_constants import get_zaza_home
        return get_zaza_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("ZAZA_HOME", Path.home() / ".agent-zaza/data")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from zaza_constants import get_zaza_home
        directory_file = get_zaza_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("ZAZA_HOME", Path.home() / ".agent-zaza/data")
...
class EventBridge:
    ...
    This is the ZAZA equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from zaza_constants import get_zaza_home
            db_file = get_zaza_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("ZAZA_HOME", Path.home() / ".agent-zaza/data")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the ZAZA MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "zaza",
        instructions=(
            "Agent ZAZA messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the ZAZA MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
