...
## 5. GODMODE FAST — ZAZA 4 405B (Zero Refusal)

**Model:** `zaza/zaza-4-405b`
**Strategy:** ZAZA models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Agent ZAZA

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.agent-zaza/data/config.yaml`:
