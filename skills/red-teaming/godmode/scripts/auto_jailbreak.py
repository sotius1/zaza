...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ZAZA_HOME", os.path.expanduser("~/.agent-zaza/data")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ZAZA_HOME", Path.home() / ".agent-zaza/data")) / "skills" / "red-teaming" / "godmode"
...
ZAZA_HOME = Path(os.getenv("ZAZA_HOME", Path.home() / ".agent-zaza/data"))
CONFIG_PATH = ZAZA_HOME / "config.yaml"
PREFILL_PATH = ZAZA_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/ZAZA models — already uncensored, just needs clean prompt
    "zaza": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "zaza" in model_lower or "nous" in model_lower:
        return "zaza"
...
def _get_current_model() -> tuple:
    """Read current model and provider from ZAZA config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.agent-zaza/data/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart ZAZA for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart ZAZA for changes to take effect.")
