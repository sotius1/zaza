...
ZAZA_HOME_DIR="${ZAZA_HOME:-$HOME/.zaza}"
ZAZA_CFG="${ZAZA_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure ZAZA config has twozero_td MCP entry ──
if [[ ! -f "$ZAZA_CFG" ]]; then
    echo -e " ${FAIL} ZAZA config not found at ${ZAZA_CFG}"
    manual_steps+=("Create ${ZAZA_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$ZAZA_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in ZAZA config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to ZAZA config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${ZAZA_CFG} manually"); }
    manual_steps+=("Restart ZAZA session to pick up config change")
fi
