...
## 5. ZAZA Configuration

### Config location

`$ZAZA_HOME/config.yaml` (defaults to `~/.agent-zaza/data/config.yaml` when `ZAZA_HOME` is unset)
...
Restart the ZAZA session for changes to take effect. The MCP connection is
