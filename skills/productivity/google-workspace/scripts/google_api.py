#!/usr/bin/env python3
"""Google Workspace API CLI for Agent ZAZA.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing ZAZA-facing JSON contract and falls back to the Python client
...
from _zaza_home import get_zaza_home
...
def _gws_binary() -> str | None:
    override = os.getenv("ZAZA_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Agent ZAZA")
