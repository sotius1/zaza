#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Agent ZAZA.
...
# Ensure sibling modules (_zaza_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _zaza_home import display_zaza_home, get_zaza_home

ZAZA_HOME = get_zaza_home()
TOKEN_PATH = ZAZA_HOME / "google_token.json"
CLIENT_SECRET_PATH = ZAZA_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = ZAZA_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same ZAZA profile to refresh consent."
    )
...
def install_deps():
    ...
    try:
        ...
        print("  pip install 'agent-zaza[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to ZAZA home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_zaza_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for ZAZA")
