"""Centralized logging setup for Agent ZAZA.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.agent-zaza/data/logs/`` (profile-aware via ``get_zaza_home()``).
...
from zaza_constants import get_config_path, get_zaza_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_zaza_session_injector", False):
    ...
    _session_record_factory._zaza_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``zaza logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway",),
...
def setup_logging(
    *,
    ...
    """Configure the ZAZA logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    zaza_home
        Override for the ZAZA home directory.  Falls back to
        ``get_zaza_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_zaza_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._zaza_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the zaza group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from zaza_cli.config import is_managed
