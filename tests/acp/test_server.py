"""Tests for acp_adapter.server — ZAZAACPAgent ACP server."""
...
from acp_adapter.server import ZAZAACPAgent, ZAZA_VERSION
...
def agent(mock_manager):
    """ZAZAACPAgent backed by a mock session manager."""
    return ZAZAACPAgent(session_manager=mock_manager)
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "agent-zaza"
        assert resp.agent_info.version == ZAZA_VERSION
...
class TestSessionOps:
    ...
    async def test_new_session_returns_model_state(self):
        ...
        acp_agent = ZAZAACPAgent(session_manager=manager)

        with patch(
            "zaza_cli.models.curated_models_for_provider",
...
class TestSessionConfiguration:
    ...
    async def test_set_session_model_accepts_provider_prefixed_choice(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("zaza_cli.config.load_config", lambda: {
...
class TestRegisterSessionMcpServers:
    ...
    async def test_registers_stdio_servers(self, agent, mock_manager):
        ...
        state.agent.enabled_toolsets = ["agent-zaza-acp"]
    ...
    async def test_refreshes_agent_tool_surface(self, agent, mock_manager):
        ...
        mock_defs.assert_called_once_with(
            enabled_toolsets=["agent-zaza-acp", "mcp-srv"],
            disabled_toolsets=None,
            quiet_mode=True,
        )
        assert state.agent.enabled_toolsets == ["agent-zaza-acp", "mcp-srv"]
