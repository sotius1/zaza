...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.agent-zaza/data."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /zaza, /btw, /stop,
        ...
        for expected in ("/zaza", "/btw", "/stop", "/model", "/help"):
...
class TestSlashCommands:
    ...
    # instead of as /zaza subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /zaza btw."""
        command = {
            "command": "/btw",
    ...
    @pytest.mark.asyncio
    async def test_legacy_zaza_prefix_still_works(self, adapter):
        """Backward compat: /zaza btw foo must still route to /btw foo.

        Old workspace manifests only declared /zaza as the single slash.
    ...
    @pytest.mark.asyncio
    async def test_legacy_zaza_freeform_question(self, adapter):
        """/zaza <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/zaza",
