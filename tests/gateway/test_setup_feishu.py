"""Tests for _setup_feishu() in zaza_cli/gateway.py.
...
def _run_setup_feishu(
    *,
    ...
    with patch("zaza_cli.gateway.save_env_value", side_effect=mock_save), \
         patch("zaza_cli.gateway.get_env_value", side_effect=mock_get), \
         patch("zaza_cli.gateway.prompt_yes_no", side_effect=prompt_yes_no_responses), \
         patch("zaza_cli.gateway.prompt_choice", side_effect=prompt_choice_responses), \
         patch("zaza_cli.gateway.prompt", side_effect=prompt_responses), \
         patch("zaza_cli.gateway.print_info"), \
         patch("zaza_cli.gateway.print_success"), \
         patch("zaza_cli.gateway.print_warning"), \
         patch("zaza_cli.gateway.print_error"), \
         patch("zaza_cli.gateway.color", side_effect=lambda t, c: t), \
         patch("gateway.platforms.feishu.qr_register", return_value=qr_result):

        from zaza_cli.gateway import _setup_feishu
        _setup_feishu()
