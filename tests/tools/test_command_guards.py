...
def _clean_state():
    ...
    saved = {}
    for k in ("ZAZA_INTERACTIVE", "ZAZA_GATEWAY_SESSION", "ZAZA_EXEC_ASK", "ZAZA_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["ZAZA_INTERACTIVE"] = "1"
        session_key = os.getenv("ZAZA_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No ZAZA_INTERACTIVE or ZAZA_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
