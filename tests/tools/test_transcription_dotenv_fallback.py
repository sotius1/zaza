...
``~/.agent-zaza/data/.env`` entries. These tests confirm each STT provider now
...
class TestProviderSelectionGate:
    ...
    ``os.environ`` a user with keys in ``~/.agent-zaza/data/.env`` would be told
    ...
    def test_explicit_groq_sees_dotenv(self):
        ...
        with patch.object(tt, "_HAS_FASTER_WHISPER", False), \
             ...
             patch("zaza_cli.config.load_env",
...
class TestEndToEndRegressionGuard:
    """End-to-end probe: patch ``zaza_cli.config.load_env`` to simulate
    ``~/.agent-zaza/data/.env`` carrying the key while ``os.environ`` does not.
    ...
    def test_xai_key_only_in_dotenv_before_fix(self, monkeypatch):
        ...
        with patch("zaza_cli.config.load_env",
                   return_value={"XAI_API_KEY": "dotenv-secret"}):
