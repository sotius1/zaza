...
def test_zaza_provider_subclass_exists():
    """ZAZAMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _ZAZA_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _ZAZA_PROVIDER_CLS is not None
    assert issubclass(_ZAZA_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_zaza_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns ZAZAMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _ZAZA_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("ZAZA_HOME", str(tmp_path))
    ...
    assert _ZAZA_PROVIDER_CLS is not None
    assert isinstance(provider, _ZAZA_PROVIDER_CLS)
    assert provider._zaza_server_name == "srv"
