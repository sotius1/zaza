...
class TestWriteDenyExactPaths:
    ...
    def test_zaza_env(self):
        # ``.env`` under the active ZAZA_HOME (profile-aware, not just
        # ``~/.agent-zaza/data``) must be write-denied. The hermetic test conftest
        # points ZAZA_HOME at a tempdir — resolve via get_zaza_home()
        # to match the denylist.
        from zaza_constants import get_zaza_home
        path = str(get_zaza_home() / ".env")
        assert _is_write_denied(path) is True
...
class TestWriteAllowed:
    ...
    def test_zaza_config_not_env(self):
        path = os.path.join(str(Path.home()), ".agent-zaza/data", "config.yaml")
