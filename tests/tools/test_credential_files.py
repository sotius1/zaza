...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        zaza_home.mkdir()
        (zaza_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"ZAZA_HOME": str(zaza_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(zaza_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.agent-zaza/data/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        zaza_home = tmp_path / ".agent-zaza/data"
        zaza_home.mkdir()
        (zaza_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        zaza_home = tmp_path / ".agent-zaza/data"
        zaza_home.mkdir()
        (zaza_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        zaza_home = tmp_path / ".agent-zaza/data"
        zaza_home.mkdir()
        (zaza_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.agent-zaza/data/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        zaza_home = tmp_path / ".agent-zaza/data"
        (zaza_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"ZAZA_HOME": str(zaza_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.zaza")

        assert mounts[0]["container_path"] == "/home/user/.agent-zaza/data/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.agent-zaza/data/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.agent-zaza/data/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape ZAZA_HOME."""
        monkeypatch.setenv("ZAZA_HOME", str(tmp_path / ".agent-zaza/data"))
        (tmp_path / ".agent-zaza/data").mkdir()

        # Create a sensitive file one level above zaza_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside zaza_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside ZAZA_HOME must still be registered."""
        zaza_home = tmp_path / ".agent-zaza/data"
        zaza_home.mkdir()
        monkeypatch.setenv("ZAZA_HOME", str(zaza_home))
        (zaza_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_zaza_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of ZAZA_HOME must be allowed."""
        zaza_home = tmp_path / ".agent-zaza/data"
        zaza_home.mkdir()
        subdir = zaza_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside ZAZA_HOME pointing outside must be rejected."""
        zaza_home = tmp_path / ".agent-zaza/data"
        zaza_home.mkdir()
        monkeypatch.setenv("ZAZA_HOME", str(zaza_home))

        # Create a sensitive file outside zaza_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside zaza_home pointing outside
        symlink = zaza_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes ZAZA_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, zaza_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape ZAZA_HOME."""
        zaza_home = tmp_path / ".agent-zaza/data"
        zaza_home.mkdir()
        monkeypatch.setenv("ZAZA_HOME", str(zaza_home))
        ...
        sensitive.write_text("{}")
        self._write_config(zaza_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(zaza_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside ZAZA_HOME via config must still mount."""
        zaza_home = tmp_path / ".agent-zaza/data"
        zaza_home.mkdir()
        monkeypatch.setenv("ZAZA_HOME", str(zaza_home))

        (zaza_home / "oauth.json").write_text("{}")
        self._write_config(zaza_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        zaza_home = tmp_path / ".agent-zaza/data"
        zaza_home.mkdir()
        (zaza_home / "cache" / "documents").mkdir(parents=True)
        (zaza_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("ZAZA_HOME", str(zaza_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.agent-zaza/data/cache/documents" in paths
        assert "/root/.agent-zaza/data/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.agent-zaza/data/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        zaza_home = tmp_path / ".agent-zaza/data"
        zaza_home.mkdir()
        # Use legacy dir name — get_zaza_dir prefers old if it exists
        (zaza_home / "document_cache").mkdir()
        (zaza_home / "image_cache").mkdir()
        monkeypatch.setenv("ZAZA_HOME", str(zaza_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(zaza_home / "document_cache") in host_paths
        assert str(zaza_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.agent-zaza/data/cache/documents" in container_paths
        assert "/root/.agent-zaza/data/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        zaza_home = tmp_path / ".agent-zaza/data"
        doc_dir = zaza_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        zaza_home = tmp_path / ".agent-zaza/data"
        ss_dir = zaza_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.agent-zaza/data/cache/screenshots/session_abc/screen1.png"
