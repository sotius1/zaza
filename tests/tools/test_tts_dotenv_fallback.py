...
TTS provider tools must resolve API keys from ``~/.agent-zaza/data/.env`` (via
``zaza_cli.config.get_env_value``) and not only from ``os.environ`` —
...
already addressed for ``agent/credential_pool`` and ``zaza_cli/auth``.
"""
...
class TestDotenvFallbackPerProvider:
    """For each affected provider, when only ``~/.agent-zaza/data/.env`` carries the
    ...
    ``zaza_cli.config.load_env`` integration. Before the fix, ``os.getenv``
...
class TestRegressionGuard:
    ...
    ``zaza_cli.config.load_env`` to simulate ``~/.agent-zaza/data/.env`` carrying the
    ...
    def test_check_tts_requirements_sees_dotenv_minimax(self, monkeypatch):
        ...
        in ``~/.agent-zaza/data/.env``, even though the dispatcher would later succeed.
        """
        from tools import tts_tool
