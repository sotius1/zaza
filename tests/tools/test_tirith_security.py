...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._zaza_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_zaza_bin(self):
        """After _INSTALL_FAILED, manual install in ZAZA_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        zaza_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(zaza_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(zaza_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._zaza_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == zaza_bin
            assert _tirith_mod._resolved_path == zaza_bin
            mock_clear.assert_called_once()
...
class TestZAZAHomeIsolation:
    def test_zaza_bin_dir_respects_zaza_home(self):
        """_zaza_bin_dir must use ZAZA_HOME, not hardcoded ~/.agent-zaza/data."""
        from tools.tirith_security import _zaza_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"ZAZA_HOME": tmpdir}):
            result = _zaza_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_zaza_home(self):
        """_failure_marker_path must use ZAZA_HOME, not hardcoded ~/.agent-zaza/data."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"ZAZA_HOME": "/custom/zaza"}):
            result = _failure_marker_path()
        assert result == "/custom/zaza/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets ZAZA_HOME; verify it's active."""
        zaza_home = os.getenv("ZAZA_HOME")
        assert zaza_home is not None, "ZAZA_HOME should be set by conftest"
        assert "zaza_test" in zaza_home, "Should point to test temp dir"

    def test_get_zaza_home_fallback(self):
        """Without ZAZA_HOME set, falls back to ~/.agent-zaza/data."""
        from tools.tirith_security import _get_zaza_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove ZAZA_HOME entirely
            os.environ.pop("ZAZA_HOME", None)
            result = _get_zaza_home()
        assert result == os.path.join(os.path.expanduser("~"), ".agent-zaza/data")
