...
class TestFirecrawlClientConfig:
    ...
    def test_tool_gateway_domain_builds_firecrawl_gateway_origin(self):
        ...
        with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "zaza.com"}):
            with patch("tools.web_tools._read_nous_access_token", return_value="nous-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    ...
                    mock_fc.assert_called_once_with(
                        api_key="nous-token",
                        api_url="https://firecrawl-gateway.zaza.com",
                    )
        ...
        with patch.dict(os.environ, {
            ...
            with patch("tools.web_tools._read_nous_access_token", return_value="nous-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    ...
                    mock_fc.assert_called_once_with(
                        api_key="nous-token",
                        api_url="http://firecrawl-gateway.zaza.com",
                    )
                    assert result is mock_fc.return_value
    ...
    def test_nous_auth_token_respects_zaza_home_override(self, tmp_path):
        """Auth lookup should read from ZAZA_HOME/auth.json, not ~/.agent-zaza/data/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".agent-zaza/data").mkdir(parents=True)
...
class TestBackendSelection:
    ...
    ``zaza tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
