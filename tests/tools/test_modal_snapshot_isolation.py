...
def _restore_tool_modules():
    original_zaza_home = os.environ.get("ZAZA_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_zaza_home is None:
            os.environ.pop("ZAZA_HOME", None)
        else:
            os.environ["ZAZA_HOME"] = original_zaza_home
        _reset_modules(("tools", "zaza_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    zaza_cli = types.ModuleType("zaza_cli")
    zaza_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["zaza_cli"] = zaza_cli
    zaza_home = tmp_path / "zaza-home"
    os.environ["ZAZA_HOME"] = str(zaza_home)
    sys.modules["zaza_cli.config"] = types.SimpleNamespace(
        get_zaza_home=lambda: zaza_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="agent-zaza", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": zaza_home / "modal_snapshots.json",
