...
def test_openrouter_base_url_applies_or_headers(mock_openai):
    ...
    assert headers["HTTP-Referer"] == "https://agent-zaza.zaza.com"
    assert headers["X-OpenRouter-Title"] == "Agent ZAZA"
...
def test_ai_gateway_base_url_applies_attribution_headers(mock_openai):
    ...
    headers = agent._client_kwargs["default_headers"]
    assert headers["HTTP-Referer"] == "https://agent-zaza.zaza.com"
    assert headers["X-Title"] == "Agent ZAZA"
    assert headers["User-Agent"].startswith("ZAZAAgent/")
