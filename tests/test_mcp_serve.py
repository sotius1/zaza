"""
Tests for mcp_serve — ZAZA MCP server.
...
def _isolate_zaza_home(tmp_path, monkeypatch):
    """Redirect ZAZA_HOME to a temp directory."""
    monkeypatch.setenv("ZAZA_HOME", str(tmp_path))
    try:
        import zaza_constants
        monkeypatch.setattr(zaza_constants, "get_zaza_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking zaza_state schema."""
