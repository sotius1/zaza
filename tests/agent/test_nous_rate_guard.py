...
def rate_guard_env(tmp_path, monkeypatch):
    ...
    zaza_home = str(tmp_path / ".agent-zaza/data")
    os.makedirs(zaza_home, exist_ok=True)
    monkeypatch.setenv("ZAZA_HOME", zaza_home)
...
class TestIsGenuineNousRateLimit:
    ...
    Nous Portal multiplexes upstreams (DeepSeek, Kimi, MiMo, ZAZA).
