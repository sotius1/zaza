"""Tests for the top-level `./zaza` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./zaza` should use `zaza_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "zaza"
    called = []

    fake_main_module = types.ModuleType("zaza_cli.main")

    def fake_main():
        called.append("zaza_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "zaza_cli.main", fake_main_module)
