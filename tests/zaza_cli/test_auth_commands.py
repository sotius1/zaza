...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    (zaza_home / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    monkeypatch.setenv("ZAZA_HOME", str(tmp_path / "zaza"))
    ...
    payload = json.loads((tmp_path / "zaza" / "auth.json").read_text())
...
def test_auth_add_anthropic_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.run_zaza_oauth_login_pure",
        lambda: {
            "access_token": token,
    ...
    entry = next(item for item in entries if item["source"] == "manual:zaza_pkce")
    assert entry["label"] == "claude@example.com"
    assert entry["source"] == "manual:zaza_pkce"
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "zaza_cli.auth._nous_device_code_login",
        lambda **kwargs: {
            "portal_base_url": "https://portal.example.com",
            "inference_base_url": "https://inference.example.com/v1",
            "client_id": "agent-zaza-cli-toolset",
    ...
    # `zaza auth add nous` must also populate providers.nous so the
    ...
    # raises "ZAZA is not logged into Nous Portal" and the agent dies.
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`zaza auth add nous --type oauth --label <name>` must preserve the
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "zaza_cli.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
...
def test_auth_remove_reindexes_priorities(tmp_path, monkeypatch):
    ...
    from zaza_cli.auth_commands import auth_remove_command
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from zaza_cli.auth_commands import auth_reset_command
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`zaza logout --provider openai-codex` must still clear model.provider.
    ...
    config_text = (zaza_home / "config.yaml").read_text()
...
def test_logout_defaults_to_configured_codex_when_no_active_provider(tmp_path, monkeypatch, capsys):
    """Bare `zaza logout` should target configured Codex if auth has no active provider."""
    zaza_home = tmp_path / "zaza"
    monkeypatch.setenv("ZAZA_HOME", str(zaza_home))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}, "credential_pool": {}})
    (zaza_home / "config.yaml").write_text(
...
def test_logout_clears_stale_active_codex_without_provider_credentials(tmp_path, monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "Logged out of OpenAI Codex." in out
    auth_payload = json.loads((zaza_home / "auth.json").read_text())
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from zaza_cli.auth_commands import auth_list_command
    ...
    monkeypatch.setattr(
        "zaza_cli.auth_commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("zaza_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("zaza_cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("zaza_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "zaza_cli.auth_commands.time.time",
...
def test_auth_remove_claude_code_suppresses_reseed(tmp_path, monkeypatch):
    ...
    (zaza_home / "auth.json").write_text(json.dumps(auth_store))

    from types import SimpleNamespace
    from zaza_cli.auth_commands import auth_remove_command
    auth_remove_command(SimpleNamespace(provider="anthropic", target="1"))

    updated = json.loads((zaza_home / "auth.json").read_text())
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from zaza_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
def test_unsuppress_credential_source_returns_false_when_absent(tmp_path, monkeypatch):
    ...
    from zaza_cli.auth import unsuppress_credential_source
...
def test_auth_add_codex_clears_suppression_marker(tmp_path, monkeypatch):
    """Re-linking codex via `zaza auth add openai-codex` must clear any suppression marker."""
    monkeypatch.setenv("ZAZA_HOME", str(tmp_path / "zaza"))
    monkeypatch.setattr(
        "agent.credential_pool._seed_from_singletons",
        lambda provider, entries: (False, set()),
    ...
    # Pre-existing suppression (simulating a prior `zaza auth remove`)
    (zaza_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
    ...
    payload = json.loads((zaza_home / "auth.json").read_text())
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("zaza_cli.auth._import_codex_cli_tokens", _fake_import)
    ...
    after = json.loads((zaza_home / "auth.json").read_text())
...
def test_auth_remove_env_seeded_suppresses_shell_exported_var(tmp_path, monkeypatch, capsys):
    """`zaza auth remove xai 1` must stick even when the env var is exported
    by the shell (not written into ~/.agent-zaza/data/.env).  Before PR for #13371 the
...
def test_auth_remove_env_seeded_dotenv_only_no_shell_hint(tmp_path, monkeypatch, capsys):
    """When the env var lives only in ~/.agent-zaza/data/.env (not the shell), the
    ...
    (zaza_home / ".env").write_text("DEEPSEEK_API_KEY=sk-ds-only\n")
    ...
    assert (zaza_home / ".env").read_text().strip() == ""


def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    """Re-adding a credential via `zaza auth add <provider>` clears any
    ...
    from types import SimpleNamespace
    from zaza_cli.auth import is_source_suppressed
    from zaza_cli.auth_commands import auth_add_command
...
def test_seed_from_env_respects_env_suppression(tmp_path, monkeypatch):
    ...
    via `zaza auth remove`.  This is the gate that prevents shell-exported
...
# =============================================================================
# Unified credential-source stickiness — every source ZAZA reads from has a
...
def test_seed_from_singletons_respects_copilot_suppression(tmp_path, monkeypatch):
    ...
    import zaza_cli.copilot_auth as ca
...
def test_seed_from_singletons_respects_zaza_pkce_suppression(tmp_path, monkeypatch):
    """anthropic zaza_pkce must not re-seed from ~/.agent-zaza/data/.anthropic_oauth.json when suppressed."""
    zaza_home = tmp_path / "zaza"
    zaza_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("ZAZA_HOME", str(zaza_home))

    import yaml
    (zaza_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (zaza_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["zaza_pkce"]},
    }))

    # Stub the readers so only zaza_pkce is "available"; claude_code returns None
    import agent.anthropic_adapter as aa
    monkeypatch.setattr(aa, "read_zaza_oauth_credentials", lambda: {
    ...
    # zaza_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "zaza_pkce" not in active
...
def test_seed_custom_pool_respects_config_suppression(tmp_path, monkeypatch):
    ...
    import yaml
    (zaza_home / "config.yaml").write_text(yaml.dump({
        "model": {},
        "custom_providers": [
...
def test_credential_sources_registry_has_expected_steps():
    ...
    expected = {
        ...
        "~/.agent-zaza/data/.anthropic_oauth.json",
...
def test_auth_add_clears_all_suppressions_including_non_env(tmp_path, monkeypatch):
    """Re-adding a credential via `zaza auth add <provider>` clears ALL
...
def test_auth_remove_codex_manual_device_code_suppresses_canonical(tmp_path, monkeypatch):
    """Removing a manual:device_code entry (from `zaza auth add openai-codex`)
