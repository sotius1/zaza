"""Comprehensive tests for zaza_cli.profiles module.
...
from zaza_cli.profiles import (
    ...
    _get_profiles_root,
    _get_default_zaza_home,
)


# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and ZAZA_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.agent-zaza/data/profiles)
    * ZAZA_HOME  -> tmp_path/.zaza  (so get_zaza_home() agrees)
    * Creates the bare-minimum ~/.agent-zaza/data directory.
    """
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    default_home = tmp_path / ".agent-zaza/data"
    default_home.mkdir(exist_ok=True)
    monkeypatch.setenv("ZAZA_HOME", str(default_home))
    return tmp_path
...
class TestGetProfileDir:
    ...
    def test_default_returns_zaza_home(self, profile_env):
        tmp_path = profile_env
        result = get_profile_dir("default")
        assert result == tmp_path / ".agent-zaza/data"

    def test_named_profile_returns_profiles_subdir(self, profile_env):
        tmp_path = profile_env
        result = get_profile_dir("coder")
        assert result == tmp_path / ".agent-zaza/data" / "profiles" / "coder"
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.agent-zaza/data must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".agent-zaza/data"
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("zaza_cli.profiles._cleanup_gateway_service"):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".agent-zaza/data" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_zaza_home_returns_default(self, profile_env):
        # ZAZA_HOME points to tmp_path/.zaza which is the default
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom ZAZA_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("ZAZA_HOME", str(custom))
        # With Docker-aware roots, a custom ZAZA_HOME is the default —
...
class TestResolveProfileEnv:
    ...
    def test_existing_profile_returns_path(self, profile_env):
        ...
        assert result == str(tmp_path / ".agent-zaza/data" / "profiles" / "coder")

    def test_default_returns_default_home(self, profile_env):
        tmp_path = profile_env
        result = resolve_profile_env("default")
        assert result == str(tmp_path / ".agent-zaza/data")
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("zaza")
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        old_dir = tmp_path / ".agent-zaza/data" / "profiles" / "oldname"
        assert old_dir.is_dir()

        # Mock alias collision to avoid subprocess calls
        with patch("zaza_cli.profiles.check_alias_collision", return_value="skip"):
            new_dir = rename_profile("oldname", "newname")

        assert not old_dir.is_dir()
        assert new_dir.is_dir()
        assert new_dir == tmp_path / ".agent-zaza/data" / "profiles" / "newname"

    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        tmp_path = profile_env
        create_profile("ssi_health", no_alias=True)
        honcho_path = tmp_path / ".agent-zaza/data" / "honcho.json"
        ...
        assert "zaza.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["zaza.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["zaza.heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "zaza.ssi_health": {"workspace": "zaza", "enabled": True}
            }
        }))
        ...
        assert "zaza.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["zaza.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["zaza.heimdall"]["workspace"] == "zaza"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "zaza.ssi_health": {"aiPeer": "ssi_health"},
                "zaza.heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["zaza.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["zaza.heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("agent-zaza", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".zaza_history",
        ...
        excluded_prefixes = [
            "default/agent-zaza", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.zaza_history",
...
class TestCompletion:
    ...
    def test_bash_completion_has_zaza_profiles_function(self):
        script = generate_bash_completion()
        assert "_zaza_profiles" in script

    def test_zsh_completion_has_zaza_function(self):
        script = generate_zsh_completion()
        assert "_zaza" in script


# ===================================================================
# TestGetProfilesRoot / TestGetDefaultZAZAHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_zaza_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".agent-zaza/data" / "profiles"

    def test_default_zaza_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_zaza_home()
        assert home == tmp_path / ".agent-zaza/data"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (ZAZA_HOME outside ~/.agent-zaza/data), profiles go under ZAZA_HOME."""
    ...
    def test_default_zaza_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_zaza_home() returns ZAZA_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZAZA_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (ZAZA_HOME under ~/.agent-zaza/data), profiles root is still ~/.agent-zaza/data/profiles."""
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under ZAZA_HOME."""
        from zaza_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZAZA_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under ZAZA_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("ZAZA_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".agent-zaza/data" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from zaza_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".agent-zaza/data"
