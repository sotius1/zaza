...
from zaza_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from zaza_cli.auth import _resolve_verify
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("ZAZA_CA_BUNDLE", raising=False)
        result = _resolve_verify(auth_state={"tls": {}})
        assert result is True

    def test_missing_zaza_ca_bundle_env_falls_back(self, monkeypatch):
        from zaza_cli.auth import _resolve_verify

        monkeypatch.setenv("ZAZA_CA_BUNDLE", "/nonexistent/zaza-ca.pem")
...
def _setup_nous_auth(
    ...
    *,
    ...
    (zaza_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_get_nous_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    ...
    """
    from zaza_cli.auth import get_nous_auth_status
...
def test_get_nous_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    ...
    zaza_home = tmp_path / "zaza"
    _setup_nous_auth(zaza_home, access_token="at-123")
    monkeypatch.setenv("ZAZA_HOME", str(zaza_home))
    monkeypatch.setattr(
...
def test_get_nous_auth_status_prefers_runtime_auth_store_over_stale_pool(tmp_path, monkeypatch):
    ...
    zaza_home = tmp_path / "zaza"
    _setup_nous_auth(zaza_home, access_token="at-fresh")
    monkeypatch.setenv("ZAZA_HOME", str(zaza_home))
...
def test_get_nous_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("zaza_cli.auth.resolve_nous_runtime_credentials", _boom)
...
def test_refresh_token_persisted_when_mint_returns_insufficient_credits(tmp_path, monkeypatch):
    zaza_home = tmp_path / "zaza"
    _setup_nous_auth(zaza_home, refresh_token="refresh-old")
    monkeypatch.setenv("ZAZA_HOME", str(zaza_home))
    ...
    monkeypatch.setattr("zaza_cli.auth._refresh_access_token", _fake_refresh_access_token)
    monkeypatch.setattr("zaza_cli.auth._mint_agent_key", _fake_mint_agent_key)
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `zaza model` → Nous Portal → Skip (keep current) after
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        auth_path = zaza_home / "auth.json"
        auth_path.write_text(json.dumps({
            "version": 1,
            "active_provider": "openrouter",
            "providers": {"openrouter": {"api_key": "sk-or-fake"}},
        }))
        return zaza_home, config_path, auth_path
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        import zaza_cli.auth as auth_mod
        import zaza_cli.models as models_mod
        import zaza_cli.nous_subscription as ns

        fake_auth_state = {
            "access_token": "fake-nous-token",
            "agent_key": "fake-agent-key",
            "inference_base_url": "https://inference-api.zaza.com",
            "portal_base_url": "https://portal.zaza.com",
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from zaza_cli.auth import PROVIDER_REGISTRY, _login_nous

        zaza_home, config_path, auth_path = self._setup_home_with_openrouter(
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `zaza auth add nous`
    ...
    """
    from zaza_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    must succeed (not raise "ZAZA is not logged into Nous Portal").
    ...
    """
    from zaza_cli.auth import persist_nous_credentials, resolve_nous_runtime_credentials
    ...
    # doesn't raise "ZAZA is not logged into Nous Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_persist_nous_credentials_embeds_custom_label(tmp_path, monkeypatch):
    ...
    Previously `zaza auth add nous --type oauth --label <name>` silently
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    ``error_description`` containing "reuse detected", ZAZA must surface an
    ...
    re-authenticate" string led users to report this as a ZAZA persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from zaza_cli.auth import _refresh_access_token
    ...
    assert "zaza auth add nous" in message.lower()
