...
Ensures ``zaza update`` warns the user when dashboard processes from a
...
from zaza_cli.main import _warn_stale_dashboard_processes
...
class TestWarnStaleDashboardProcesses:
    ...
    def test_warning_printed_for_running_dashboard(self, capsys):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=_ps_line(12345, "python3 -m zaza_cli.main dashboard --port 9119") + "\n",
                stderr="",
            )
            _warn_stale_dashboard_processes()
        output = capsys.readouterr().out
    ...
    def test_multiple_dashboard_pids(self, capsys):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(12345, "python3 -m zaza_cli.main dashboard --port 9119"),
                    _ps_line(12346, "zaza dashboard --port 9120 --no-open"),
                    _ps_line(12347, "python /home/x/zaza_cli/main.py dashboard"),
                ]) + "\n",
                stderr="",
            )
            _warn_stale_dashboard_processes()
    ...
    def test_self_pid_excluded(self, capsys):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(os.getpid(), "python3 -m zaza_cli.main dashboard"),
                    _ps_line(12345, "zaza dashboard --port 9119"),
                ]) + "\n",
                stderr="",
            )
            _warn_stale_dashboard_processes()
    ...
    def test_unrelated_process_containing_word_dashboard_not_matched(self, capsys):
        """A process whose cmdline contains 'dashboard' but isn't a zaza
        dashboard process must NOT be flagged.  This guards against the old
        ``pgrep -f "zaza.*dashboard"`` greedy regex that matched e.g. a
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                ...
                stdout="\n".join([
                    ...
                    _ps_line(12345, "python3 -m zaza_cli.main dashboard --port 9119"),
                    # zaza running something else, with "dashboard" as a
                    # substring of an unrelated arg — should NOT match.
                    _ps_line(22222, "python3 -m zaza_cli.main chat -q 'rewrite my dashboard'"),
    ...
    def test_grep_lines_ignored(self, capsys):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout="\n".join([
                    _ps_line(99999, "grep zaza dashboard"),
                    _ps_line(12345, "zaza dashboard --port 9119"),
                ]) + "\n",
                stderr="",
            )
...
class TestWindowsWmicEncoding:
    ...
    `zaza update` on non-UTF-8 system locales (e.g. cp936 on zh-CN).
    """
    ...
    def test_wmic_invoked_with_utf8_ignore_errors(self, monkeypatch):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            mock_run.return_value = MagicMock(
                returncode=0,
                stdout=(
                    "CommandLine=python -m zaza_cli.main dashboard\n"
                    "ProcessId=12345\n"
                ),
                stderr="",
            )
    ...
    def test_wmic_returns_none_stdout_does_not_crash(self, monkeypatch, capsys):
        ...
        ``None.split('\\n')`` and aborting `zaza update` (#17049)."""
        monkeypatch.setattr(sys, "platform", "win32")
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = MagicMock(
