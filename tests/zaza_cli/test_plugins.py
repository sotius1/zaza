"""Tests for the ZAZA plugin system (zaza_cli.plugins)."""
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<zaza_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<zaza_home>/plugins/``; we derive
    ``<zaza_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <ZAZA_HOME>/config.yaml.
        # Config is always read from ZAZA_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        zaza_home_str = os.environ.get("ZAZA_HOME")
        if zaza_home_str:
            zaza_home = Path(zaza_home_str)
        else:
            zaza_home = base.parent
        zaza_home.mkdir(parents=True, exist_ok=True)
        cfg_path = zaza_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.agent-zaza/data/plugins/ are discovered."""
        plugins_dir = tmp_path / "zaza_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("ZAZA_HOME", str(tmp_path / "zaza_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ZAZA_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".agent-zaza/data" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under zaza_plugins.<name>."""
        plugins_dir = tmp_path / "zaza_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("ZAZA_HOME", str(tmp_path / "zaza_test"))

        # Clean up any prior namespace module
        sys.modules.pop("zaza_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "zaza_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="zaza_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import zaza_cli.plugins as plugins_mod
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("zaza_cli.plugins._plugin_manager", mgr):
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "zaza_test" / "plugins"
        # Set ZAZA_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("zaza_cli.plugins.PluginContext.dispatch_tool.__module__", "zaza_cli.plugins"):
