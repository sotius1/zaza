...
Covers:
 - Normal path (tokens already in ZAZA auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — ZAZA
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `zaza auth openai-codex`. The old
...
@pytest.fixture()
def zaza_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in ZAZA auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(zaza_auth_only_env):
    """openai-codex appears when tokens are already in ZAZA auth store."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or ZAZA
    auth store."""
    zaza_home = tmp_path / ".agent-zaza/data"
    zaza_home.mkdir()
    ...
    (zaza_home / "auth.json").write_text(
