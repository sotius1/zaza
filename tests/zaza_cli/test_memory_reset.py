"""Tests for the `zaza memory reset` CLI command.
...
- Profile-scoped reset (uses ZAZA_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake ZAZA_HOME with memory files."""
    zaza_home = tmp_path / ".agent-zaza/data"
    memories = zaza_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("ZAZA_HOME", str(zaza_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nZAZA repo is at ~/.agent-zaza/data/agent-zaza\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `zaza memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        zaza_home = tmp_path / ".agent-zaza/data"
        (zaza_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("ZAZA_HOME", str(zaza_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's ZAZA_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        zaza_home = tmp_path / ".agent-zaza/data"
        zaza_home.mkdir(parents=True)
        # No memories dir
        monkeypatch.setenv("ZAZA_HOME", str(zaza_home))

        # The memories dir won't exist; get_zaza_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
