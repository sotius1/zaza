...
import zaza_cli.doctor as doctor
import zaza_cli.gateway as gateway_cli
from zaza_cli import doctor as doctor_mod
from zaza_cli.doctor import _has_provider_env_config
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(doctor_mod, "ZAZA_HOME", zaza_home)
    monkeypatch.delenv("ZAZA_INTERACTIVE", raising=False)

    seen = {}

    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("ZAZA_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_zaza_home(self, tmp_path, provider=""):
        """Create a minimal ZAZA_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_zaza_home(tmp_path, provider)
...
def test_run_doctor_accepts_zaza_provider_ids_that_catalog_aliases(
