"""Tests that provider selection via `zaza model` always persists correctly.
...
def config_home(tmp_path, monkeypatch):
    """Isolated ZAZA_HOME with a minimal string-format config."""
    home = tmp_path / "zaza"
    home.mkdir()
...
class TestSaveModelChoiceAlwaysDict:
    def test_string_model_becomes_dict(self, config_home):
        ...
        from zaza_cli.auth import _save_model_choice
...
class TestProviderPersistsAfterModelSave:
    def test_api_key_provider_saved_when_model_was_string(self, config_home, monkeypatch):
        ...
        from zaza_cli.main import _model_flow_api_key_provider
        ...
        with patch("zaza_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("zaza_cli.auth.deactivate_provider"), \
    ...
    def test_copilot_provider_saved_when_selected(self, config_home):
        ...
        from zaza_cli.main import _model_flow_copilot
        from zaza_cli.config import load_config
        ...
        with patch(
            ...
            "zaza_cli.auth._prompt_model_selection",
            return_value="gpt-5.4",
        ), patch(
            "zaza_cli.main._prompt_reasoning_effort_selection",
            return_value="high",
        ), patch(
            "zaza_cli.auth.deactivate_provider",
        ):
    ...
    def test_copilot_acp_provider_saved_when_selected(self, config_home):
        ...
        from zaza_cli.main import _model_flow_copilot_acp
        from zaza_cli.config import load_config

        with patch(
            "zaza_cli.auth.get_external_process_provider_status",
            return_value={
            ...
            "zaza_cli.auth.resolve_external_process_provider_credentials",
            return_value={
    ...
    def test_opencode_go_models_are_selectable_and_persist_normalized(self, config_home, monkeypatch):
        ...
        with patch("zaza_cli.models.fetch_api_models", return_value=["opencode-go/kimi-k2.5", "opencode-go/minimax-m2.7"]), \
             patch("zaza_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("zaza_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
    ...
    def test_opencode_go_same_provider_switch_recomputes_api_mode(self, config_home, monkeypatch):
        ...
        with patch("zaza_cli.models.fetch_api_models", return_value=["opencode-go/kimi-k2.5", "opencode-go/minimax-m2.5"]), \
             patch("zaza_cli.auth._prompt_model_selection", return_value="minimax-m2.5"), \
             patch("zaza_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
    ...
    def test_lmstudio_provider_saved_when_selected(self, config_home, monkeypatch):
        ...
        monkeypatch.setattr(
            "zaza_cli.models.fetch_lmstudio_models",
...
class TestBaseUrlValidation:
    ...
    def test_invalid_base_url_rejected(self, config_home, monkeypatch, capsys):
        ...
        from zaza_cli.main import _model_flow_api_key_provider
        from zaza_cli.config import load_config, get_env_value

        # User types a shell command instead of a URL at the base URL prompt
        with patch("zaza_cli.auth._prompt_model_selection", return_value="glm-5"), \
             patch("zaza_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value="nano ~/.agent-zaza/data/.env"):
    ...
    def test_stepfun_provider_saved_with_selected_region(self, config_home, monkeypatch):
        from zaza_cli.main import _model_flow_stepfun
        from zaza_cli.config import load_config, get_env_value
