"""Unit tests for zaza_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf zaza-ok"])
        try:
            output = _read_until(bridge, b"zaza-ok")
            assert b"zaza-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$ZAZA_PTY_TEST\""],
            env={**os.environ, "ZAZA_PTY_TEST": "pty-env-works"},
        )
        try:
