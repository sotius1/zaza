"""Tests for zaza_cli/completion.py — shell completion script generation."""
...
from zaza_cli.completion import _walk, generate_bash, generate_zsh, generate_fish
...
def _make_parser() -> argparse.ArgumentParser:
    """Build a minimal parser that mirrors the real zaza structure."""
    p = argparse.ArgumentParser(prog="zaza")
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "_zaza_completion()" in out
        assert "complete -F _zaza_completion zaza" in out
...
class TestGenerateFish:
    def test_disables_file_completion(self):
        ...
        assert "complete -c zaza -f" in out
...
class TestSubcommandDrift:
    def test_SUBCOMMANDS_covers_required_commands(self):
        ...
        """
        import inspect
        from zaza_cli.main import _coalesce_session_name_args
...
class TestProfileCompletion:
    ...
    def test_bash_has_profiles_helper(self):
        out = generate_bash(_make_parser())
        assert "_zaza_profiles()" in out
        assert 'profiles_dir="$HOME/.agent-zaza/data/profiles"' in out
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        """After 'zaza profile use', complete with profile names."""
        out = generate_bash(_make_parser())
        # The profile case should have _zaza_profiles for name-taking actions
        lines = out.split("\n")
        ...
        for line in lines:
            ...
            if in_profile_case and "_zaza_profiles" in line:
                has_profiles_in_action = True
                break
        assert has_profiles_in_action, "profile actions should complete with _zaza_profiles"

    def test_zsh_has_profiles_helper(self):
        out = generate_zsh(_make_parser())
        assert "_zaza_profiles()" in out
        assert "$HOME/.agent-zaza/data/profiles" in out
    ...
    def test_fish_profile_actions_complete_names(self):
        ...
        assert "__zaza_profiles" in out
        count = out.count("(__zaza_profiles)")
