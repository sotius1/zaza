"""Tests for the update check mechanism in zaza_cli.banner."""
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    with patch("zaza_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ZAZA_HOME", str(tmp_path))
    with patch("zaza_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    import zaza_cli.banner as banner

    # Create a fake banner.py so the fallback path also has no .git
    fake_banner = tmp_path / "zaza_cli" / "banner.py"
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when ZAZA_HOME has no git repo."""
    import zaza_cli.banner as banner
    ...
    # Point ZAZA_HOME at a temp dir with no agent-zaza/.git
    monkeypatch.setenv("ZAZA_HOME", str(tmp_path))
    with patch("zaza_cli.banner.subprocess.run") as mock_run:
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from zaza_cli.main import _invalidate_update_cache

    # Build a fake ~/.agent-zaza/data with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"ZAZA_HOME": str(default_home)}):
