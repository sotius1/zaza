...
def container_env(tmp_path, monkeypatch):
    """Set up a fake ZAZA_HOME with .container-mode file."""
    zaza_home = tmp_path / ".agent-zaza/data"
    zaza_home.mkdir()
    monkeypatch.setenv("ZAZA_HOME", str(zaza_home))
    monkeypatch.delenv("ZAZA_DEV", raising=False)

    container_mode = zaza_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=agent-zaza\n"
        "exec_user=zaza\n"
        "zaza_bin=/data/current-package/bin/zaza\n"
    )
    return zaza_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("zaza_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "agent-zaza"
    assert info["exec_user"] == "zaza"
    assert info["zaza_bin"] == "/data/current-package/bin/zaza"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("zaza_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_zaza_dev(container_env, monkeypatch):
    """Returns None when ZAZA_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("ZAZA_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_zaza_dev_zero(container_env, monkeypatch):
    """ZAZA_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("ZAZA_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        zaza_home = Path(tmpdir) / ".agent-zaza/data"
        zaza_home.mkdir()
        (zaza_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("zaza_constants.is_container", return_value=False), \
             patch("zaza_cli.config.get_zaza_home", return_value=zaza_home), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("ZAZA_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=zaza-custom\n"
        "exec_user=myuser\n"
        "zaza_bin=/opt/zaza/bin/zaza\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "zaza-custom"
    assert info["exec_user"] == "myuser"
    assert info["zaza_bin"] == "/opt/zaza/bin/zaza"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from zaza_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "zaza"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "agent-zaza" in cmd
    assert "/data/current-package/bin/zaza" in cmd
    assert "chat" in cmd
