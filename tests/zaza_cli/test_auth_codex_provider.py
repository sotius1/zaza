"""Tests for Codex auth — tokens stored in ZAZA auth store (~/.agent-zaza/data/auth.json)."""
...
def _setup_zaza_auth(zaza_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the ZAZA auth store."""
...
def test_read_codex_tokens_missing(tmp_path, monkeypatch):
    ...
    zaza_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store
    (zaza_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
    monkeypatch.setenv("ZAZA_HOME", str(zaza_home))
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    zaza_home = tmp_path / "zaza"
    _setup_zaza_auth(zaza_home, access_token="")
    monkeypatch.setenv("ZAZA_HOME", str(zaza_home))
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_zaza_auth(zaza_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("ZAZA_HOME", str(zaza_home))
    ...
    monkeypatch.setattr("zaza_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    zaza_home = tmp_path / "zaza"
    _setup_zaza_auth(zaza_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("ZAZA_HOME", str(zaza_home))
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to ZAZA auth store, not ~/.codex/."""
    zaza_home = tmp_path / "zaza"
    codex_home = tmp_path / "codex-cli"
    zaza_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "zaza-at", "refresh_token": "zaza-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches ZAZA store
    assert not (codex_home / "auth.json").exists()

    # ZAZA auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "zaza-at"


def test_resolve_returns_zaza_auth_store_source(tmp_path, monkeypatch):
    zaza_home = tmp_path / "zaza"
    _setup_zaza_auth(zaza_home)
    monkeypatch.setenv("ZAZA_HOME", str(zaza_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "zaza-auth-store"
...
def _patch_httpx(monkeypatch, response):
    ...
    monkeypatch.setattr("zaza_cli.auth.httpx.Client", _factory)
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "zaza_cli.auth.resolve_codex_runtime_credentials",
        lambda: {"base_url": DEFAULT_CODEX_BASE_URL},
    )
    monkeypatch.setattr(
        "zaza_cli.auth._import_codex_cli_tokens",
    ...
    monkeypatch.setattr("zaza_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("zaza_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
