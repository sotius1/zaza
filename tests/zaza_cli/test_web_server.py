"""Tests for zaza_cli.web_server and related config utilities."""
...
class TestReloadEnv:
    ...
    def test_adds_new_vars(self, tmp_path):
        ...
        with patch("zaza_cli.config.get_env_path", return_value=env_file):
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known ZAZA vars not present in .env."""
        env_file = tmp_path / ".env"
    ...
    def test_does_not_remove_unknown_vars(self, tmp_path):
        """reload_env() preserves non-ZAZA env vars even when absent from .env."""
        env_file = tmp_path / ".env"
        env_file.write_text("")
        with patch("zaza_cli.config.get_env_path", return_value=env_file):
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_zaza_home):
        """Create a TestClient and isolate the state DB under the test ZAZA_HOME."""
        try:
        ...
        from zaza_cli.web_server import app, _SESSION_HEADER_NAME, _SESSION_TOKEN

        monkeypatch.setattr(zaza_state, "DEFAULT_DB_PATH", get_zaza_home() / "state.db")
    ...
    def test_get_status(self):
        ...
        assert "zaza_home" in data
        assert "active_sessions" in data

    def test_get_status_filters_unconfigured_gateway_platforms(self, monkeypatch):
        import gateway.config as gateway_config
        import zaza_cli.web_server as web_server
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from zaza_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_no_token(self, tmp_path):
        ...
        from starlette.testclient import TestClient
        from zaza_cli.web_server import app
        from zaza_cli.config import save_env_value
    ...
    def test_reveal_env_var_bad_token(self, tmp_path):
        ...
        from zaza_cli.config import save_env_value
        from zaza_cli.web_server import _SESSION_HEADER_NAME
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from zaza_cli.config import save_env_value
        from zaza_cli.web_server import _SESSION_TOKEN
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/zaza_cli/web_server.py")
        assert resp.status_code in (200, 404)
        if resp.status_code == 200:
...
class TestBuildSchemaFromConfig:
    def test_produces_expected_field_count(self):
        from zaza_cli.web_server import CONFIG_SCHEMA
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from zaza_cli.web_server import _build_schema_from_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_zaza_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import zaza_cli.skills_config as skills_config
        import zaza_cli.web_server as web_server
    ...
    def test_toolsets_list_matches_cli_enabled_state(self, monkeypatch):
        import zaza_cli.tools_config as tools_config
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from zaza_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from zaza_cli.web_server import _denormalize_config_from_web
...
class TestModelInfoEndpoint:
    ...
    def test_model_info_with_dict_config(self, monkeypatch):
        import zaza_cli.web_server as ws
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_missing_name(self):
        from zaza_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.agent-zaza/data/dashboard-themes/."""

    def test_returns_empty_when_dir_missing(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ZAZA_HOME", str(tmp_path))
        from zaza_cli import web_server
...
# or ``sh -c 'printf …'``) instead of the real ``zaza --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_streams_child_stdout_to_client(self, monkeypatch):
        ...
        with self.client.websocket_connect(self._url()) as conn:
            ...
            while time.monotonic() < deadline:
                ...
                if frame:
                    buf += frame
                if b"zaza-ws-ok" in buf:
                    break
            assert b"zaza-ws-ok" in buf
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from zaza_cli.pty_bridge import PtyUnavailableError
        ...
        import zaza_cli.web_server as ws_mod
    ...
    def test_channel_param_propagates_sidecar_url(self, monkeypatch):
        ...
        ZAZA_TUI_SIDECAR_URL env var pointing back at /api/pub on the
