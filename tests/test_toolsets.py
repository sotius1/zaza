...
class TestToolsetConsistency:
    ...
    def test_zaza_platforms_share_core_tools(self):
        """All zaza-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on zaza-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["agent-zaza-cli-toolset", "zaza-telegram", "zaza-discord", "zaza-whatsapp", "zaza-slack", "zaza-signal", "zaza-homeassistant"]
