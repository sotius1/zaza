...
class TestManifest:
    ...
    def test_manifest_fields(self):
        ...
        # Required env vars are the user-facing ZAZA_ prefixed keys.
        assert "ZAZA_LANGFUSE_PUBLIC_KEY" in data["requires_env"]
        assert "ZAZA_LANGFUSE_SECRET_KEY" in data["requires_env"]
...
class TestDiscovery:
    def test_plugin_is_discovered_as_standalone_opt_in(self, tmp_path, monkeypatch):
        ...
        from zaza_cli import plugins as plugins_mod

        # Isolated ZAZA_HOME so we don't read the developer's config.yaml.
...
class TestRuntimeGate:
    ...
    def test_get_langfuse_returns_none_without_credentials(self, monkeypatch):
        for k in (
            "ZAZA_LANGFUSE_PUBLIC_KEY", "ZAZA_LANGFUSE_SECRET_KEY",
    ...
    def test_get_langfuse_caches_failure_no_config_load(self, monkeypatch):
        ...
        def tracking_get(key, default=None):
            if key.startswith(("ZAZA_LANGFUSE_", "LANGFUSE_")):
                called["n"] += 1
    ...
    def test_get_langfuse_does_not_import_zaza_config(self, monkeypatch):
        ...
        # Drop any cached import of zaza_cli.config.
        sys.modules.pop("zaza_cli.config", None)

        langfuse_plugin = self._fresh_plugin()
        for _ in range(20):
            langfuse_plugin._get_langfuse()

        assert "zaza_cli.config" not in sys.modules, (
            "langfuse plugin imported zaza_cli.config — regression toward "
