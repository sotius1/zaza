...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure ZAZA_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def _make_provider(self, tmp_path, monkeypatch, api_key="rdb-test-key"):
        ...
        (tmp_path / ".agent-zaza/data").mkdir(exist_ok=True)
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", zaza_home=str(tmp_path / ".agent-zaza/data"))
    ...
    def test_initialize_profile_project(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", zaza_home=profile_home)
        assert p._client.project == "zaza-coder"
        p.shutdown()

    def test_initialize_seeds_soul_md(self, tmp_path, monkeypatch):
        p = self._make_provider(tmp_path, monkeypatch)
        soul_path = tmp_path / ".agent-zaza/data" / "SOUL.md"
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        zaza_home = tmp_path / ".agent-zaza/data"
        zaza_home.mkdir(exist_ok=True)
        monkeypatch.setenv("ZAZA_HOME", str(zaza_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", zaza_home=str(zaza_home))
        return p
