"""Tests for zaza_logging — centralized logging setup."""
...
def _reset_logging_state():
    ...
    """
    zaza_logging._logging_initialized = False
    ...
    zaza_logging._install_session_record_factory()
    yield
    ...
    for h in list(root.handlers):
        if h not in pre_existing:
            ...
            h.close()
    zaza_logging._logging_initialized = False
    zaza_logging.clear_session_context()


@pytest.fixture
def zaza_home(tmp_path, monkeypatch):
    """Provide an isolated ZAZA_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_zaza_home from conftest,
    reading it back from the env var to avoid double-mkdir conflicts.
    """
    home = Path(os.environ["ZAZA_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, zaza_home):
        log_dir = zaza_logging.setup_logging(zaza_home=zaza_home)
        assert log_dir == zaza_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, zaza_home):
        zaza_logging.setup_logging(zaza_home=zaza_home)
        root = logging.getLogger()
    ...
    def test_creates_errors_log_handler(self, zaza_home):
        zaza_logging.setup_logging(zaza_home=zaza_home)
        root = logging.getLogger()
    ...
    def test_idempotent_no_duplicate_handlers(self, zaza_home):
        zaza_logging.setup_logging(zaza_home=zaza_home)
        zaza_logging.setup_logging(zaza_home=zaza_home)  # second call — should be no-op
    ...
    def test_force_reinitializes(self, zaza_home):
        zaza_logging.setup_logging(zaza_home=zaza_home)
        # Force still won't add duplicate handlers because _add_rotating_handler
        # checks by resolved path.
        zaza_logging.setup_logging(zaza_home=zaza_home, force=True)
    ...
    def test_custom_log_level(self, zaza_home):
        zaza_logging.setup_logging(zaza_home=zaza_home, log_level="DEBUG")
    ...
    def test_custom_max_size_and_backup(self, zaza_home):
        zaza_logging.setup_logging(
            zaza_home=zaza_home, max_size_mb=10, backup_count=5
        )
    ...
    def test_suppresses_noisy_loggers(self, zaza_home):
        zaza_logging.setup_logging(zaza_home=zaza_home)
    ...
    def test_writes_to_agent_log(self, zaza_home):
        zaza_logging.setup_logging(zaza_home=zaza_home)

        test_logger = logging.getLogger("test_zaza_logging.write_test")
        ...
        agent_log = zaza_home / "logs" / "agent.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for agent.log" in content

    def test_warnings_appear_in_both_logs(self, zaza_home):
        zaza_logging.setup_logging(zaza_home=zaza_home)

        test_logger = logging.getLogger("test_zaza_logging.warning_test")
        ...
        agent_log = zaza_home / "logs" / "agent.log"
        errors_log = zaza_home / "logs" / "errors.log"
        assert "this is a warning" in agent_log.read_text()
        assert "this is a warning" in errors_log.read_text()

    def test_info_not_in_errors_log(self, zaza_home):
        zaza_logging.setup_logging(zaza_home=zaza_home)

        test_logger = logging.getLogger("test_zaza_logging.info_test")
    ...
    def test_explicit_params_override_config(self, zaza_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (zaza_home / "config.yaml").write_text(yaml.dump(config))

        zaza_logging.setup_logging(zaza_home=zaza_home, log_level="WARNING")
    ...
    def test_record_factory_installed(self, zaza_home):
        """The custom record factory injects session_tag on all records."""
        zaza_logging.setup_logging(zaza_home=zaza_home)
        factory = logging.getLogRecordFactory()
        assert getattr(factory, "_zaza_session_injector", False), (
            "Record factory should have _zaza_session_injector marker"
        )
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, zaza_home):
        zaza_logging.setup_logging(zaza_home=zaza_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, zaza_home):
        zaza_logging.setup_logging(zaza_home=zaza_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_created_after_cli_init(self, zaza_home):
        ...
        gw_log = zaza_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "gateway connected after cli init" in gw_log.read_text()

    def test_gateway_log_created_after_cli_init_without_duplicate_handlers(self, zaza_home):
    ...
    def test_gateway_log_receives_gateway_records(self, zaza_home):
    ...
    def test_gateway_log_rejects_non_gateway_records(self, zaza_home):
    ...
    def test_agent_log_still_receives_all(self, zaza_home):
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, zaza_home):
        """When session context is set, log lines include [session_id]."""
        zaza_logging.setup_logging(zaza_home=zaza_home)
        zaza_logging.set_session_context("abc123")
    ...
    def test_no_session_tag_without_context(self, zaza_home):
    ...
    def test_clear_session_context(self, zaza_home):
        """After clearing, session tag disappears."""
        zaza_logging.setup_logging(zaza_home=zaza_home)
        zaza_logging.set_session_context("xyz789")
        zaza_logging.clear_session_context()
    ...
    def test_session_context_thread_isolated(self, zaza_home):
        ...
        def thread_a():
            zaza_logging.set_session_context("thread_a_session")
            logging.getLogger("test.thread_a").info("from thread A")
            for h in logging.getLogger().handlers:
                h.flush()

        def thread_b():
            zaza_logging.set_session_context("thread_b_session")
...
class TestRecordFactory:
    ...
    def test_tag_with_context(self):
        zaza_logging.set_session_context("sess_42")
        factory = logging.getLogRecordFactory()
        record = factory("test", logging.INFO, "", 0, "msg", (), None)
    ...
    def test_works_with_any_handler(self):
        ...
        zaza_logging.set_session_context("any_handler_test")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = zaza_logging._ComponentFilter(("gateway",))
    ...
    def test_multiple_prefixes(self):
        f = zaza_logging._ComponentFilter(("agent", "run_agent", "model_tools"))
...
class TestComponentPrefixes:
    ...
    def test_gateway_prefix(self):
        assert "gateway" in zaza_logging.COMPONENT_PREFIXES
        assert ("gateway",) == zaza_logging.COMPONENT_PREFIXES["gateway"]

    def test_agent_prefix(self):
        prefixes = zaza_logging.COMPONENT_PREFIXES["agent"]
        assert "agent" in prefixes
        assert "run_agent" in prefixes
        assert "model_tools" in prefixes

    def test_tools_prefix(self):
        assert ("tools",) == zaza_logging.COMPONENT_PREFIXES["tools"]

    def test_cli_prefix(self):
        prefixes = zaza_logging.COMPONENT_PREFIXES["cli"]
        assert "zaza_cli" in prefixes
        assert "cli" in prefixes

    def test_cron_prefix(self):
        assert ("cron",) == zaza_logging.COMPONENT_PREFIXES["cron"]


class TestSetupVerboseLogging:
    """setup_verbose_logging() adds a DEBUG-level console handler."""

    def test_adds_stream_handler(self, zaza_home):
        zaza_logging.setup_logging(zaza_home=zaza_home)
        zaza_logging.setup_verbose_logging()
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_zaza_verbose", False)
        ]
        assert len(verbose_handlers) == 1
        assert verbose_handlers[0].level == logging.DEBUG

    def test_idempotent(self, zaza_home):
        zaza_logging.setup_logging(zaza_home=zaza_home)
        zaza_logging.setup_verbose_logging()
        zaza_logging.setup_verbose_logging()  # second call
...
class TestAddRotatingHandler:
    ...
    def test_creates_directory(self, tmp_path):
        ...
        zaza_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_log_filter_attached(self, tmp_path):
        ...
        formatter = logging.Formatter("%(message)s")
        component_filter = zaza_logging._ComponentFilter(("test",))
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        zaza_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("zaza_cli.config.is_managed", return_value=True):
                zaza_logging._add_rotating_handler(
                    logger, log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, zaza_home):
        level, max_size, backup = zaza_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, zaza_home):
        import yaml
    ...
    def test_handles_missing_logging_section(self, zaza_home):
        import yaml
        config = {"model": "test"}
        (zaza_home / "config.yaml").write_text(yaml.dump(config))
