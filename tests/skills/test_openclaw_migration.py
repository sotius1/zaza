...
"""
    ...
    assert "Tyler Williams > Active Projects: Agent ZAZA" in entries
...
def test_migrator_copies_skill_and_merges_allowlist(tmp_path: Path):
    ...
    target = tmp_path / ".agent-zaza/data"
    target.mkdir()
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `zaza cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # zaza_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.agent-zaza/data/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "zaza_config_mod"}
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``ZAZA``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "ZAZA prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told ZAZA to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "ZAZA config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "ZAZA uses tools well"
    # All-lowercase matches → lowercase ``zaza``; this preserves the
    # real filesystem path ``~/.agent-zaza/data`` (ZAZA home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "zaza should always respond concisely"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    # Same case-preservation rule as above.
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "ZAZA remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "zaza prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "ZAZA was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "zaza uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "ZAZA said to ask ZAZA about ZAZA settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.zaza`` (the real ZAZA home), not the broken
    ``.ZAZA``.
    ...
    rewritten to ``~/.ZAZA/config.yaml`` — a path that doesn't exist —
    and the agent kept trying to read it.
    """
    mod = load_module()
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.agent-zaza/data/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .zaza directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.agent-zaza/data'"
    # Sentence with both lowercase path and capitalized prose.
    assert mod.rebrand_text("openclaw config path: ~/.openclaw/") == \
        "zaza config path: ~/.agent-zaza/data/"
