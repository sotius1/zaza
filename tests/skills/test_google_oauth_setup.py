...
class TestZAZAConstantsFallback:
    """Tests for _zaza_home.py fallback when zaza_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_zaza_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _zaza_home.py with zaza_constants blocked."""
        monkeypatch.setitem(sys.modules, "zaza_constants", None)
        spec = importlib.util.spec_from_file_location("_zaza_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_zaza_home_env_var(self, monkeypatch, tmp_path):
        """When zaza_constants is missing, ZAZA_HOME comes from env var."""
        monkeypatch.setenv("ZAZA_HOME", str(tmp_path / "custom-zaza"))
        module = self._load_helper(monkeypatch)
        assert module.get_zaza_home() == tmp_path / "custom-zaza"

    def test_fallback_defaults_to_dot_zaza(self, monkeypatch):
        """When zaza_constants is missing and ZAZA_HOME unset, default to ~/.agent-zaza/data."""
        monkeypatch.delenv("ZAZA_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_zaza_home() == Path.home() / ".agent-zaza/data"

    def test_fallback_ignores_empty_zaza_home(self, monkeypatch):
        """Empty/whitespace ZAZA_HOME is treated as unset."""
        monkeypatch.setenv("ZAZA_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_zaza_home() == Path.home() / ".agent-zaza/data"

    def test_fallback_display_zaza_home_shortens_path(self, monkeypatch):
        """Fallback display_zaza_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("ZAZA_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_zaza_home() == "~/.agent-zaza/data"

    def test_fallback_display_zaza_home_profile_path(self, monkeypatch):
        """Fallback display_zaza_home() handles profile paths under ~/."""
        monkeypatch.setenv("ZAZA_HOME", str(Path.home() / ".zaza/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_zaza_home() == "~/.agent-zaza/data/profiles/coder"

    def test_fallback_display_zaza_home_custom_path(self, monkeypatch):
        """Fallback display_zaza_home() returns full path for non-home locations."""
        monkeypatch.setenv("ZAZA_HOME", "/opt/zaza-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_zaza_home() == "/opt/zaza-custom"

    def test_delegates_to_zaza_constants_when_available(self):
        """When zaza_constants IS importable, _zaza_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_zaza_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import zaza_constants
        assert module.get_zaza_home is zaza_constants.get_zaza_home
        assert module.display_zaza_home is zaza_constants.display_zaza_home
