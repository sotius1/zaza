"""Shared fixtures for the agent-zaza test suite.
...
2. **Isolated ZAZA_HOME.** ZAZA_HOME points to a per-test tempdir so
   code reading ``~/.agent-zaza/data/*`` via ``get_zaza_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".agent-zaza/data"`` instead of the canonical
   ``get_zaza_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No ZAZA_SESSION_* inheritance** — the agent's current gateway
...
# ZAZA_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_ZAZA_BEHAVIORAL_VARS = frozenset({
    "ZAZA_YOLO_MODE",
    "ZAZA_INTERACTIVE",
    "ZAZA_QUIET",
    "ZAZA_TOOL_PROGRESS",
    ...
    "ZAZA_MAX_ITERATIONS",
    "ZAZA_SESSION_PLATFORM",
    "ZAZA_SESSION_CHAT_ID",
    "ZAZA_SESSION_CHAT_NAME",
    "ZAZA_SESSION_THREAD_ID",
    "ZAZA_SESSION_SOURCE",
    "ZAZA_SESSION_KEY",
    "ZAZA_GATEWAY_SESSION",
    "ZAZA_PLATFORM",
    "ZAZA_INFERENCE_PROVIDER",
    "ZAZA_MANAGED",
    "ZAZA_DEV",
    "ZAZA_CONTAINER",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and ZAZA_HOME to per-test tempdirs so code that
    reads ``~/.agent-zaza/data/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral ZAZA_* vars that could change test semantics.
    for name in _ZAZA_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect ZAZA_HOME to a per-test tempdir. Code that reads
    #    ``~/.agent-zaza/data/*`` via ``get_zaza_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.agent-zaza/data/*`` via
    #    ``Path.home() / ".agent-zaza/data"`` instead of ``get_zaza_home()``
    #    is a bug to fix at the callsite.
    fake_zaza_home = tmp_path / "zaza_test"
    fake_zaza_home.mkdir()
    (fake_zaza_home / "sessions").mkdir()
    (fake_zaza_home / "cron").mkdir()
    (fake_zaza_home / "memories").mkdir()
    (fake_zaza_home / "skills").mkdir()
    monkeypatch.setenv("ZAZA_HOME", str(fake_zaza_home))
    ...
    #    ~/.agent-zaza/data/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import zaza_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_zaza_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal zaza config dict suitable for unit tests."""
    return {
