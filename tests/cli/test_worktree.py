...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("zaza/zaza-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned zaza/* and pr-* branches."""

    def test_prunes_orphaned_zaza_branch(self, git_repo):
        """zaza/zaza-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "zaza/zaza-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "zaza/zaza-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "zaza/zaza-deadbeef" in result.stdout
        ...
        assert "zaza/zaza-deadbeef" in orphaned
        ...
        assert "zaza/zaza-deadbeef" not in result.stdout
