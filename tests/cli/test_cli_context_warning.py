...
def _isolate(tmp_path, monkeypatch):
    """Isolate ZAZA_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal ZazaCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = ZazaCLI.__new__(ZazaCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["zaza-core"]
        obj.compact = False
