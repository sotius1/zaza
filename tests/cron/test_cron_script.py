...
- Path resolution (absolute, relative to ZAZA_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp ZAZA_HOME."""
    zaza_home = tmp_path / ".agent-zaza/data"
    zaza_home.mkdir()
    (zaza_home / "cron").mkdir()
    (zaza_home / "cron" / "output").mkdir()
    (zaza_home / "scripts").mkdir()
    monkeypatch.setenv("ZAZA_HOME", str(zaza_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "ZAZA_DIR", zaza_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", zaza_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", zaza_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", zaza_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.agent-zaza/data/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("ZAZA_SESSION_PLATFORM") is None
        assert os.environ.get("ZAZA_SESSION_CHAT_ID") is None
        assert os.environ.get("ZAZA_SESSION_CHAT_NAME") is None
