...
class TestConfigFilePermissions(unittest.TestCase):
    ...
    def test_save_config_sets_0600(self):
        ...
        with patch("zaza_cli.config.get_config_path", return_value=config_path), \
             patch("zaza_cli.config.ensure_zaza_home"):
    ...
    def test_save_env_value_sets_0600(self):
        ...
        with patch("zaza_cli.config.get_env_path", return_value=env_path), \
             patch("zaza_cli.config.ensure_zaza_home"):
    ...
    def test_ensure_zaza_home_sets_0700(self):
        home = Path(self.tmpdir) / ".agent-zaza/data"
        with patch("zaza_cli.config.get_zaza_home", return_value=home):
