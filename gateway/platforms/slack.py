...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        tokens_file = get_zaza_home() / "slack_tokens.json"
        ...
        try:
            ...
            # /model work directly without /zaza prefix). A single regex
            ...
            # manifest (see `zaza slack manifest`). In Socket Mode, Slack
            ...
            from zaza_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/zaza$")

            @self._app.command(_slash_pattern)
            async def handle_zaza_command(ack, command):
                await ack()
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own ZAZA session — matching the per-thread behavior channels already
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "zaza_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "zaza_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "zaza_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "zaza_approve_once": "once",
            "zaza_approve_session": "session",
            "zaza_approve_always": "always",
            "zaza_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/zaza <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/zaza
        ...
        if slash_name in ("zaza", ""):
            # Legacy /zaza <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from zaza_cli.commands import slack_subcommand_map
