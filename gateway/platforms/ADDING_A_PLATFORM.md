...
Checklist for integrating a new messaging platform into the ZAZA gateway.
...
And add it to the `zaza-gateway` composite:

```python
"zaza-gateway": {
    "includes": [..., "zaza-your-platform"]
}
```
...
## 12. Status Display (`zaza_cli/status.py`)
...
## 13. Gateway Setup Wizard (`zaza_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ zaza_cli/ toolsets.py \
  --include="*.py" -l | sort -u
