...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(os.getenv("ZAZA_DISCORD_TEXT_BATCH_DELAY_SECONDS", "0.6"))
        self._text_batch_split_delay_seconds = float(os.getenv("ZAZA_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0"))
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields ZAZA manages."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your ZAZA session")
        ...
        @tree.command(name="status", description="Show ZAZA session status")
        ...
        @tree.command(name="stop", description="Stop the running ZAZA agent")
        ...
        @tree.command(name="update", description="Update Agent ZAZA to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the ZAZA gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a ZAZA session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to ZAZA in the thread",
        ...
        # zaza_cli/commands.py automatically appear as Discord slash
        ...
        try:
            from zaza_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from zaza_cli.commands import _iter_plugin_command_entries
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        """
        try:
            from zaza_cli.commands import discord_skill_commands_by_category
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new ZAZA session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by ZAZA: **{name}**"
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        thread_name = content[:80] if content else "ZAZA"
        if len(content) > 80:
        ...
        try:
            ...
            try:
                seed_msg = await message.channel.send(f"\U0001f9f5 Thread created by ZAZA: **{thread_name}**")
                thread = await seed_msg.create_thread(
                    name=thread_name,
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``zaza update --gateway``
...
if DISCORD_AVAILABLE:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``zaza update`` prompts.
