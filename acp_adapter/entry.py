"""CLI entry point for the agent-zaza ACP adapter.

Loads environment variables from ``~/.agent-zaza/data/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from ZAZA_HOME (default ``~/.agent-zaza/data``)."""
    from zaza_cli.env_loader import load_zaza_dotenv

    zaza_home = get_zaza_home()
    loaded = load_zaza_dotenv(zaza_home=zaza_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", zaza_home / ".env"
        )
...
def main() -> None:
    ...
    logger.info("Starting agent-zaza ACP adapter")
    ...
    import acp
    from .server import ZAZAACPAgent
