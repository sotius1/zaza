"""ACP permission bridging — maps ACP approval requests to zaza approval callbacks."""
...
# Maps ACP PermissionOptionKind -> zaza approval result strings
_KIND_TO_ZAZA = {
    "allow_once": "once",
    "allow_always": "always",
    "reject_once": "deny",
...
def make_approval_callback(
    ...
    """
    Return a zaza-compatible ``approval_callback(command, description) -> str``
        ...
        if isinstance(outcome, AllowedOutcome):
            ...
            for opt in options:
                if opt.option_id == option_id:
                    return _KIND_TO_ZAZA.get(opt.kind, "deny")
