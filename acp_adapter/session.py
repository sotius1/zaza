"""ACP session manager — maps ACP sessions to ZAZA AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.agent-zaza/data/state.db``) so they
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["agent-zaza-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed ZAZA agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by ZAZA AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current ZAZA runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.agent-zaza/data/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``ZAZA_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_zaza_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=zaza_home / "state.db")
            return self._db_instance
        except Exception:
