"""ACP agent server — exposes Agent ZAZA via the Agent Client Protocol."""
...
try:
    from zaza_cli import __version__ as ZAZA_VERSION
except Exception:
    ZAZA_VERSION = "0.0.0"
...
class ZAZAACPAgent(acp.Agent):
    """ACP Agent implementation wrapping ZAZA AIAgent."""
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from zaza_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from zaza_cli.models import detect_provider_for_model, parse_model_input
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["agent-zaza-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        if provider:
            auth_methods = [
                AuthMethodAgent(
                    ...
                    description=f"Authenticate ZAZA using the currently configured {provider} runtime credentials.",
                )
            ]
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="agent-zaza", version=ZAZA_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # ZAZA' threat model (ACP is stdio-only, local-trust), but poor
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run ZAZA on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # not the executor's. Also set ZAZA_INTERACTIVE so approval.py
        # takes the CLI-interactive path (which calls the registered
        # callback via prompt_dangerous_approval) instead of the
        # non-interactive auto-approve branch (GHSA-96vc-wcxf-jjff).
        # ACP's conn.request_permission maps cleanly to the interactive
        # callback shape — not the gateway-queue ZAZA_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind ZAZA_SESSION_KEY for this session so per-session caches
            ...
            previous_interactive = os.environ.get("ZAZA_INTERACTIVE")
            ...
            try:
                ...
                # Restore ZAZA_INTERACTIVE.
                if previous_interactive is None:
                    os.environ.pop("ZAZA_INTERACTIVE", None)
                else:
                    os.environ["ZAZA_INTERACTIVE"] = previous_interactive
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            ...
            # stomp on each other's ContextVar writes (ZAZA_SESSION_KEY in
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Agent ZAZA v{ZAZA_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when ZAZA has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
