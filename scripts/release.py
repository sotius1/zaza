...
VERSION_FILE = REPO_ROOT / "zaza_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "274096618+agent-zaza-dhabibi@users.noreply.github.com": "dhabibi",
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "zaza@zaza.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/ZAZA/agent-zaza",
    ...
    lines.append(f"# Agent ZAZA v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Agent ZAZA. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Agent ZAZA Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Agent ZAZA Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Agent ZAZA v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Agent ZAZA v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Agent ZAZA v{new_version} ({calver_date})' "
