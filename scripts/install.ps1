# ============================================================================
# Zaza Agent Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   irm https://raw.githubusercontent.com/NousResearch/agent-zaza/main/scripts/install.ps1 | iex
#
...
param(
    ...
    [switch]$SkipSetup,
    [string]$Branch = "main",
    [string]$ZazaHome = "$env:LOCALAPPDATA\zaza",
    [string]$InstallDir = "$env:LOCALAPPDATA\zaza\agent-zaza"
)
...
$RepoUrlSsh = "git@github.com:NousResearch/agent-zaza.git"
$RepoUrlHttps = "https://github.com/NousResearch/agent-zaza.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "│             ⚕ Zaza Agent Installer                    │" -ForegroundColor Magenta
    Write-Host "├─────────────────────────────────────────────────────────┤" -ForegroundColor Magenta
...
function Test-Node {
    ...
    $managedNode = "$ZazaHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$ZazaHome\node;$env:Path"
        Write-Success "Node.js $version found (Zaza-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # Fallback: download binary zip to ~/.zaza/node/
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\zaza-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$ZazaHome\node") { Remove-Item -Recurse -Force "$ZazaHome\node" }
                Move-Item $extractedDir.FullName "$ZazaHome\node"
                $env:Path = "$ZazaHome\node;$env:Path"

                $version = & "$ZazaHome\node\node.exe" --version
                Write-Success "Node.js $version installed to ~/.zaza/node/"
                $script:HasNode = $true
...
function Install-Repository {
    ...
    if (Test-Path $InstallDir) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                $zipUrl = "https://github.com/NousResearch/agent-zaza/archive/refs/heads/$Branch.zip"
                $zipPath = "$env:TEMP\agent-zaza-$Branch.zip"
                $extractPath = "$env:TEMP\agent-zaza-extract"
...
function Set-PathVariable {
    Write-Info "Setting up zaza command..."
    
    if ($NoVenv) {
        $zazaBin = "$InstallDir"
    } else {
        $zazaBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so zaza is globally available
    # On Windows, the zaza.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$zazaBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$zazaBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $zazaBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set ZAZA_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\zaza instead
    # of the Unix default ~/.zaza
    $currentZazaHome = [Environment]::GetEnvironmentVariable("ZAZA_HOME", "User")
    if (-not $currentZazaHome -or $currentZazaHome -ne $ZazaHome) {
        [Environment]::SetEnvironmentVariable("ZAZA_HOME", $ZazaHome, "User")
        Write-Success "Set ZAZA_HOME=$ZazaHome"
    }
    $env:ZAZA_HOME = $ZazaHome
    
    # Update current session
    $env:Path = "$zazaBin;$env:Path"
    
    Write-Success "zaza command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.zaza directory structure
    New-Item -ItemType Directory -Force -Path "$ZazaHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZazaHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZazaHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZazaHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZazaHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZazaHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZazaHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZazaHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ZazaHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$ZazaHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.zaza/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.zaza/.env"
        }
    } else {
        Write-Info "~/.zaza/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$ZazaHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.zaza/config.yaml from template"
        }
    } else {
        Write-Info "~/.zaza/config.yaml already exists, keeping it"
    }
    
    # Create SOUL.md if it doesn't exist (global persona file)
    $soulPath = "$ZazaHome\SOUL.md"
...
"@ | Set-Content -Path $soulPath -Encoding UTF8
        Write-Success "Created ~/.zaza/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.zaza/"
    
    # Seed bundled skills into ~/.zaza/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.zaza/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.zaza/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$ZazaHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.zaza/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        try {
            ...
            Write-Warn "TUI npm install failed (zaza --tui may not work)"
        }
        Pop-Location
    }
...
function Invoke-SetupWizard {
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m zaza_cli.main setup
    } else {
        python -m zaza_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $zazaCmd = "$InstallDir\venv\Scripts\zaza.exe"
    if (-not (Test-Path $zazaCmd)) {
        $zazaCmd = "zaza"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$ZazaHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'zaza whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$ZazaHome\logs\gateway.log"
            Start-Process -FilePath $zazaCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$ZazaHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: zaza gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: zaza gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$ZazaHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$ZazaHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ZazaHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ZazaHome\agent-zaza\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   zaza              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   zaza setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   zaza config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   zaza config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   zaza gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   zaza update       " -NoNewline -ForegroundColor Green
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/NousResearch/agent-zaza/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
