#!/bin/bash
# ============================================================================
# Agent ZAZA Installer
# ============================================================================
# Installation script for Linux, macOS, and Android/Termux.
# Uses uv for desktop/server installs and Python's stdlib venv + pip on Termux.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/NousResearch/agent-zaza/main/scripts/install.sh | bash
#
# Or with options:
...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/agent-zaza.git"
REPO_URL_HTTPS="https://github.com/NousResearch/agent-zaza.git"
ZAZA_HOME="${ZAZA_HOME:-$HOME/.zaza}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${ZAZA_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$ZAZA_INSTALL_DIR"
...
#   code at /usr/local/lib/agent-zaza, command at /usr/local/bin/zaza,
#   data still at /root/.zaza (ZAZA_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.agent-zaza/data/agent-zaza"
            echo "                   default (root, Linux): /usr/local/lib/agent-zaza"
            echo "  --zaza-home PATH  Data directory (default: ~/.agent-zaza/data, or \$ZAZA_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, ZAZA installs the code under"
            echo "  /usr/local/lib/agent-zaza and links the command into"
            echo "  /usr/local/bin/zaza (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$ZAZA_HOME"
            echo "  (default /root/.zaza).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$ZAZA_HOME/agent-zaza are preserved in-place."
            exit 0
            ;;
        *)
...
# Decide where the repo checkout + venv live, and where the `zaza` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $ZAZA_HOME/agent-zaza
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $ZAZA_HOME/agent-zaza
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/agent-zaza
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $ZAZA_HOME/agent-zaza — then preserve it)
#
# Always no-op when the user set --dir or $ZAZA_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in ZAZA_HOME.
    if is_termux; then
        INSTALL_DIR="$ZAZA_HOME/agent-zaza"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$ZAZA_HOME/agent-zaza/.git" ]; then
            INSTALL_DIR="$ZAZA_HOME/agent-zaza"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/agent-zaza)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/agent-zaza"
        ROOT_FHS_LAYOUT=true
        log_info "Root install on Linux — using FHS layout"
        log_info "  Code:    $INSTALL_DIR"
        log_info "  Command: /usr/local/bin/zaza"
        log_info "  Data:    $ZAZA_HOME (unchanged)"
        return 0
    fi
...
get_zaza_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/zaza" ]; then
        echo "$link_dir/zaza"
    else
        echo "zaza"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/NousResearch/agent-zaza/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if [ -x "$ZAZA_HOME/node/bin/node" ]; then
        export PATH="$ZAZA_HOME/node/bin:$PATH"
        local found_ver=$("$ZAZA_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (ZAZA-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.agent-zaza/data/node/..."
    ...
    # Place into ~/.agent-zaza/data/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$ZAZA_HOME/node"
    mkdir -p "$ZAZA_HOME"
    mv "$extracted_dir" "$ZAZA_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$ZAZA_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$ZAZA_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$ZAZA_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$ZAZA_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$ZAZA_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.agent-zaza/data/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Agent ZAZA itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="zaza-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if ZAZA behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up zaza command..."

    if [ "$USE_VENV" = true ]; then
        ZAZA_BIN="$INSTALL_DIR/venv/bin/zaza"
    else
        ZAZA_BIN="$(which zaza 2>/dev/null || echo "")"
        if [ -z "$ZAZA_BIN" ]; then
            log_warn "zaza not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$ZAZA_BIN" ]; then
        log_warn "zaza entry point not found at $ZAZA_BIN"
    ...
    # Create a user-facing shim for the zaza command.
    mkdir -p "$command_link_dir"
    ln -sf "$ZAZA_BIN" "$command_link_dir/zaza"
    log_success "Symlinked zaza → $command_link_display_dir/zaza"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "zaza command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v zaza' \
                >/dev/null 2>&1; then
        ...
        log_info "zaza not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Agent ZAZA — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Agent ZAZA — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Agent ZAZA — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so zaza works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.agent-zaza/data directory structure (config at top level, code in subdir)
    mkdir -p "$ZAZA_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.agent-zaza/data/.env (top level, easy to find)
    if [ ! -f "$ZAZA_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$ZAZA_HOME/.env"
            log_success "Created ~/.agent-zaza/data/.env from template"
        else
            touch "$ZAZA_HOME/.env"
            log_success "Created ~/.agent-zaza/data/.env"
        fi
    else
        log_info "~/.agent-zaza/data/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.agent-zaza/data/config.yaml (top level, easy to find)
    if [ ! -f "$ZAZA_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$ZAZA_HOME/config.yaml"
            log_success "Created ~/.agent-zaza/data/config.yaml from template"
        fi
    else
        log_info "~/.agent-zaza/data/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$ZAZA_HOME/SOUL.md" ]; then
        cat > "$ZAZA_HOME/SOUL.md" << 'SOUL_EOF'
# Agent ZAZA Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how ZAZA communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.agent-zaza/data/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.agent-zaza/data/"

    # Seed bundled skills into ~/.agent-zaza/data/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.agent-zaza/data/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.agent-zaza/data/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$ZAZA_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$ZAZA_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.agent-zaza/data/skills/"
        fi
    fi
}
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ubuntu|debian|raspbian|pop|linuxmint|elementary|zorin|kali|parrot)
                ...
                log_info "This is standard Playwright setup — ZAZA itself does not require root access."
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (zaza --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'zaza setup' after install."
        return 0
    fi
    ...
    # Run zaza setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m zaza_cli.main setup < /dev/tty
    else
        python -m zaza_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$ZAZA_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for ZAZA to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$ZAZA_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'zaza whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                ZAZA_CMD="$(get_zaza_command_path)"
                $ZAZA_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'zaza whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'zaza gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $ZAZA_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $ZAZA_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: zaza gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: zaza gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $ZAZA_CMD gateway > "$ZAZA_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.agent-zaza/data/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: zaza gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: zaza gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $ZAZA_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $ZAZA_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $ZAZA_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}zaza${NC}              Start chatting"
    echo -e "   ${GREEN}zaza setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}zaza config${NC}       View/edit configuration"
    echo -e "   ${GREEN}zaza config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}zaza gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}zaza update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'zaza' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'zaza' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'zaza' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
