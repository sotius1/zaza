#!/usr/bin/env python3
"""Drive the ZAZA TUI under ZAZA_DEV_PERF and summarize the pipeline.
...
~30 Hz (matching xterm key-repeat), summarizes ~/.agent-zaza/data/perf.log on exit.

The --tui build must exist (run `npm run build` in ui-tui first). This script
launches `node dist/entry.js` directly with ZAZA_TUI_RESUME set so it
bypasses the zaza_cli wrapper — we want repeatable timing, not the CLI's
session-picker flow.

Environment overrides:
  ZAZA_PERF_LOG     (default ~/.agent-zaza/data/perf.log)
  ZAZA_PERF_NODE    (default node from $PATH)
  ZAZA_TUI_DIR      (default /home/bb/agent-zaza/ui-tui)

Exit code is 0 if the harness ran and parsed results, 2 if the TUI crashed
or produced no perf data (suggests ZAZA_DEV_PERF wiring is broken).
"""
...
DEFAULT_TUI_DIR = Path(os.environ.get("ZAZA_TUI_DIR", "/home/bb/agent-zaza/ui-tui"))
DEFAULT_LOG = Path(os.environ.get("ZAZA_PERF_LOG", str(Path.home() / ".agent-zaza/data" / "perf.log")))
DEFAULT_STATE_DB = Path.home() / ".agent-zaza/data" / "state.db"
...
def format_report(data: dict[str, Any]) -> str:
    ...
    if not react:
        out.append("  (no react events — ZAZA_DEV_PERF wired? threshold too high?)")
    else:
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["ZAZA_DEV_PERF"] = "1"
    env["ZAZA_DEV_PERF_MS"] = str(args.threshold_ms)
    env["ZAZA_DEV_PERF_LOG"] = str(log)
    env["ZAZA_TUI_RESUME"] = sid
    ...
    node = os.environ.get("ZAZA_PERF_NODE", "node")
...
def main() -> int:
    ...
    p.add_argument("--threshold-ms", type=float, default=0.0, help="ZAZA_DEV_PERF_MS (0 = capture all)")
...
def loop_mode(args: argparse.Namespace) -> int:
    ...
    pkg_root = tui_dir / "packages" / "zaza-ink" / "src"
