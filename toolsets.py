...
_ZAZA_CORE_TOOLS = [
    ...
    # Browser harness — direct CDP into user's real Chrome
    "browser_harness_run", "browser_harness_doctor", "browser_harness_reload",
    # Text-to-speech
    "text_to_speech",
    # Planning & memory
    "todo", "memory",
...
TOOLSETS = {
    ...
    "browser_harness": {
        "description": "Direct CDP control over the user's real Chrome via browser-harness — self-healing harness, agent edits helpers.py mid-task, coordinate clicks, raw CDP, remote Browser Use cloud sessions. 10x more capable than the abstracted browser_* tools for any non-trivial site.",
        "tools": [
            "browser_harness_run", "browser_harness_doctor", "browser_harness_reload",
            "web_search"
        ],
    ...
    # ==========================================================================
    # Full ZAZA toolsets (CLI + messaging platforms)
    #
    ...
    "agent-zaza-cli-toolset": {
        "description": "Full interactive CLI toolset - all default tools plus cronjob management",
        "tools": _ZAZA_CORE_TOOLS,
        "includes": []
    },

    "agent-zaza-cron": {
        # Mirrors agent-zaza-cli-toolset so cron's "default" toolset is the same set of
        # core tools users see interactively — then `zaza tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant, rl) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as agent-zaza-cli-toolset; gated by `zaza tools`",
        "tools": _ZAZA_CORE_TOOLS,
        "includes": []
    },
    ...
    "zaza-email": {
        "description": "Email bot toolset - interact with ZAZA via email (IMAP/SMTP)",
        "tools": _ZAZA_CORE_TOOLS,
        "includes": []
    },
    ...
    "zaza-sms": {
        "description": "SMS bot toolset - interact with ZAZA via SMS (Twilio)",
        "tools": _ZAZA_CORE_TOOLS,
        "includes": []
    },
    ...
    "agent-zaza-gateway": {
        ...
        "tools": [],
        "includes": ["zaza-telegram", "zaza-discord", "zaza-whatsapp", "zaza-slack", "zaza-signal", "zaza-bluebubbles", "zaza-homeassistant", "zaza-email", "zaza-sms", "zaza-mattermost", "zaza-matrix", "zaza-dingtalk", "zaza-feishu", "zaza-wecom", "zaza-wecom-callback", "zaza-weixin", "zaza-qqbot", "zaza-webhook", "zaza-yuanbao"]
    }
}
