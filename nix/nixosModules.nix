# nix/nixosModules.nix — NixOS module for agent-zaza
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: zaza runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $ZAZA_HOME/.env
# and read by zaza at startup — no container recreation needed for env changes.
#
# Tool resolution: the zaza wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.agent-zaza;
    effectivePackage = if cfg.extraPythonPackages == [ ] then cfg.package
      else cfg.package.override { inherit (cfg) extraPythonPackages; };
    agent-zaza = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-agent-zaza)
    deepConfigType = lib.types.mkOptionType {
      name = "zaza-config-attrs";
      description = "ZAZA YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "zaza-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "zaza-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'ZAZA_DOC_EOF'\n${value}\nZAZA_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "agent-zaza";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/zaza";
    ...
    # zaza user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "zaza-container-entrypoint" ''
      set -eu

      ZAZA_UID="''${ZAZA_UID:?ZAZA_UID must be set}"
      ZAZA_GID="''${ZAZA_GID:?ZAZA_GID must be set}"

      # ── Group: ensure a group with GID=$ZAZA_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$ZAZA_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="zaza"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$ZAZA_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$ZAZA_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$ZAZA_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$ZAZA_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="zaza"
        TARGET_HOME="/home/zaza"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$ZAZA_UID" -g "$ZAZA_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$ZAZA_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$ZAZA_UID:$ZAZA_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure ZAZA_HOME is owned by the target user
      if [ -n "''${ZAZA_HOME:-}" ] && [ -d "$ZAZA_HOME" ]; then
        chown -R "$ZAZA_UID:$ZAZA_GID" "$ZAZA_HOME"
      fi
      ...
      if [ ! -f /var/lib/zaza-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/zaza-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/zaza ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/zaza
        chmod 0440 /etc/sudoers.d/zaza
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$ZAZA_UID" --regid="$ZAZA_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $ZAZA_HOME/.env.
    ...
    # Default: /var/lib/zaza/workspace → /data/workspace.
  ...
  in {
    options.services.agent-zaza = with lib; {
      enable = mkEnableOption "Agent ZAZA gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = agent-zaza;
        description = "The agent-zaza package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/zaza";
        description = "State directory. Contains .zaza/ subdir (ZAZA_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative ZAZA config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $ZAZA_HOME/.env at activation time.
          ZAZA reads this file on every startup via load_zaza_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $ZAZA_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $ZAZA_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `zaza gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the zaza user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the zaza plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. ZAZA discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          zaza_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as zaza (python312).
        '';
        example = literalExpression ''
          [
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the zaza CLI to environment.systemPackages and export
          ZAZA_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.agent-zaza/data symlink to the service
            stateDir. These users are automatically added to the zaza group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.agent-zaza.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the zaza CLI to system PATH and export ZAZA_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.agent-zaza/data/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.ZAZA_HOME = "${cfg.stateDir}/.zaza";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.agent-zaza.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }

      # ── Warnings ──────────────────────────────────────────────────────
      # ── Per-user profile for extraPackages ───────────────────────────
      # Wire extraPackages into the zaza user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.agent-zaza: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed zaza
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure zaza is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.zaza        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.agent-zaza/data/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.agent-zaza/data/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.agent-zaza/data/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.agent-zaza/data/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.agent-zaza/data/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."agent-zaza-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.zaza
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.zaza ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.zaza ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.zaza -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.agent-zaza/data/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.agent-zaza/data/$_subdir"
            chmod 2770 "${cfg.stateDir}/.agent-zaza/data/$_subdir"
            find "${cfg.stateDir}/.agent-zaza/data/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.agent-zaza/data/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.agent-zaza/data/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.agent-zaza/data/config.yaml
            chmod 0640 ${cfg.stateDir}/.agent-zaza/data/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.agent-zaza/data/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.agent-zaza/data/.managed
          chmod 0644 ${cfg.stateDir}/.agent-zaza/data/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.agent-zaza/data/.container-mode <<'ZAZA_CONTAINER_MODE_EOF'
# Written by NixOS activation script. Do not edit manually.
backend=${cfg.container.backend}
container_name=${containerName}
exec_user=${cfg.user}
zaza_bin=${containerDataDir}/current-package/bin/zaza
ZAZA_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.agent-zaza/data/.container-mode
            chmod 0644 ${cfg.stateDir}/.agent-zaza/data/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.agent-zaza/data/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.zaza";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.zaza" ]; then
                  rm -f "${symlinkPath}"
                  echo "agent-zaza: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.agent-zaza/data -> stateDir/.zaza for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.zaza";
                target = "${cfg.stateDir}/.zaza";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "agent-zaza: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.agent-zaza/data/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.agent-zaza/data/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.agent-zaza/data/auth.json
              fi
            ''}
          ''}
          ...
          # ZAZA reads $ZAZA_HOME/.env at startup via load_zaza_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.agent-zaza/data/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'ZAZA_NIX_ENV_EOF'
${envFileContent}
ZAZA_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.agent-zaza/data/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.agent-zaza/data/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.agent-zaza/data/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.agent-zaza = {
          description = "Agent ZAZA Gateway";
          ...
          serviceConfig = {
            ...
            # $ZAZA_HOME/.env by the activation script. load_zaza_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/zaza"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the zaza group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.agent-zaza = {
          description = "Agent ZAZA Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              ZAZA_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              ZAZA_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env ZAZA_UID="$ZAZA_UID" \
                --env ZAZA_GID="$ZAZA_GID" \
                --env ZAZA_HOME=${containerDataDir}/.zaza \
                --env ZAZA_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/zaza gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
