...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "zaza-cross-eval" { } (
          if failures != [] then
        ...
        package-contents = pkgs.runCommand "zaza-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${agent-zaza}/bin/zaza || (echo "FAIL: zaza binary missing"; exit 1)
          test -x ${agent-zaza}/bin/agent-zaza || (echo "FAIL: agent-zaza binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${agent-zaza}/bin/zaza version 2>&1 | grep -qi "zaza" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "zaza-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in zaza agent-zaza zaza-acp; do
            test -x ${agent-zaza}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "zaza-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking zaza --help ==="
          ${agent-zaza}/bin/zaza --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${agent-zaza}/bin/zaza --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "zaza-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${agent-zaza}/share/agent-zaza/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${agent-zaza}/share/agent-zaza/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "ZAZA_BUNDLED_SKILLS" ${agent-zaza}/bin/zaza || \
            (echo "FAIL: ZAZA_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: ZAZA_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-tui = pkgs.runCommand "zaza-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${agent-zaza}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${agent-zaza}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          test -d ${agent-zaza}/ui-tui/node_modules || (echo "FAIL: node_modules missing"; exit 1)
          echo "PASS: node_modules present"

          grep -q "ZAZA_TUI_DIR" ${agent-zaza}/bin/zaza || \
            (echo "FAIL: ZAZA_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: ZAZA_TUI_DIR set in wrapper"
        ...
        # Verify ZAZA_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        zaza-node = pkgs.runCommand "zaza-node-version" { } ''
          set -e
          echo "=== Checking ZAZA_NODE in wrapper ==="
          grep -q "ZAZA_NODE" ${agent-zaza}/bin/zaza || \
            (echo "FAIL: ZAZA_NODE not set in wrapper"; exit 1)
          echo "PASS: ZAZA_NODE present in wrapper"

          ZAZA_NODE=$(sed -n "s/^export ZAZA_NODE='\(.*\)'/\1/p" ${agent-zaza}/bin/zaza)
          test -x "$ZAZA_NODE" || (echo "FAIL: ZAZA_NODE=$ZAZA_NODE not executable"; exit 1)
          echo "PASS: ZAZA_NODE executable at $ZAZA_NODE"

          NODE_MAJOR=$("$ZAZA_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All ZAZA_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify ZAZA_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "zaza-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(ZAZA_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking ZAZA_MANAGED guards ==="
          check_blocked "config set" ${agent-zaza}/bin/zaza config set model foo
          check_blocked "config edit" ${agent-zaza}/bin/zaza config edit
        ...
        extra-python-packages = let
          ...
          zazaWithExtra = agent-zaza.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "zaza-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${zazaWithExtra}/bin/zaza || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${zazaWithExtra}/bin/zaza || \
          ...
          if grep -q "PYTHONPATH" ${agent-zaza}/bin/zaza; then
        ...
        in pkgs.runCommand "zaza-config-roundtrip" {
          ...
          merge_and_load() {
            local zaza_home="$1"
            export ZAZA_HOME="$zaza_home"
            ${configMergeScript} ${nixSettings} "$zaza_home/config.yaml"
            ${zazaVenv}/bin/python3 -c '
import json, sys
