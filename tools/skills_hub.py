#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the ZAZA Skills Hub.
...
Used by zaza_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            zaza_meta = metadata.get("zaza", {})
            if isinstance(zaza_meta, dict):
                tags = zaza_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by ZAZA) but not activated
...
# ---------------------------------------------------------------------------
# ZAZA centralized index source
# ---------------------------------------------------------------------------

ZAZA_INDEX_URL = "https://agent-zaza.zaza.com/docs/api/skills-index.json"
ZAZA_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "zaza-index.json"
ZAZA_INDEX_TTL = 6 * 3600  # 6 hours


def _load_zaza_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for ZAZA_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if ZAZA_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - ZAZA_INDEX_CACHE_FILE.stat().st_mtime
            if age < ZAZA_INDEX_TTL:
                return json.loads(ZAZA_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(ZAZA_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("ZAZA index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("ZAZA index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        ZAZA_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        ZAZA_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class ZAZAIndexSource(SkillSource):
    """Skill source backed by the centralized ZAZA Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "zaza-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "zaza-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        ZAZAIndexSource(auth=auth), # Centralized index (search + resolved install paths)
