...
Tracks per-skill usage metadata in a sidecar JSON file (~/.agent-zaza/data/skills/.usage.json)
...
def _read_bundled_manifest_names() -> Set[str]:
    ...
    Reads ~/.agent-zaza/data/skills/.bundled_manifest (format: "name:hash" per line).
...
def _read_hub_installed_names() -> Set[str]:
    ...
    Reads ~/.agent-zaza/data/skills/.hub/lock.json (see tools/skills_hub.py :: HubLockFile).
    """
...
def archive_skill(skill_name: str) -> Tuple[bool, str]:
    """Move an agent-created skill directory to ~/.agent-zaza/data/skills/.archive/.
...
def restore_skill(skill_name: str) -> Tuple[bool, str]:
    """Move an archived skill back to ~/.agent-zaza/data/skills/. Restores to the flat
...
def _find_skill_dir(skill_name: str) -> Optional[Path]:
    ...
    Handles both flat (~/.agent-zaza/data/skills/<skill>/SKILL.md) and category-nested
    (~/.agent-zaza/data/skills/<category>/<skill>/SKILL.md) layouts.
    """
    base = _skills_dir()
    if not base.exists():
        return None
