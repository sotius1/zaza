...
def managed_nous_tools_enabled() -> bool:
    ...
    try:
        ...
        from zaza_cli.models import check_nous_free_tier
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.agent-zaza/data/.env`` (via
    ``zaza_cli.config.get_env_value`` when available) so tool-side
