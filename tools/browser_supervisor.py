...
One ``CDPSupervisor`` runs per ZAZA ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "zaza-dialog-bridge.invalid"
...
(() => {
  if (window.__zazaDialogBridgeInstalled) return;
  window.__zazaDialogBridgeInstalled = true;
  const ENDPOINT = "http://zaza-dialog-bridge.invalid/";
