...
This module provides the glue:
    - ``ZAZATokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Agent ZAZA"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses ZAZA_HOME so each profile gets its own OAuth tokens.
    Layout: ``ZAZA_HOME/mcp-tokens/``
    """
    try:
        from zaza_constants import get_zaza_home
        base = Path(get_zaza_home())
    except ImportError:
        base = Path(os.environ.get("ZAZA_HOME", str(Path.home() / ".agent-zaza/data")))
...
# ---------------------------------------------------------------------------
# ZAZATokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class ZAZATokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        ZAZA_HOME/mcp-tokens/<server_name>.json         -- tokens
        ZAZA_HOME/mcp-tokens/<server_name>.client.json   -- client info
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # ZAZA records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to ZAZA.</p></body></html>"
            ) if code else (
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Agent ZAZA")
    scope = cfg.get("scope")
