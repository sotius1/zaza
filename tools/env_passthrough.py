...
def _is_zaza_provider_credential(name: str) -> bool:
    """True if ``name`` is a ZAZA-managed provider credential (API key,
    token, or similar) per ``_ZAZA_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-ZAZA API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    in the blocklist and remain legitimately registerable — skills that
    wrap third-party APIs still work.
    """
    try:
        from tools.environments.local import _ZAZA_PROVIDER_ENV_BLOCKLIST
    except Exception:
        return False
    return name in _ZAZA_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are ZAZA-managed provider credentials (from
    ``_ZAZA_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a ZAZA-managed
    ...
    Non-ZAZA third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_zaza_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register ZAZA provider "
                "credential %r (blocked by _ZAZA_PROVIDER_ENV_BLOCKLIST). "
