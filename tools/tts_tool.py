...
Configuration is loaded from ~/.agent-zaza/data/config.yaml under the 'tts:' key.
...
from tools.xai_http import zaza_xai_user_agent
...
def _get_default_output_dir() -> str:
    ...
    return str(get_zaza_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.agent-zaza/data/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.agent-zaza/data/config.yaml.
    ...
    try:
        ...
        logger.debug("zaza_cli.config not available, using default TTS config")
        return {}
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    # output_format when ZAZA actually needs a non-default format/override.
    ...
    response = requests.post(
        ...
        headers={
            ...
            "User-Agent": zaza_xai_user_agent(),
        },
        json=payload,
        timeout=60,
    )
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.agent-zaza/data/config.yaml (tts: section).
    ...
    platform = get_session_env("ZAZA_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'agent-zaza[mistral]'"
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run zaza setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'zaza setup tts' and choose KittenTTS, or install manually: "
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_zaza_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
