...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/zaza-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/zaza-results/{tool_use_id}.txt on Termux)
...
STORAGE_DIR = "/tmp/zaza-results"
HEREDOC_MARKER = "ZAZA_PERSIST_EOF"
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/zaza-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"ZAZA_PERSIST_{uuid.uuid4().hex[:8]}"
...
def maybe_persist_tool_result(
    ...
    # Sandbox unavailable — fall back to ZAZA's local tool-output store so
    # the agent can still expand the full output later via fetch_tool_output.
    # Better than a lossy inline truncation: nothing is permanently dropped.
    try:
        from agent.tool_output_store import maybe_offload
        offloaded = maybe_offload(
            content,
            tool_name=tool_name,
            tool_args={"tool_call_id": tool_use_id},
            threshold_bytes=0,  # we already know we want to offload
        )
        if offloaded is not None:
            logger.info(
                "Offloaded large tool result locally: %s (%s, %d bytes -> %s)",
                tool_name, tool_use_id, offloaded.size_bytes, offloaded.reference,
            )
            return offloaded.inline_text
    except Exception as exc:
        logger.warning("Local offload failed for %s: %s", tool_use_id, exc)
