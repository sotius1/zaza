...
it is automatically downloaded from GitHub releases to $ZAZA_HOME/bin/tirith.
...
def _get_zaza_home() -> str:
    """Return the ZAZA home directory, respecting ZAZA_HOME env var."""
    return str(get_zaza_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_zaza_home(), ".tirith-install-failed")
...
def _zaza_bin_dir() -> str:
    """Return $ZAZA_HOME/bin, creating it if needed."""
    d = os.path.join(_get_zaza_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $ZAZA_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_zaza_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $ZAZA_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $ZAZA_HOME/bin/tirith
    ...
    zaza_bin = os.path.join(_zaza_bin_dir(), "tirith")
    if os.path.isfile(zaza_bin) and os.access(zaza_bin, os.X_OK):
        _resolved_path = zaza_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return zaza_bin
