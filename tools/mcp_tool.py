...
discovers their tools, and registers them into the agent-zaza tool registry
so the agent can call them like any built-in tool.

Configuration is read from ~/.agent-zaza/data/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.agent-zaza/data/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            zaza_home = os.path.expanduser(
                os.getenv(
                    "ZAZA_HOME", os.path.join(os.path.expanduser("~"), ".agent-zaza/data")
                )
            )
            candidates = [
                os.path.join(zaza_home, "node", "bin", resolved_command),
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # Latched True after the first successful list_tools handshake.
        # The welcome banner reads this to render a green status even
        # when ``self.session`` momentarily flickers to None (between
        # reconnects, during probe handshakes).  Set in
        # ``_discover_tools``; never cleared without explicit shutdown.
        self._initial_connect_succeeded: bool = False
        self._config: dict = {}
    ...
    async def _run_stdio(self, config: dict):
        ...
        # ~/.agent-zaza/data/logs/mcp-stderr.log.
    ...
    async def _discover_tools(self):
        ...
        Latches ``_initial_connect_succeeded=True`` after the first
        successful handshake so the welcome banner can report a green
        status even if the session reference flickers later (e.g.
        ...
        if self._tools is not None:
            self._initial_connect_succeeded = True
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `zaza mcp login {server_name}` (or delete the tokens "
            f"file under ~/.agent-zaza/data/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the ZAZA config file.
    ...
    ``os.environ`` (which includes ``~/.agent-zaza/data/.env`` loaded at startup).
    """
    try:
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves ZAZA's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the ZAZA registry schema format.
...
def get_mcp_status() -> List[dict]:
    ...
    for name, cfg in configured.items():
        ...
        # Use the lifecycle flag as the source of truth for the panel
        # display.  ``server.session`` flickers to None during normal
        # reconnect cycles and during probe handshakes, which historically
        # made the welcome banner show "failed" for healthy servers
        # immediately after start-up.  ``_initial_connect_succeeded``
        # latches True after the first successful list_tools handshake
        # and only flips back to False if the task is shut down for good.
        tool_count = 0
        ...
        if server is not None and (
            ...
            or getattr(server, "_initial_connect_succeeded", False)
            or tool_count > 0
        ):
            entry = {
                "name": name,
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``zaza tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the ZAZA registry.
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple zaza
