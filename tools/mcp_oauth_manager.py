...
`mcp_tool.py`, and `zaza_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# ZAZAMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_zaza_provider_class() -> Optional[type]:
    ...
    class ZAZAMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._zaza_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`ZAZATokenStorage` persisting an absolute
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._zaza_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        logger.debug(
                            ...
                            "token_endpoint=%s",
                            self._zaza_server_name, asm.token_endpoint,
                        )
                        break
    ...
    return ZAZAMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_ZAZA_PROVIDER_CLS: Optional[type] = _make_zaza_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`ZAZAMCPOAuthProvider` directly using the helpers
        ...
        storage = ZAZATokenStorage(server_name)
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``zaza mcp remove <name>`` and (indirectly) by
        ``zaza mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
