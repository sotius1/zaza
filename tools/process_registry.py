...
CHECKPOINT_PATH = get_zaza_home() / "processes.json"
...
class ProcessRegistry:
    ...
    def spawn_via_env(
        ...
        log_path = f"{temp_dir}/zaza_bg_{session.id}.log"
        pid_path = f"{temp_dir}/zaza_bg_{session.id}.pid"
        exit_path = f"{temp_dir}/zaza_bg_{session.id}.exit"
    ...
    def _reconcile_local_exit(self, session: "ProcessSession") -> None:
        ...
        a daemon spawned by `zaza update` restarting the gateway) is still
