...
Then set ``CAMOFOX_URL=http://localhost:9377`` in ``~/.agent-zaza/data/.env``.
"""
...
from zaza_cli.config import cfg_get, load_config
...
def _managed_persistence_enabled() -> bool:
    """Return whether ZAZA-managed persistence is enabled for Camofox.
...
def _get_session(task_id: Optional[str]) -> Dict[str, Any]:
    ...
    derived from the ZAZA profile so the Camofox server can map it
    ...
    with _sessions_lock:
        ...
        if _managed_persistence_enabled():
            ...
            session = {
                "user_id": f"zaza_{uuid.uuid4().hex[:10]}",
                "tab_id": None,
...
def camofox_vision(question: str, annotate: bool = False,
    ...
    try:
        ...
        screenshots_dir = get_zaza_home() / "browser_screenshots"
